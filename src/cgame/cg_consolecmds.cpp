...
qboolean CG_ConsoleCommand( void )
{
  ...
  cmd = static_cast<consoleCommand_t*>(bsearch( CG_Argv(0), commands,
              ARRAY_LEN(commands), sizeof(commands[0]), cmdcmp));
...
qboolean CG_Console_CompleteArgument( int argNum )
{
  ...
  cmd = static_cast<consoleCommandCompletions_t*>(bsearch(CG_Argv(0) + 1,
              commandCompletions, ARRAY_LEN(commandCompletions),
              sizeof(commandCompletions[0]), cmdcmp));
