...
======================
*/
static int CG_GetCorpseNum( class_t _class )
{
    int           i;
    clientInfo_t*  match;
    char*          modelName;
    ...
    modelName = BG_ClassConfig( _class )->modelName;
    skinName = BG_ClassConfig( _class )->skinName;
...
======================
*/
void CG_PrecacheClientInfo( class_t _class, char* model, char* skin )
{
    clientInfo_t*  ci;
    ...
    if( old->team != _new->team )
    {
        if( _new->team == TEAM_NONE )
            ...
            CG_Printf( "%s" S_COLOR_WHITE " joined the %ss\n", _new->name,
                       BG_TeamName( ( team_t )_new->team ) );
        else
            CG_Printf( "%s" S_COLOR_WHITE " left the %ss and joined the %ss\n",
                       _new->name, BG_TeamName( old->team ), BG_TeamName( ( team_t )_new->team ) );
    }
}
...
static bool CG_PlayerShadow( centity_t* cent, float* shadowPlane, class_t _class )
{
    ...
    BG_ClassBoundingBox( _class, mins, maxs, NULL, NULL, NULL );
    mins[ 2 ] = 0.0f;
    maxs[ 2 ] = 2.0f;
    ...
    *shadowPlane = 0;
    ...
    CG_ImpactMark( cgs.media.shadowMarkShader, trace.endpos, trace.plane.normal,
                   ...
                   24.0f * BG_ClassConfig( _class )->shadowScale, true );
...
===============
*/
static void CG_PlayerSplash( centity_t* cent, class_t _class )
{
    vec3_t      start, end;
    vec3_t      mins, maxs;
    trace_t     trace;
    ...
    CG_ImpactMark( cgs.media.wakeMarkShader, trace.endpos, trace.plane.normal,
                   ...
                   32.0f * BG_ClassConfig( _class )->shadowScale, true );
}
...
void CG_Player( centity_t* cent )
{
    ...
    //rescale the model
    scale = BG_ClassConfig( _class )->modelScale;
    ...
    VectorMA( legs.origin, BG_ClassConfig( _class )->zOffset, surfNormal, legs.origin );
    VectorCopy( legs.origin, legs.lightingOrigin );
    VectorCopy( legs.origin, legs.oldorigin ); // don't positionally lerp at all
...
void CG_Corpse( centity_t* cent )
{
    ...
    BG_ClassBoundingBox( ( class_t )es->clientNum, aliveZ, NULL, NULL, deadZ, NULL );
    ...
    legs.shadowPlane = shadowPlane;
    legs.renderfx = renderfx;
    legs.origin[ 2 ] += BG_ClassConfig( ( class_t )es->clientNum )->zOffset;
    VectorCopy( legs.origin, legs.oldorigin ); // don't positionally lerp at all
    
    //rescale the model
    scale = BG_ClassConfig( ( class_t )es->clientNum )->modelScale;
