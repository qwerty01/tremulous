...
const char *cg_customSoundNames[MAX_CUSTOM_SOUNDS] = {"*death1.wav", "*death2.wav", "*death3.wav", "*jump1.wav",
    "*pain25_1.wav", "*pain50_1.wav", "*pain75_1.wav", "*pain100_1.wav", "*falling1.wav", "*gasp.wav", "*drown.wav",
    "*fall1.wav", "*taunt.wav"};
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            if (!ci->customMetalFootsteps[i])
                ci->customMetalFootsteps[i] =
                    trap_S_RegisterSound(va("sound/player/footsteps/clank%d.wav", i + 1), false);
        }
    }
...
======================
*/
static int CG_GetCorpseNum(class_t klass)
{
    int i;
    clientInfo_t *match;
    char *modelName;
    ...
    modelName = BG_ClassConfig(klass)->modelName;
    skinName = BG_ClassConfig(klass)->skinName;
...
=============
*/
static void CG_StatusMessages(clientInfo_t *_new, clientInfo_t *old)
{
    if (!old->infoValid)
        return;

    if (strcmp(_new->name, old->name))
        CG_Printf("%s" S_COLOR_WHITE " renamed to %s\n", old->name, _new->name);

    if (old->team != _new->team)
    {
        if (_new->team == TEAM_NONE)
            CG_Printf("%s" S_COLOR_WHITE " left the %ss\n", _new->name, BG_TeamName(old->team));
        else if (old->team == TEAM_NONE)
            CG_Printf("%s" S_COLOR_WHITE " joined the %ss\n", _new->name, BG_TeamName(_new->team));
        else
            CG_Printf("%s" S_COLOR_WHITE " left the %ss and joined the %ss\n", _new->name, BG_TeamName(old->team),
                BG_TeamName(_new->team));
    }
}
...
void CG_NewClientInfo(int clientNum)
{
    ...
    if (clientNum == cg.clientNum && ci->infoValid && ci->team != newInfo.team)
    {
        ...
        trap_Cvar_VariableStringBuffer(va("cg_%sConfig", BG_TeamName(newInfo.team)), config, sizeof(config));
...
===============
*/
static void CG_PlayerAnimation(
    centity_t *cent, int *legsOld, int *legs, float *legsBackLerp, int *torsoOld, int *torso, float *torsoBackLerp)
{
    clientInfo_t *ci;
    int clientNum;
...
==================
*/
static void CG_SwingAngles(
    float destination, float swingTolerance, float clampTolerance, float speed, float *angle, bool *swinging)
{
    float swing;
    float move;
    float scale;
...
static void CG_PlayerWWSmoothing(centity_t *cent, vec3_t in[3], vec3_t out[3])
{
    ...
    if (!VectorCompare(surfNormal, cent->pe.lastNormal))
    {
        ...
        else
        {
            ...
            rotAngle = DotProduct(inAxis[0], lastAxis[0]) + DotProduct(inAxis[1], lastAxis[1]) +
    ...
    for (i = MAXSMOOTHS - 1; i >= 0; i--)
    {
        ...
        if (cg.time < cent->pe.sList[i].time + MODEL_WWSMOOTHTIME)
        {
            ...
            RotatePointAroundVector(
                outAxis[0], cent->pe.sList[i].rotAxis, inAxis[0], sFraction * cent->pe.sList[i].rotAngle);
            RotatePointAroundVector(
                outAxis[1], cent->pe.sList[i].rotAxis, inAxis[1], sFraction * cent->pe.sList[i].rotAngle);
            RotatePointAroundVector(
                outAxis[2], cent->pe.sList[i].rotAxis, inAxis[2], sFraction * cent->pe.sList[i].rotAngle);
...
static void CG_PlayerNonSegAngles(centity_t *cent, vec3_t srcAngles, vec3_t nonSegAxis[3])
{
    ...
    {
        CG_SwingAngles(
            localAngles[YAW], 40, 90, cg_swingSpeed.value, &cent->pe.nonseg.yawAngle, &cent->pe.nonseg.yawing);
    }
        ...
        if (active & (1 << UP_JETPACK))
        {
            if (es->pos.trDelta[2] > 10.0f)
            {
                ...
                trap_S_AddLoopingSound(
                    cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.jetpackAscendSound);
            }
            else if (es->pos.trDelta[2] < -10.0f)
            {
                ...
                trap_S_AddLoopingSound(
                    cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.jetpackDescendSound);
            }
            else
            {
                ...
                trap_S_AddLoopingSound(
                    cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.jetpackIdleSound);
            }
...
static bool CG_PlayerShadow(centity_t *cent, float *shadowPlane, class_t klass)
{
    ...
    BG_ClassBoundingBox(klass, mins, maxs, NULL, NULL, NULL);
    mins[2] = 0.0f;
    maxs[2] = 2.0f;
    ...
    *shadowPlane = 0;
    ...
    CG_ImpactMark(cgs.media.shadowMarkShader, trace.endpos, trace.plane.normal, cent->pe.legs.yawAngle, 0.0f, 0.0f,
        0.0f, alpha, false, 24.0f * BG_ClassConfig(klass)->shadowScale, true);
...
===============
*/
static void CG_PlayerSplash(centity_t *cent, class_t klass)
{
    vec3_t start, end;
    vec3_t mins, maxs;
    trace_t trace;
    ...
    CG_ImpactMark(cgs.media.wakeMarkShader, trace.endpos, trace.plane.normal, cent->pe.legs.yawAngle, 1.0f, 1.0f, 1.0f,
        1.0f, false, 32.0f * BG_ClassConfig(klass)->shadowScale, true);
}
...
void CG_Player(centity_t *cent)
{
    ...
    if (!ci->nonsegmented)
        CG_PlayerAnimation(
            cent, &legs.oldframe, &legs.frame, &legs.backlerp, &torso.oldframe, &torso.frame, &torso.backlerp);
    else
    ...
    if ((es->number == cg.snap->ps.clientNum && cg.renderingThirdPerson) || es->number != cg.snap->ps.clientNum)
        shadow = CG_PlayerShadow(cent, &shadowPlane, klass);

    // add a water splash if partially in and out of water
    CG_PlayerSplash(cent, klass);
    ...
    // rescale the model
    scale = BG_ClassConfig(klass)->modelScale;
    ...
    VectorMA(legs.origin, BG_ClassConfig(klass)->zOffset, surfNormal, legs.origin);
    VectorCopy(legs.origin, legs.lightingOrigin);
    VectorCopy(legs.origin, legs.oldorigin);  // don't positionally lerp at all
    ...
    if (!ci->nonsegmented)
    {
        ...
        if ((es->eFlags & EF_POISONCLOUDED) && (es->number != cg.snap->ps.clientNum || cg.renderingThirdPerson))
        {
            ...
            CG_SetAttachmentTag(&cent->poisonCloudedPS->attachment, head, head.hModel, "tag_head");
