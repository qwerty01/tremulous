...
void trap_Cvar_Set(const char *var_name, const char *value) { syscall(CG_CVAR_SET, var_name, value); }
...
void trap_FS_Read(void *buffer, int len, fileHandle_t f) { syscall(CG_FS_READ, buffer, len, f); }

void trap_FS_Write(const void *buffer, int len, fileHandle_t f) { syscall(CG_FS_WRITE, buffer, len, f); }

void trap_FS_FCloseFile(fileHandle_t f) { syscall(CG_FS_FCLOSEFILE, f); }

void trap_FS_Seek(fileHandle_t f, long offset, enum FS_Origin origin) { syscall(CG_FS_SEEK, f, offset, origin); }
...
void trap_SendConsoleCommand(const char *text) { syscall(CG_SENDCONSOLECOMMAND, text); }
...
void trap_CM_TransformedBiSphereTrace(trace_t *results, const vec3_t start, const vec3_t end, float startRad,
    float endRad, clipHandle_t model, int mask, const vec3_t origin)
{
    syscall(CG_CM_TRANSFORMEDBISPHERETRACE, results, start, end, PASSFLOAT(startRad), PASSFLOAT(endRad), model, mask,
        origin);
}
...
int trap_S_SoundDuration(sfxHandle_t handle) { return syscall(CG_S_SOUNDDURATION, handle); }
#endif
...
qhandle_t trap_R_RegisterModel(const char *name) { return syscall(CG_R_REGISTERMODEL, name); }

qhandle_t trap_R_RegisterSkin(const char *name) { return syscall(CG_R_REGISTERSKIN, name); }

qhandle_t trap_R_RegisterShader(const char *name) { return syscall(CG_R_REGISTERSHADER, name); }

qhandle_t trap_R_RegisterShaderNoMip(const char *name) { return syscall(CG_R_REGISTERSHADERNOMIP, name); }
...
void trap_R_AddRefEntityToScene(const refEntity_t *re) { syscall(CG_R_ADDREFENTITYTOSCENE, re); }
...
#ifndef MODULE_INTERFACE_11
void trap_R_SetClipRegion(const float *region) { syscall(CG_R_SETCLIPREGION, region); }
#endif
...
void trap_GetGlconfig(glconfig_t *glconfig) { syscall(CG_GETGLCONFIG, glconfig); }
...
int trap_Parse_AddGlobalDefine(char *define) { return syscall(CG_PARSE_ADD_GLOBAL_DEFINE, define); }

int trap_Parse_LoadSource(const char *filename) { return syscall(CG_PARSE_LOAD_SOURCE, filename); }

int trap_Parse_FreeSource(int handle) { return syscall(CG_PARSE_FREE_SOURCE, handle); }

int trap_Parse_ReadToken(int handle, pc_token_t *pc_token) { return syscall(CG_PARSE_READ_TOKEN, handle, pc_token); }
...
// this returns a handle.  arg0 is the name in the format "idlogo.roq", set arg1 to NULL, alteredstates to false (do not
// alter gamestate)
...
e_status trap_CIN_StopCinematic(int handle) { return static_cast<e_status>(syscall(CG_CIN_STOPCINEMATIC, handle)); }

// will run a frame of the cinematic but will not draw it.  Will return FMV_EOF if the end of the cinematic has been
// reached.
e_status trap_CIN_RunCinematic(int handle) { return static_cast<e_status>(syscall(CG_CIN_RUNCINEMATIC, handle)); }
...
int trap_GetDemoState(void) { return syscall(CG_GETDEMOSTATE); }

int trap_GetDemoPos(void) { return syscall(CG_GETDEMOPOS); }
...
void trap_Key_GetBindingBuf(int keynum, char *buf, int buflen) { syscall(CG_KEY_GETBINDINGBUF, keynum, buf, buflen); }

void trap_Key_SetBinding(int keynum, const char *binding) { syscall(CG_KEY_SETBINDING, keynum, binding); }

#ifndef MODULE_INTERFACE_11
void trap_Key_SetOverstrikeMode(bool state) { syscall(CG_KEY_SETOVERSTRIKEMODE, state); }

bool trap_Key_GetOverstrikeMode(void) { return syscall(CG_KEY_GETOVERSTRIKEMODE); }

void trap_Field_CompleteList(const char *listJson) { syscall(CG_FIELD_COMPLETELIST, listJson); }
#endif
