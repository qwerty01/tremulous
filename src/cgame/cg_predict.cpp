...
================
*/
void CG_BiSphereTrace(trace_t *result, const vec3_t start, const vec3_t end, const float startRadius,
    const float endRadius, int skipNumber, int mask)
{
    trace_t t;
    vec3_t mins, maxs;
...
static void CG_TouchTriggerPrediction(void)
{
    ...
    for (i = 0; i < cg_numTriggerEntities; i++)
    {
        ...
        trap_CM_BoxTrace(&trace, cg.predictedPlayerState.origin, cg.predictedPlayerState.origin, cg_pmove.mins,
...
static int CG_IsUnacceptableError(playerState_t *ps, playerState_t *pps)
{
    ...
    if (pps->damageEvent != ps->damageEvent || pps->damageYaw != ps->damageYaw || pps->damagePitch != ps->damagePitch ||
...
void CG_PredictPlayerState(void)
{
    ...
    if (cg_optimizePrediction.integer)
    {
        ...
        else
        {
            ...
            for (i = cg.stateHead; i != cg.stateTail; i = (i + 1) % NUM_SAVED_STATES)
            {
                ...
                errorcode = CG_IsUnacceptableError(&cg.predictedPlayerState, &cg.savedPmoveStates[i]);
    ...
    for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++)
    {
        ...
        if (!cg_optimizePrediction.integer)
        {
            Pmove(&cg_pmove);
                 ...
                 (cmdNum >= predictCmd || (stateIndex + 1) % NUM_SAVED_STATES == cg.stateHead))
        {
            Pmove(&cg_pmove);
    ...
    CG_AdjustPositionForMover(cg.predictedPlayerState.origin, cg.predictedPlayerState.groundEntityNum, cg.physicsTime,
        cg.time, cg.predictedPlayerState.origin);
