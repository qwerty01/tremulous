...
static void CG_Obituary( entityState_t* ent )
{
    ...
    if( attacker != ENTITYNUM_WORLD )
    {
        ...
        {
            CG_CenterPrint( va( "You killed %s%s",
                                teamKill ? S_COLOR_RED "TEAMMATE " S_COLOR_WHITE : "",
...
void CG_EntityEvent( centity_t* cent, vec3_t position )
{
    ...
    if( cg.snap->ps.persistant[ PERS_SPECSTATE ] != SPECTATOR_NOT )
        ...
        steptime = BG_Class( ( class_t )cg.snap->ps.stats[ STAT_CLASS ] )->steptime;
    ...
    switch( event )
    {
        ...
        case EV_FALL_SHORT:
            ...
            if( clientNum == cg.predictedPlayerState.clientNum )
            {
                ...
                cg.landChange = -1 * BG_Class( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ] )->landBob;
                cg.landTime = cg.time;
            }
            break;
        ...
        case EV_FALL_MEDIUM:
            ...
            {
                // smooth landing z changes
                cg.landChange = -2 * BG_Class( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ] )->landBob;
                cg.landTime = cg.time;
            }
            break;
        ...
        case EV_FALL_FAR:
            ...
            {
                // smooth landing z changes
                cg.landChange = -3 * BG_Class( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ] )->landBob;
                cg.landTime = cg.time;
            }
            break;
        ...
        case EV_JUMP:
            ...
            if( BG_ClassHasAbility( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ], SCA_WALLJUMPER ) )
            {
        ...
        case EV_MISSILE_HIT:
            ...
            CG_MissileHitEntity( ( weapon_t )es->weapon, ( weaponMode_t )es->generic1, position, dir, es->otherEntityNum, es->torsoAnim );
            break;
            
        case EV_MISSILE_MISS:
            ByteToDir( es->eventParm, dir );
            CG_MissileHitWall( ( weapon_t )es->weapon, ( weaponMode_t )es->generic1, 0, position, dir, IMPACTSOUND_DEFAULT, es->torsoAnim );
            break;
            
        case EV_MISSILE_MISS_METAL:
            ByteToDir( es->eventParm, dir );
            CG_MissileHitWall( ( weapon_t )es->weapon, ( weaponMode_t )es->generic1, 0, position, dir, IMPACTSOUND_METAL, es->torsoAnim );
            break;
