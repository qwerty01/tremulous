    ...
    if (attacker != ENTITYNUM_WORLD)
    {
        ...
        if (cg_killMsg.integer == 1)
        {
            ...
            if (icon > WP_NONE)
            {
                Com_sprintf(killMessage, sizeof(killMessage), "%s%s",
                    teamKill ? S_COLOR_RED "TEAMMATE " S_COLOR_WHITE : "", targetName);
        ...
        else if (message)
        {
            CG_Printf("%s" S_COLOR_WHITE " %s %s%s" S_COLOR_WHITE "%s\n", targetName, message,
                teamKill ? S_COLOR_RED "TEAMMATE " S_COLOR_WHITE : "", attackerName, message2);
        }

        if (attacker == cg.clientNum)
        {
            CG_CenterPrint(va("You killed %s%s", teamKill ? S_COLOR_RED "TEAMMATE " S_COLOR_WHITE : "", targetName),
...
void CG_EntityEvent(centity_t *cent, vec3_t position)
{
    ...
    switch (event)
    {
        ...
        case EV_DEATH3:
            trap_S_StartSound(
                NULL, es->number, CHAN_VOICE, CG_CustomSound(es->number, va("*death%i.wav", event - EV_DEATH1 + 1)));
            break;
...
void CG_CheckEvents(centity_t *cent)
{
    ...
    {
        event = static_cast<entity_event_t>(cent->currentState.eType - ET_EVENTS);
        ...
        if (cent->currentState.eFlags & EF_PLAYER_EVENT)
        {
            ...
            oldEvent = static_cast<entity_event_t>(cent->currentState.event);
