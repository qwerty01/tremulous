...
static void CG_DrawPlayerCreditsValue( rectDef_t* rect, vec4_t color, bool padding )
{
    int           value;
    playerState_t* ps;
    centity_t*     cent;
    ...
    if( value > -1 )
    {
        ...
        {
            if( !BG_AlienCanEvolve( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ],
                                    ...
                                    vec4_t foreColor, qhandle_t shader )
{
    ...
    switch( weapon )
    {
        ...
        default:
            ...
            {
                maxDelay = ( float )BG_Weapon( ( weapon_t )cent->currentState.weapon )->reloadTime;
...
static void CG_DrawPlayerPoisonBarbs( rectDef_t* rect, vec4_t color, qhandle_t shader )
{
    ...
    maxBarbs = BG_Weapon( ( weapon_t )cg.snap->ps.weapon )->maxAmmo;
...
{
    if( BG_ClassHasAbility( ( class_t )cg.snap->ps.stats[ STAT_CLASS ], SCA_ALIENSENSE ) )
        CG_AlienSense( rect );
}
...
static void CG_DrawUsableBuildable( rectDef_t* rect, qhandle_t shader, vec4_t color )
{
    ...
    if( es->eType == ET_BUILDABLE && BG_Buildable( ( buildable_t )es->modelindex )->usable &&
            cg.predictedPlayerState.stats[ STAT_TEAM ] == BG_Buildable( ( buildable_t )es->modelindex )->team )
    {
        ...
        if( ( es->modelindex == BA_H_REACTOR || es->modelindex == BA_H_REPEATER ) &&
                ( !BG_Weapon( ( weapon_t )cg.snap->ps.weapon )->usesEnergy ||
                  BG_Weapon( ( weapon_t )cg.snap->ps.weapon )->infiniteAmmo ) )
        {
    ...
    for( i = 0; i < MAX_CLIENTS && displayCount < maxDisplayCount; i++ )
    {
        ...
        else
        {
            if( ci->team == TEAM_HUMANS )
                ...
                curWeapon = BG_Class( ( class_t )ci->curWeaponClass )->startWeapon;
...
void CG_DrawWeaponIcon( rectDef_t* rect, vec4_t color )
{
    ...
    if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_ALIENS &&
            !BG_AlienCanEvolve( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ],
...
static void CG_ScanForCrosshairEntity( void )
{
    ...
    if( trace.entityNum >= MAX_CLIENTS )
    {
        ...
        if( s->eType == ET_BUILDABLE && BG_Buildable( ( buildable_t )s->modelindex )->team ==
...
static void CG_Draw2D( void )
{
                 ...
                 && cg.snap->ps.stats[ STAT_HEALTH ] > 0 ) )
    {
        menu = Menus_FindByName( BG_ClassConfig(
                                     ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ] )->hudName );
    ...
    CG_DrawVote( ( team_t )cg.predictedPlayerState.stats[ STAT_TEAM ] );
    CG_DrawWarmup( );
    CG_DrawQueue( );
