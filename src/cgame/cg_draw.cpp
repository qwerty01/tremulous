...
static void CG_AlignText(
    Rectangle *rect, const char *text, float scale, float w, float h, int align, int valign, float *x, float *y)
{
    float tx, ty;
...
static void CG_DrawProgressBar(Rectangle *rect, vec4_t color, float scale, int align, int textalign, int textStyle,
    float borderSize, float progress)
{
    ...
    else
    {
        ...
        CG_DrawPic(
            rimWidth + rect->x + doneWidth, rect->y + rect->h - rimWidth, leftWidth, rimWidth, cgs.media.whiteShader);
...
static void CG_DrawPlayerCreditsValue(Rectangle *rect, vec4_t color, bool padding)
{
    int value;
    playerState_t *ps;
    centity_t *cent;
    ...
    if (value > -1)
    {
        ...
        {
            if (!BG_AlienCanEvolve(cg.predictedPlayerState.stats[STAT_CLASS], value, cgs.alienStage) &&
...
static void CG_DrawPlayerCreditsFraction(Rectangle *rect, vec4_t color, qhandle_t shader)
{
    float fraction;
    float height;
    ...
    fraction =
        ((float)(cg.predictedPlayerState.persistant[PERS_CREDIT] % ALIEN_CREDITS_PER_KILL)) / ALIEN_CREDITS_PER_KILL;
    ...
    trap_R_DrawStretchPic(
        rect->x, rect->y - height + rect->h, rect->w, height, 0.0f, 1.0f - fraction, 1.0f, 1.0f, shader);
    trap_R_SetColor(NULL);
}
...
==============
*/
static void CG_DrawPlayerStamina(int ownerDraw, Rectangle *rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader)
{
...
==============
*/
static void CG_DrawPlayerStaminaBolt(Rectangle *rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader)
{
    ...
    {
        if (stamina >= 0)
            Vector4Lerp((sin(cg.time / 150.0f) + 1) / 2, backColor, foreColor, color);
        else
            Vector4Lerp((sin(cg.time / 2000.0f) + 1) / 2, backColor, foreColor, color);
    }
    else
    {
        if (stamina < 0)
...
==============
*/
static void CG_DrawPlayerClipsRing(Rectangle *rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader)
{
    playerState_t *ps = &cg.snap->ps;
    centity_t *cent;
...
==============
*/
static void CG_DrawPlayerBuildTimerRing(Rectangle *rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader)
{
    playerState_t *ps = &cg.snap->ps;
    float buildTime = ps->stats[STAT_MISC];
    float progress;
...
==============
*/
static void CG_DrawPlayerBoosted(Rectangle *rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader)
{
...
==============
*/
static void CG_DrawPlayerBoosterBolt(Rectangle *rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader)
{
    vec4_t color;
    ...
    if ((cg.snap->ps.stats[STAT_STATE] & SS_BOOSTED) && (cg.snap->ps.stats[STAT_STATE] & SS_BOOSTEDWARNING))
        Vector4Lerp((sin(cg.time / 100.0f) + 1) / 2, backColor, foreColor, color);
    else
        Vector4Copy(foreColor, color);
...
==============
*/
static void CG_DrawPlayerPoisonBarbs(Rectangle *rect, vec4_t color, qhandle_t shader)
{
    bool vertical;
...
==============
*/
static void CG_DrawPlayerWallclimbing(Rectangle *rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader)
{
...
static void CG_DrawPlayerAmmoValue(Rectangle *rect, vec4_t color)
{
    int value;
    int valueMarked = -1;
    bool bp = false;
...
==============
*/
static void CG_DrawAlienSense(Rectangle *rect)
{
...
==============
*/
static void CG_DrawHumanScanner(Rectangle *rect, qhandle_t shader, vec4_t color)
{
...
==============
*/
static void CG_DrawUsableBuildable(Rectangle *rect, qhandle_t shader, vec4_t color)
{
    vec3_t view, point;
    trace_t trace;
    entityState_t *es;
        ...
        cg.predictedPlayerState.stats[STAT_TEAM] == BG_Buildable(es->modelindex)->team)
    {
        ...
        if ((es->modelindex == BA_H_REACTOR || es->modelindex == BA_H_REPEATER) &&
            (!BG_Weapon(cg.snap->ps.weapon)->usesEnergy || BG_Weapon(cg.snap->ps.weapon)->infiniteAmmo))
        {
...
static void CG_DrawPlayerBuildTimer(Rectangle *rect, vec4_t color)
{
    int index;
    playerState_t *ps;
...
static void CG_DrawPlayerClipsValue(Rectangle *rect, vec4_t color)
{
    int value;
    playerState_t *ps = &cg.snap->ps;
...
static void CG_DrawPlayerHealthValue(Rectangle *rect, vec4_t color)
{
    trap_R_SetColor(color);
...
==============
*/
static void CG_DrawPlayerHealthCross(Rectangle *rect, vec4_t ref_color)
{
    qhandle_t shader;
    vec4_t color;
    float ref_alpha;
...
static float CG_ChargeProgress(void)
{
    ...
    progress = ((float)cg.predictedPlayerState.stats[STAT_MISC] - min) / (max - min);
...
static void CG_DrawPlayerChargeBarBG(Rectangle *rect, vec4_t ref_color, qhandle_t shader)
{
    vec4_t color;
...
static void CG_DrawPlayerChargeBar(Rectangle *rect, vec4_t ref_color, qhandle_t shader)
{
    vec4_t color;
    ...
    if (cg.snap->ps.weapon == WP_LUCIFER_CANNON && cg.snap->ps.stats[STAT_MISC] >= LCANNON_CHARGE_TIME_WARN &&
        (cg.time & 128))
    {
        color[0] = 1.0f;
        color[1] = 0.0f;
...
static void CG_DrawProgressLabel(Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign,
    int textvalign, const char *s, float fraction)
{
...
static void CG_DrawMediaProgress(
    Rectangle *rect, vec4_t color, float scale, int align, int textalign, int textStyle, float borderSize)
{
    CG_DrawProgressBar(rect, color, scale, align, textalign, textStyle, borderSize, cg.mediaFraction);
}

static void CG_DrawMediaProgressLabel(
    Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign, int textvalign)
{
    CG_DrawProgressLabel(
        rect, text_x, text_y, color, scale, textalign, textvalign, "Map and Textures", cg.mediaFraction);
}

static void CG_DrawBuildablesProgress(
    Rectangle *rect, vec4_t color, float scale, int align, int textalign, int textStyle, float borderSize)
{
    CG_DrawProgressBar(rect, color, scale, align, textalign, textStyle, borderSize, cg.buildablesFraction);
}

static void CG_DrawBuildablesProgressLabel(
    Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign, int textvalign)
{
    CG_DrawProgressLabel(
        rect, text_x, text_y, color, scale, textalign, textvalign, "Buildable Models", cg.buildablesFraction);
}

static void CG_DrawCharModelProgress(
    Rectangle *rect, vec4_t color, float scale, int align, int textalign, int textStyle, float borderSize)
{
    CG_DrawProgressBar(rect, color, scale, align, textalign, textStyle, borderSize, cg.charModelFraction);
}

static void CG_DrawCharModelProgressLabel(
    Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign, int textvalign)
{
    CG_DrawProgressLabel(
...
static void CG_DrawOverallProgress(
    Rectangle *rect, vec4_t color, float scale, int align, int textalign, int textStyle, float borderSize)
{
    float total;
    ...
    CG_DrawProgressBar(rect, color, scale, align, textalign, textStyle, borderSize, total);
}

static void CG_DrawLevelShot(Rectangle *rect)
{
    const char *s;
    const char *info;
    qhandle_t levelshot;
...
static void CG_DrawLevelName(Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign,
    int textvalign, int textStyle)
{
    const char *s;
...
static void CG_DrawMOTD(Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign,
    int textvalign, int textStyle)
{
    const char *s;
...
static void CG_DrawHostname(Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign,
    int textvalign, int textStyle)
{
    char buffer[1024];
    const char *info;
...
==============
*/
static void CG_DrawDemoPlayback(Rectangle *rect, vec4_t color, qhandle_t shader)
{
...
==============
*/
static void CG_DrawDemoRecording(Rectangle *rect, vec4_t color, qhandle_t shader)
{
    if (!cg_drawDemoState.integer)
        return;
...
static void CG_DrawKiller(Rectangle *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
    // fragged by ... line
    if (cg.killerName[0])
    {
        int x = rect->x + rect->w / 2;
        UI_Text_Paint(x - UI_Text_Width(CG_GetKillerText(), scale) / 2, rect->y + rect->h, scale, color,
...
==================
*/
static void CG_DrawTeamSpectators(Rectangle *rect, float scale, int textvalign, vec4_t color, qhandle_t shader)
{
    float y;
...
==================
*/
static void CG_DrawFollow(Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign,
    int textvalign, int textStyle)
{
    float tx, ty;
    ...
    if (cg.snap && cg.snap->ps.pm_flags & PMF_FOLLOW)
    {
        ...
        UI_Text_Paint(text_x + tx, text_y + ty, scale, color, buffer, 0, 0, textStyle);
    }
}
...
==================
*/
static void CG_DrawTeamLabel(Rectangle *rect, team_t team, float text_x, float text_y, vec4_t color, float scale,
...
==================
*/
static void CG_DrawStageReport(Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign,
    int textvalign, int textStyle)
{
    ...
    if (cg.snap->ps.stats[STAT_TEAM] == TEAM_ALIENS)
    {
        ...
        if (cgs.alienNextStageThreshold < 0)
            ...
            Com_sprintf(s, MAX_TOKEN_CHARS, "Stage %d, 1 frag for next stage", cgs.alienStage + 1);
        else
            Com_sprintf(s, MAX_TOKEN_CHARS, "Stage %d, %d frags for next stage", cgs.alienStage + 1, kills);
    }
    else if (cg.snap->ps.stats[STAT_TEAM] == TEAM_HUMANS)
    {
        ...
        if (cgs.humanNextStageThreshold < 0)
            ...
            Com_sprintf(s, MAX_TOKEN_CHARS, "Stage %d, 1 credit for next stage", cgs.humanStage + 1);
        else
            Com_sprintf(s, MAX_TOKEN_CHARS, "Stage %d, %d credits for next stage", cgs.humanStage + 1, credits);
    }
...
static void CG_DrawFPS(Rectangle *rect, float text_x, float text_y, float scale, vec4_t color, int textalign,
    int textvalign, int textStyle, bool scalableText)
{
    const char *s;
    float tx, ty;
...
=================
*/
static void CG_DrawTimerMins(Rectangle *rect, vec4_t color)
{
    int mins, seconds;
    int msec;
...
=================
*/
static void CG_DrawTimerSecs(Rectangle *rect, vec4_t color)
{
    int mins, seconds;
    int msec;
...
=================
*/
static void CG_DrawTimer(Rectangle *rect, float text_x, float text_y, float scale, vec4_t color, int textalign,
    int textvalign, int textStyle)
{
    const char *s;
    float tx, ty;
...
static int QDECL SortWeaponClass(const void *a, const void *b)
{
    ...
    {
        if (ca->curWeaponClass == PCL_ALIEN_BUILDER0_UPG || cb->curWeaponClass == PCL_ALIEN_BUILDER0_UPG ||
            ca->curWeaponClass == PCL_ALIEN_BUILDER0 || cb->curWeaponClass == PCL_ALIEN_BUILDER0)
        {
            out = -out;
        }
    }
...
static void CG_DrawTeamOverlay(Rectangle *rect, float scale, vec4_t color)
{
    const char *s;
    int i;
    float x = rect->x;
    ...
    teamOverlayMode_t mode = static_cast<teamOverlayMode_t>(cg_drawTeamOverlay.integer);
    teamOverlaySort_t sort = static_cast<teamOverlaySort_t>(cg_teamOverlaySortMode.integer);
    ...
    if (mode == TEAMOVERLAY_ALL || mode == TEAMOVERLAY_SUPPORT)
    {
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            ...
            if (ci->infoValid && pci != ci && ci->team == pci->team)
            {
                ...
                {
                    if (ci->curWeaponClass == PCL_ALIEN_BUILDER0 || ci->curWeaponClass == PCL_ALIEN_BUILDER0_UPG ||
                        ci->curWeaponClass == PCL_ALIEN_LEVEL1 || ci->curWeaponClass == PCL_ALIEN_LEVEL1_UPG ||
    ...
    else  // find nearby
    {
        for (i = 0; i < cg.snap->numEntities; i++)
        {
            ...
            if (cent->currentState.eType != ET_PLAYER || team != pci->team || cent->currentState.eFlags & EF_DEAD)
            {
                continue;
            }
    ...
    {
        qsort(displayClients, maxDisplayCount, sizeof(displayClients[0]), SortScore);
    }
    else if (sort == TEAMOVERLAY_SORT_WEAPONCLASS)
    {
        qsort(displayClients, maxDisplayCount, sizeof(displayClients[0]), SortWeaponClass);
    }
    ...
    for (i = 0; i < MAX_CLIENTS && displayCount < maxDisplayCount; i++)
    {
        ...
        trap_R_SetColor(color);
        CG_DrawPic(x, y, backgroundWidth, iconSize, cgs.media.teamOverlayShader);
        ...
        else
        {
            ...
            CG_DrawPic(x + leftMargin, y, iconSize, iconSize, cg_weapons[curWeapon].weaponIcon);
            if (cg.predictedPlayerState.stats[STAT_TEAM] == TEAM_HUMANS)
            {
                if (ci->upgrade != UP_NONE)
                {
                    CG_DrawPic(x + iconSize + leftMargin, y, iconSize, iconSize, cg_upgrades[ci->upgrade].upgradeIcon);
                }
            }
            else
            {
                if (curWeapon == WP_ABUILD2 || curWeapon == WP_ALEVEL1_UPG || curWeapon == WP_ALEVEL2_UPG ||
                    curWeapon == WP_ALEVEL3_UPG)
                {
                    CG_DrawPic(x + iconSize + leftMargin, y, iconSize, iconSize, cgs.media.upgradeClassIconShader);
                }
            }

            s = va(" [^%c%3d^7] ^7%s", CG_GetColorCharForHealth(displayClients[i]), ci->health,
                CG_ConfigString(CS_LOCATIONS + ci->location));
        }

        trap_R_SetColor(NULL);
        nameMaxX = nameMaxXCp = x + 2.0f * iconSize + leftMargin + midSep + nameWidth;
        UI_Text_Paint_Limit(&nameMaxXCp, x + 2.0f * iconSize + leftMargin + midSep, y + iconSize - iconTopMargin,
            fontScale, tcolor, name, 0, 0);

        maxXCp = maxX;

        UI_Text_Paint_Limit(&maxXCp, nameMaxX, y + iconSize - iconTopMargin, fontScale, tcolor, s, 0, 0);
        y += iconSize;
        displayCount++;
    }
}
...
=================
*/
static void CG_DrawClock(Rectangle *rect, float text_x, float text_y, float scale, vec4_t color, int textalign,
    int textvalign, int textStyle)
{
    const char *s;
    float tx, ty;
...
==================
*/
static void CG_DrawSnapshot(Rectangle *rect, float text_x, float text_y, float scale, vec4_t color, int textalign,
    int textvalign, int textStyle)
{
    const char *s;
    float tx, ty;
...
==================
*/
static void CG_DrawKillMsg(Rectangle *rect, float text_x, float text_y, float scale, vec4_t color, int textalign,
    int textvalign, int textStyle)
{
    int i;
    vec4_t hcolor;
    ...
    if (cgs.killMsgLastPos != cgs.killMsgPos)
    {
        ...
        for (i = cgs.killMsgPos - 1; i >= cgs.killMsgLastPos; i--)
        {
            ...
            UI_Text_Paint(rect->x + TINYCHAR_WIDTH, rect->y - (cgs.killMsgPos - i) * 20, scale, color,
                cgs.killMsgKillers[j], 0, 0, textStyle);
            x += w + 3;

            if (cg_weapons[cgs.killMsgWeapons[j]].weaponIcon != WP_NONE)
            {
                CG_DrawPic(rect->x + TINYCHAR_WIDTH + x, rect->y - (cgs.killMsgPos - i) * 20 - 15, 16, 16,
                    cg_weapons[cgs.killMsgWeapons[j]].weaponIcon);
                x += 16 + 2;

                w = UI_Text_Width(cgs.killMsgVictims[j], scale);
                UI_Text_Paint(rect->x + TINYCHAR_WIDTH + x, rect->y - (cgs.killMsgPos - i) * 20, scale, color,
                    cgs.killMsgVictims[j], 0, 0, textStyle);
            }
        }
    }
}
...
==============
*/
static void CG_DrawLagometer(Rectangle *rect, float text_x, float text_y, float scale, vec4_t textColor)
{
    int a, x, y, i;
    float v;
    ...
    if (cg_nopredict.integer || cg_synchronousClients.integer)
        ...
        ping = va("%d", cg.ping);
    ax = rect->x + (rect->w / 2.0f) - (UI_Text_Width(ping, scale) / 2.0f) + text_x;
    ay = rect->y + (rect->h / 2.0f) + (UI_Text_Height(ping, scale) / 2.0f) + text_y;

    Vector4Copy(textColor, adjustedColor);
    adjustedColor[3] = 0.5f;
    UI_Text_Paint(ax, ay, scale, adjustedColor, ping, 0, 0, ITEM_TEXTSTYLE_NORMAL);
...
===================
*/
static void CG_DrawSpeedGraph(Rectangle *rect, vec4_t foreColor, vec4_t backColor)
{
    int i;
    float val, max, top;
    ...
    {
        val = speedSamples[(oldestSpeedSample + i + SPEEDOMETER_NUM_SAMPLES - SPEEDOMETER_NUM_DISPLAYED_SAMPLES) %
                           SPEEDOMETER_NUM_SAMPLES];
        if (val < SPEED_MED)
            VectorLerp2(val / SPEED_MED, slow, medium, color);
        else if (val < SPEED_FAST)
            VectorLerp2((val - SPEED_MED) / (SPEED_FAST - SPEED_MED), medium, fast, color);
        else
            VectorCopy(fast, color);
        trap_R_SetColor(color);
        top = rect->y + (1 - val / max) * rect->h;
        CG_DrawPic(rect->x + (i / (float)SPEEDOMETER_NUM_DISPLAYED_SAMPLES) * rect->w, top,
            rect->w / (float)SPEEDOMETER_NUM_DISPLAYED_SAMPLES, val * rect->h / max, cgs.media.whiteShader);
    }
    trap_R_SetColor(NULL);
}
...
===================
*/
static void CG_DrawSpeedText(Rectangle *rect, float text_x, float text_y, float scale, vec4_t foreColor)
{
    char speedstr[16];
    float val;
    vec4_t color;
    ...
    UI_Text_Paint(rect->x + (rect->w - UI_Text_Width(speedstr, scale)) / 2.0f,
        rect->y + (rect->h + UI_Text_Height(speedstr, scale)) / 2.0f, scale, color, speedstr, 0, 0,
...
===================
*/
static void CG_DrawSpeed(Rectangle *rect, float text_x, float text_y, float scale, vec4_t foreColor, vec4_t backColor)
{
...
===================
*/
static void CG_DrawConsole(Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign,
    int textvalign, int textStyle)
{
...
===================
*/
static void CG_DrawTutorial(Rectangle *rect, float text_x, float text_y, vec4_t color, float scale, int textalign,
    int textvalign, int textStyle)
{
...
===================
*/
void CG_DrawWeaponIcon(Rectangle *rect, vec4_t color)
{
    int maxAmmo;
    playerState_t *ps;
    weapon_t weapon;
    ...
    if (!cg_weapons[weapon].registered)
    {
        Com_Printf(S_COLOR_YELLOW
            "WARNING: CG_DrawWeaponIcon: weapon %d (%s) "
            "is not registered\n",
    ...
    if (cg.predictedPlayerState.stats[STAT_TEAM] == TEAM_ALIENS &&
        !BG_AlienCanEvolve(cg.predictedPlayerState.stats[STAT_CLASS], ps->persistant[PERS_CREDIT], cgs.alienStage))
    {
    ...
    trap_R_SetColor(color);
    CG_DrawPic(rect->x, rect->y, rect->w, rect->h, cg_weapons[weapon].weaponIcon);
    trap_R_SetColor(NULL);
}
...
=================
*/
static void CG_DrawCrosshair(Rectangle *rect, vec4_t color)
{
    float w, h;
    qhandle_t hShader;
    float x, y;
...
static void CG_ScanForCrosshairEntity(void)
{
    ...
    if (trace.entityNum >= MAX_CLIENTS)
    {
        ...
        if (s->eType == ET_BUILDABLE && BG_Buildable(s->modelindex)->team == cg.snap->ps.stats[STAT_TEAM])
...
=====================
*/
static void CG_DrawLocation(Rectangle *rect, float scale, int textalign, vec4_t color)
{
    const char *location;
    centity_t *locent;
    float maxX;
...
=====================
*/
static void CG_DrawCrosshairNames(Rectangle *rect, float scale, int textStyle)
{
    float *color;
    const char *name;
    float w, x;
    ...
    if (cg_teamOverlayUserinfo.integer && cg.snap->ps.stats[STAT_TEAM] != TEAM_NONE && cgs.teaminfoReceievedTime &&
        cgs.clientinfo[cg.crosshairClientNum].health > 0)
    {
        name = va("%s ^7[^%c%d^7]", name, CG_GetColorCharForHealth(cg.crosshairClientNum),
...
void CG_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags,
    int align, int textalign, int textvalign, float borderSize, float scale, vec4_t foreColor, vec4_t backColor,
    qhandle_t shader, int textStyle)
{
    ...
    switch (ownerDraw)
    {
        ...
        case CG_PLAYER_LOCATION:
            ...
            break;
        case CG_FOLLOW:
            CG_DrawFollow(&rect, text_x, text_y, foreColor, scale, textalign, textvalign, textStyle);
            break;
        ...
        case CG_LOAD_LEVELSHOT:
            ...
            break;
        case CG_LOAD_MEDIA:
            CG_DrawMediaProgress(&rect, foreColor, scale, align, textalign, textStyle, borderSize);
            break;
        ...
        case CG_LOAD_BUILDABLES:
            CG_DrawBuildablesProgress(&rect, foreColor, scale, align, textalign, textStyle, borderSize);
            break;
        ...
        case CG_LOAD_CHARMODEL:
            CG_DrawCharModelProgress(&rect, foreColor, scale, align, textalign, textStyle, borderSize);
            break;
        ...
        case CG_LOAD_OVERALL:
            CG_DrawOverallProgress(&rect, foreColor, scale, align, textalign, textStyle, borderSize);
            break;
        ...
        case CG_FPS:
            CG_DrawFPS(&rect, text_x, text_y, scale, foreColor, textalign, textvalign, textStyle, true);
            break;
        case CG_FPS_FIXED:
            CG_DrawFPS(&rect, text_x, text_y, scale, foreColor, textalign, textvalign, textStyle, false);
            break;
        case CG_TIMER:
...
void CG_MouseEvent(int x, int y)
{
    ...
    if (cgs.capturedMenu)
        Display_MouseMove(cgs.capturedMenu, x, y);
    else
...
*/
void CG_ShowTeamMenu(void) { Menus_ActivateByName("teamMenu"); }
...
void CG_KeyEvent(int key, bool down)
{
    ...
    if (cg.predictedPlayerState.pm_type == PM_NORMAL ||
        (cg.predictedPlayerState.pm_type == PM_SPECTATOR && cg.showScores == false))
    {
    ...
    if (cgs.capturedMenu)
        cgs.capturedMenu = NULL;
    else
    {
        if (key == K_MOUSE2 && down)
            cgs.capturedMenu = Display_CaptureItem(cgs.cursorX, cgs.cursorY);
    }
}
...
static void CG_DrawLighting(void)
{
    ...
    if ((cg.snap->ps.stats[STAT_STAMINA] < STAMINA_BLACKOUT_LEVEL) && (cg.snap->ps.stats[STAT_TEAM] == TEAM_HUMANS))
    {
...
static void CG_DrawVote(team_t team)
{
    ...
    if (cg_tutorial.integer)
    {
        Com_sprintf(yeskey, sizeof(yeskey), "[%s]", CG_KeyBinding(va("%svote yes", team == TEAM_NONE ? "" : "team")));
        Com_sprintf(nokey, sizeof(nokey), "[%s]", CG_KeyBinding(va("%svote no", team == TEAM_NONE ? "" : "team")));
    }

    if (team != TEAM_NONE)
        offset = 80;

    s = va("%sVOTE(%i): %s", team == TEAM_NONE ? "" : "TEAM", sec, cgs.voteString[team]);

    UI_Text_Paint(8, 300 + offset, 0.3f, white, s, 0, 0, ITEM_TEXTSTYLE_NORMAL);

    s = va("  Called by: \"%s\"", cgs.voteCaller[team]);

    UI_Text_Paint(8, 320 + offset, 0.3f, white, s, 0, 0, ITEM_TEXTSTYLE_NORMAL);

    s = va("  %sYes:%i %sNo:%i", yeskey, cgs.voteYes[team], nokey, cgs.voteNo[team]);

    UI_Text_Paint(8, 340 + offset, 0.3f, white, s, 0, 0, ITEM_TEXTSTYLE_NORMAL);
}
...
static void CG_Draw2D(void)
{
    ...
    if (cg.snap->ps.stats[STAT_TEAM] == TEAM_NONE ||
        (cg.snap->ps.persistant[PERS_SPECSTATE] == SPECTATOR_NOT && cg.snap->ps.stats[STAT_HEALTH] > 0))
    {
...
{
    float ss, f, l, u;
    polyVert_t verts[4] = {{{0, 0, 0}, {0, 0}, {255, 255, 255, 255}}, {{0, 0, 0}, {1, 0}, {255, 255, 255, 255}},
        {{0, 0, 0}, {1, 1}, {255, 255, 255, 255}}, {{0, 0, 0}, {0, 1}, {255, 255, 255, 255}}};
    int i, j, k;
