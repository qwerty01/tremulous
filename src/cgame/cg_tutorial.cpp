...
static bind_t bindings[] = {{"+button2", "Activate Upgrade", {K_NONE, K_NONE}},
    {"+speed", "Run/Walk", {K_NONE, K_NONE}}, {"+button6", "Dodge", {K_NONE, K_NONE}},
    {"+button8", "Sprint", {K_NONE, K_NONE}}, {"+moveup", "Jump", {K_NONE, K_NONE}},
    {"+movedown", "Crouch", {K_NONE, K_NONE}}, {"+attack", "Primary Attack", {K_NONE, K_NONE}},
    {"+button5", "Secondary Attack", {K_NONE, K_NONE}}, {"reload", "Reload", {K_NONE, K_NONE}},
    {"buy ammo", "Buy Ammo", {K_NONE, K_NONE}}, {"itemact medkit", "Use Medkit", {K_NONE, K_NONE}},
    {"+button7", "Use Structure/Evolve", {K_NONE, K_NONE}}, {"deconstruct", "Deconstruct Structure", {K_NONE, K_NONE}},
    {"weapprev", "Previous Upgrade", {K_NONE, K_NONE}}, {"weapnext", "Next Upgrade", {K_NONE, K_NONE}}};
...
static void CG_GetBindings(void)
{
    ...
    for (i = 0; i < numBindings; i++)
    {
        ...
        for (j = 0; j < K_LAST_KEY; j++)
        {
            ...
            {
                bindings[i].keys[numKeys++] = static_cast<keyNum_t>(j);
            ...
            if (bindings[i].keys[0] != K_NONE)
            {
                ...
                if (bindings[i].keys[1] != K_NONE)
                {
                    ...
                    trap_Key_KeynumToStringBuf(
                        bindings[i].keys[1], buffer + strlen(buffer), MAX_STRING_CHARS - strlen(buffer));
...
static entityState_t *CG_BuildableInRange(playerState_t *ps, float *healthFraction)
{
    ...
    CG_Trace(&trace, cg.refdef.vieworg, NULL, NULL, point, ps->clientNum, MASK_SHOT);
    ...
    if (es->eType == ET_BUILDABLE && ps->stats[STAT_TEAM] == BG_Buildable(es->modelindex)->team)
        return es;
    else
        return NULL;
}
...
static void CG_AlienBuilderText(char *text, playerState_t *ps)
{
    ...
    if (buildable > BA_NONE)
    {
        Q_strcat(text, MAX_TUTORIAL_TEXT,
            va("Press %s to place the %s\n", CG_KeyNameForCommand("+attack"), BG_Buildable(buildable)->humanName));
...
{
    Q_strcat(text, MAX_TUTORIAL_TEXT, "Touch humans to damage them\n");
...
{
    Q_strcat(text, MAX_TUTORIAL_TEXT, "Touch humans to grab them\n");
...
static void CG_AlienLevel2Text(char *text, playerState_t *ps)
{
    ...
    {
        Q_strcat(
            text, MAX_TUTORIAL_TEXT, va("Press %s to invoke an electrical attack\n", CG_KeyNameForCommand("+button5")));
    }

    Q_strcat(
        text, MAX_TUTORIAL_TEXT, va("Hold down %s then touch a wall to wall jump\n", CG_KeyNameForCommand("+moveup")));
}
...
static void CG_HumanText(char *text, playerState_t *ps)
{
    ...
    else
    {
        switch (ps->weapon)
        {
            ...
            case WP_FLAMER:
                Q_strcat(text, MAX_TUTORIAL_TEXT,
                    va("Press %s to fire the %s\n", CG_KeyNameForCommand("+attack"), BG_Weapon(ps->weapon)->humanName));
                break;
    ...
    Q_strcat(text, MAX_TUTORIAL_TEXT, va("Press %s and ", CG_KeyNameForCommand("weapprev")));
    Q_strcat(text, MAX_TUTORIAL_TEXT, va("%s to select an upgrade\n", CG_KeyNameForCommand("weapnext")));

    if (upgrade == UP_NONE || (upgrade > UP_NONE && BG_Upgrade(upgrade)->usable))
    {
        Q_strcat(text, MAX_TUTORIAL_TEXT, va("Press %s to use the %s\n", CG_KeyNameForCommand("+button2"), name));
    }

    if (ps->stats[STAT_HEALTH] <= 35 && BG_InventoryContainsUpgrade(UP_MEDKIT, ps->stats))
    {
        Q_strcat(text, MAX_TUTORIAL_TEXT,
            va("Press %s to use your %s\n", CG_KeyNameForCommand("itemact medkit"), BG_Upgrade(UP_MEDKIT)->humanName));
    }

    if (ps->stats[STAT_STAMINA] <= STAMINA_BLACKOUT_LEVEL)
    {
        Q_strcat(text, MAX_TUTORIAL_TEXT, "You are blacking out. Stop sprinting to recover stamina\n");
    }
    else if (ps->stats[STAT_STAMINA] <= STAMINA_SLOW_LEVEL)
    {
        Q_strcat(text, MAX_TUTORIAL_TEXT, "Your stamina is low. Stop sprinting to recover\n");
    }
        ...
        case BA_H_ARMOURY:
            Q_strcat(text, MAX_TUTORIAL_TEXT,
                va("Press %s to buy equipment upgrades at the %s. Sell your old weapon first!\n",
                    CG_KeyNameForCommand("+button7"), BG_Buildable(cg.nearUsableBuildable)->humanName));
            break;
        case BA_H_REPEATER:
        case BA_H_REACTOR:
            Q_strcat(text, MAX_TUTORIAL_TEXT,
    ...
    Q_strcat(text, MAX_TUTORIAL_TEXT, va("Press %s and any direction to sprint\n", CG_KeyNameForCommand("+button8")));

    Q_strcat(text, MAX_TUTORIAL_TEXT, va("Press %s and back or strafe to dodge\n", CG_KeyNameForCommand("+button6")));
}
...
static void CG_SpectatorText(char *text, playerState_t *ps)
{
    ...
    if (ps->pm_flags & PMF_FOLLOW)
    {
        if (!cg.chaseFollow)
            ...
            Q_strcat(text, MAX_TUTORIAL_TEXT, va("Press %s to stop following\n", CG_KeyNameForCommand("+button2")));

        Q_strcat(text, MAX_TUTORIAL_TEXT, va("Press %s or ", CG_KeyNameForCommand("weapprev")));
...
const char *CG_TutorialText(void)
{
    ...
    if (!cg.intermissionStarted && !cg.demoPlayback)
    {
        ...
        else if (ps->stats[STAT_HEALTH] > 0)
        {
            ...
            {
                if (BG_AlienCanEvolve(ps->stats[STAT_CLASS], ps->persistant[PERS_CREDIT], cgs.alienStage))
                {
    ...
    else if (!cg.demoPlayback)
    {
        ...
        {
            Q_strcat(text, MAX_TUTORIAL_TEXT, va("Press %s when ready to continue\n", CG_KeyNameForCommand("+attack")));
        }
        else
        {
