...
static bind_t bindings[ ] =
{
    ...
    { "+button6",       "Dodge",                  { ( keyNum_t ) - 1, ( keyNum_t ) - 1 } },
    { "+button8",       "Sprint",                 { ( keyNum_t ) - 1, ( keyNum_t ) - 1 } },
...
static entityState_t* CG_BuildableInRange( playerState_t* ps, float* healthFraction )
{
    ...
    {
        health = es->misc;
        *healthFraction = ( float )health / BG_Buildable( ( buildable_t )es->modelindex )->health;
    }
    
    if( es->eType == ET_BUILDABLE &&
            ps->stats[ STAT_TEAM ] == BG_Buildable( ( buildable_t )es->modelindex )->team )
        return es;
    else
        return NULL;
}
...
static void CG_HumanText( char* text, playerState_t* ps )
{
    ...
    if( !ps->ammo && !ps->clips && !BG_Weapon( ( weapon_t )ps->weapon )->infiniteAmmo )
    {
        //no ammo
        switch( ps->weapon )
        {
            ...
            case WP_FLAMER:
                Q_strcat( text, MAX_TUTORIAL_TEXT,
                          va( "Press %s to fire the %s\n",
                              CG_KeyNameForCommand( "+attack" ),
                              BG_Weapon( ( weapon_t )ps->weapon )->humanName ) );
                break;
        ...
        case BA_H_ARMOURY:
            Q_strcat( text, MAX_TUTORIAL_TEXT,
                      va( "Press %s to buy equipment upgrades at the %s. Sell your old weapon first!\n",
                          ...
                          BG_Buildable( ( buildable_t )cg.nearUsableBuildable )->humanName ) );
            break;
        case BA_H_REPEATER:
        case BA_H_REACTOR:
            Q_strcat( text, MAX_TUTORIAL_TEXT,
...
const char* CG_TutorialText( void )
{
    ...
    if( !cg.intermissionStarted && !cg.demoPlayback )
    {
        ...
        else if( ps->stats[ STAT_HEALTH ] > 0 )
        {
            ...
            {
                if( BG_AlienCanEvolve( ( class_t )ps->stats[ STAT_CLASS ],
