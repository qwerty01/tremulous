...
static void CG_Creep( centity_t* cent )
{
    ...
    trace_t       tr;
    vec3_t        temp, origin;
    int           scaleUpTime = BG_Buildable( ( buildable_t )cent->currentState.modelindex )->buildTime;
    int           time;
...
void CG_InitBuildables( void )
{
    ...
    {
        buildableName = BG_Buildable( ( buildable_t )i )->name;
        ...
        for( j = 0; j <= 3; j++ )
        {
            modelFile = BG_BuildableConfig( ( buildable_t )i )->models[ j ];
        ...
        for( j = BANIM_NONE + 1; j < MAX_BUILDABLE_ANIMATIONS; j++ )
        {
            ...
            if( cg_buildables[ i ].sounds[ j ].enabled )
            {
                ...
                else
                {
                    ...
                    if( BG_Buildable( ( buildable_t )i )->team == TEAM_ALIENS )
        ...
        if( ( cent->oldBuildableAnim ^ es->legsAnim ) & ANIM_TOGGLEBIT )
        {
            if( cg_debugAnim.integer )
                CG_Printf( "%d->%d l:%d t:%d %s(%d)\n",
                           ...
                           BG_Buildable( ( buildable_t )es->modelindex )->humanName, es->number );
...
{
    entityState_t*   es = &cent->currentState;
    team_t          team = BG_Buildable( ( buildable_t )es->modelindex )->team;
    int             health = es->misc;
    float           healthFrac = ( float )health / BG_Buildable( ( buildable_t )es->modelindex )->health;
...
static void CG_BuildableStatusDisplay( centity_t* cent )
{
    ...
    if( BG_Buildable( ( buildable_t )es->modelindex )->team == TEAM_ALIENS )
    ...
    BG_BuildableBoundingBox( ( buildable_t )es->modelindex, mins, maxs );
        ...
        for( i = 0; i < 3; i++ )
        {
            ...
            if( tr.entityNum < MAX_CLIENTS || ( hit->eType == ET_BUILDABLE &&
                                                ( !( es->eFlags & EF_B_SPAWNED ) ||
                                                  BG_Buildable( ( buildable_t )hit->modelindex )->transparentTest ) ) )
            {
                entNum = tr.entityNum;
    ...
    health = es->misc;
    healthScale = ( float )health / BG_Buildable( ( buildable_t )es->modelindex )->health;
    ...
    if( CG_WorldToScreen( origin, &x, &y ) )
    {
        ...
        
        {
            ...
            healthMax = BG_Buildable( ( buildable_t )es->modelindex )->health;
    ...
    switch( cg.predictedPlayerState.weapon )
    {
        ...
        case WP_HBUILD:
            return BG_Buildable( buildable )->team ==
                   BG_Weapon( ( weapon_t )cg.predictedPlayerState.weapon )->team;
...
void CG_Buildable( centity_t* cent )
{
    ...
    //offset on the Z axis if required
    VectorMA( ent.origin, BG_BuildableConfig( ( buildable_t )es->modelindex )->zOffset, surfNormal, ent.origin );
    ...
    //rescale the model
    scale = BG_BuildableConfig( ( buildable_t )es->modelindex )->modelScale;
    ...
    if( es->eFlags & EF_FIRING )
    {
        ...
        if( cg.time - cent->muzzleFlashTime > MUZZLE_FLASH_TIME ||
                BG_Buildable( ( buildable_t )es->modelindex )->turretProjType == WP_TESLAGEN )
        {
    ...
    if( cg_rangeMarkerForBlueprint.integer )
    {
        ...
        if( es->eFlags & EF_B_POWERED )
            CG_GhostBuildableRangeMarker( ( buildable_t )es->modelindex, ent.origin, surfNormal );
    }
}
