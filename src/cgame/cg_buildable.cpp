...
static void CG_Creep(centity_t *cent)
{
    ...
    if (size > 0.0f && tr.fraction < 1.0f)
        CG_ImpactMark(
            cgs.media.creepShader, origin, cent->currentState.origin2, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, false, size, true);
}
...
static bool CG_ParseBuildableSoundFile(const char *filename, buildable_t buildable)
{
    ...
    for (i = BANIM_NONE + 1; i < MAX_BUILDABLE_ANIMATIONS; i++)
    {
        ...
        sounds[i].enabled = static_cast<bool>(atoi(token));

        token = COM_Parse(&text_p);
        if (!*token)
            break;

        sounds[i].looped = static_cast<bool>(atoi(token));
    }
    ...
    if (newAnimation != lf->animationNumber || !lf->animation)
    {
        if (cg_debugRandom.integer)
            CG_Printf("newAnimation: %d lf->animationNumber: %d lf->animation: 0x%p\n", newAnimation,
                lf->animationNumber, lf->animation);
...
static void CG_BuildableAnimation(centity_t *cent, int *old, int *now, float *backLerp)
{
    ...
    else
    {
        ...
        {
            if (cg_debugAnim.integer)
                CG_Printf("%d->%d l:%d t:%d %s(%d)\n", cent->oldBuildableAnim, cent->buildableAnim, es->legsAnim,
                    es->torsoAnim, BG_Buildable(es->modelindex)->humanName, es->number);
        ...
        else if (cent->buildableIdleAnim == true && cent->buildableAnim != es->torsoAnim)
        {
            cent->buildableAnim = es->torsoAnim;
        }
...
===============
*/
static void CG_PositionAndOrientateBuildable(const vec3_t angles, const vec3_t inOrigin, const vec3_t normal,
    const int skipNumber, const vec3_t mins, const vec3_t maxs, vec3_t outAxis[3], vec3_t outOrigin)
{
    vec3_t forward, end;
    trace_t tr;
    float fraction;
    ...
    CG_CapTrace(&tr, inOrigin, mins, maxs, end, skipNumber, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
...
void CG_GhostBuildable(buildable_t buildable)
{
    ...
    CG_PositionAndOrientateBuildable(
        ps->viewangles, entity_origin, tr.plane.normal, ps->clientNum, mins, maxs, ent.axis, ent.origin);
...
static void CG_BuildableStatusDisplay(centity_t *cent)
{
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            if (tr.entityNum < MAX_CLIENTS ||
                (hit->eType == ET_BUILDABLE &&
                    (!(es->eFlags & EF_B_SPAWNED) || BG_Buildable(hit->modelindex)->transparentTest)))
            {
                entNum = tr.entityNum;
    ...
    if (cg_drawGun.integer && visible && cg.predictedPlayerState.stats[STAT_TEAM] == TEAM_HUMANS &&
...
static bool CG_PlayerIsBuilder(buildable_t buildable)
{
    switch (cg.predictedPlayerState.weapon)
    {
        ...
        case WP_ABUILD2:
        case WP_HBUILD:
            return BG_Buildable(buildable)->team == BG_Weapon(cg.predictedPlayerState.weapon)->team;
...
void CG_Buildable(centity_t *cent)
{
    ...
    if (es->pos.trType == TR_STATIONARY)
    {
        ...
        {
            CG_PositionAndOrientateBuildable(
                cent->lerpAngles, cent->lerpOrigin, surfNormal, es->number, mins, maxs, ent.axis, ent.origin);
            ...
            BG_Buildable(es->modelindex)->turretProjType == WP_TESLAGEN)
        {
                ...
                weapon->wim[WPM_PRIMARY].flashDlightColor[2])
            {
                trap_R_AddLightToScene(cent->lerpOrigin, 300 + (rand() & 31),
                    weapon->wim[WPM_PRIMARY].flashDlightColor[0], weapon->wim[WPM_PRIMARY].flashDlightColor[1],
                    weapon->wim[WPM_PRIMARY].flashDlightColor[2]);
            }
        }
