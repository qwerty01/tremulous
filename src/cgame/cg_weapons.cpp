...
void CG_RegisterUpgrade( int upgradeNum )
{
    ...
    if( upgradeInfo->registered )
    {
        CG_Printf( "CG_RegisterUpgrade: already registered: (%d) %s\n", upgradeNum,
                   BG_Upgrade( ( upgrade_t )upgradeNum )->name );
        return;
    }
    
    upgradeInfo->registered = true;
    
    if( !BG_Upgrade( ( upgrade_t )upgradeNum )->name[ 0 ] )
        CG_Error( "Couldn't find upgrade %i", upgradeNum );
        
    upgradeInfo->humanName = BG_Upgrade( ( upgrade_t )upgradeNum )->humanName;
    
    //la la la la la, i'm not listening!
    if( upgradeNum == UP_GRENADE )
        upgradeInfo->upgradeIcon = cg_weapons[ WP_GRENADE ].weaponIcon;
    else if( ( icon = BG_Upgrade( ( upgrade_t )upgradeNum )->icon ) )
...
======================
*/
static bool CG_ParseWeaponAnimationFile( const char* filename, weaponInfo_t* weapon )
{
    char*          text_p;
    int           len;
    int           i;
...
======================
*/
static bool CG_ParseWeaponFile( const char* filename, weaponInfo_t* wi )
{
    char*          text_p;
    int           len;
    char*          token;
...
void CG_RegisterWeapon( int weaponNum )
{
    ...
    if( weaponInfo->registered )
    {
        CG_Printf( "CG_RegisterWeapon: already registered: (%d) %s\n", weaponNum,
                   BG_Weapon( ( weapon_t )weaponNum )->name );
        return;
    }
    
    weaponInfo->registered = true;
    
    if( !BG_Weapon( ( weapon_t )weaponNum )->name[ 0 ] )
        CG_Error( "Couldn't find weapon %i", weaponNum );
        
    Com_sprintf( path, MAX_QPATH, "models/weapons/%s/weapon.cfg", BG_Weapon( ( weapon_t )weaponNum )->name );
    
    weaponInfo->humanName = BG_Weapon( ( weapon_t )weaponNum )->humanName;
    
    if( !CG_ParseWeaponFile( path, weaponInfo ) )
        Com_Printf( S_COLOR_RED "ERROR: failed to parse %s\n", path );
        
    Com_sprintf( path, MAX_QPATH, "models/weapons/%s/animation.cfg", BG_Weapon( ( weapon_t )weaponNum )->name );
...
static void CG_CalculateWeaponPosition( vec3_t origin, vec3_t angles )
{
    ...
    bob = BG_Class( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ] )->bob;
...
void CG_DrawItemSelect( rectDef_t* rect, vec4_t color )
{
    ...
    for( i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++ )
    {
        ...
        if( !ps->ammo && !ps->clips && !BG_Weapon( ( weapon_t )i )->infiniteAmmo )
            colinfo[ numItems ] = 1;
        else
            colinfo[ numItems ] = 0;
    ...
    for( i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++ )
    {
        if( !BG_InventoryContainsUpgrade( i, cg.snap->ps.stats ) )
            continue;
        colinfo[ numItems ] = 0;
        if( !BG_Upgrade( ( upgrade_t )i )->usable )
            colinfo[ numItems ] = 2;
    ...
    if( weapon->wim[ weaponMode ].alwaysImpact )
    {
        ...
        if( cg_entities[ entityNum ].currentState.eType == ET_PLAYER )
        {
            ...
            sound = IMPACTSOUND_FLESH;
                 ...
                 BG_Buildable( ( buildable_t )cg_entities[ entityNum ].currentState.modelindex )->team == TEAM_ALIENS )
        {
            // Alien buildables
            sound = IMPACTSOUND_FLESH;
        }
        ...
        CG_MissileHitWall( weaponNum, weaponMode, 0, origin, dir, ( impactSound_t )sound, charge );
    }
}
...
void CG_Bleed( vec3_t origin, vec3_t normal, int entityNum )
{
    ...
    {
        //ew
        team = BG_Buildable( ( buildable_t )cg_entities[ entityNum ].currentState.modelindex )->team;
        if( team == TEAM_ALIENS )
