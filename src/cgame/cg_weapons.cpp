...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent)
{
    ...
    if (weaponNum <= WP_NONE || weaponNum >= WP_NUM_WEAPONS)
    {
        Com_Printf(S_COLOR_YELLOW
            "WARNING: CG_AddPlayerWeapon: weapon "
    ...
    {
        Com_Printf(S_COLOR_YELLOW
            "WARNING: CG_AddPlayerWeapon: weapon %d (%s) "
            "is not registered\n",
            weaponNum, BG_Weapon(weaponNum)->name);
        return;
    }
    ...
    if (!ps)
    {
        ...
        if (firing && weapon->wim[weaponMode].firingSound)
        {
            trap_S_AddLoopingSound(
                cent->currentState.number, cent->lerpOrigin, vec3_origin, weapon->wim[weaponMode].firingSound);
        }
        ...
        cg.snap->ps.stats[STAT_TEAM] != TEAM_ALIENS)
    {
        trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin,
            ps ? cgs.media.lCannonWarningSound : cgs.media.lCannonWarningSound2);
    }
    ...
    if (ps || cg.renderingThirdPerson || cent->currentState.number != cg.predictedPlayerState.clientNum)
    {
        ...
        if (weapon->wim[weaponMode].flashDlightColor[0] || weapon->wim[weaponMode].flashDlightColor[1] ||
            weapon->wim[weaponMode].flashDlightColor[2])
        {
            trap_R_AddLightToScene(flash.origin, 300 + (rand() & 31), weapon->wim[weaponMode].flashDlightColor[0],
                weapon->wim[weaponMode].flashDlightColor[1], weapon->wim[weaponMode].flashDlightColor[2]);
        }
    }
}
...
void CG_AddViewWeapon(playerState_t *ps)
{
    ...
    {
        Com_Printf(S_COLOR_YELLOW
            "WARNING: CG_AddViewWeapon: weapon %d (%s) "
            "is not registered\n",
...
===============
*/
static bool CG_WeaponSelectable(weapon_t weapon) { return BG_InventoryContainsWeapon(weapon, cg.snap->ps.stats); }
...
void CG_DrawItemSelect(Rectangle *rect, vec4_t color)
{
    ...
    for (i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++)
    {
        ...
        {
            Com_Printf(S_COLOR_YELLOW
                "WARNING: CG_DrawItemSelect: weapon %d (%s) "
                "is not registered\n",
                i, BG_Weapon(i)->name);
            continue;
        }
    ...
    for (i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++)
    {
        ...
        {
            Com_Printf(S_COLOR_YELLOW
                "WARNING: CG_DrawItemSelect: upgrade %d (%s) "
                "is not registered\n",
                i, BG_Upgrade(i)->name);
            continue;
        }
...
=================
*/
void CG_MissileHitWall(weapon_t weaponNum, weaponMode_t weaponMode, int clientNum, vec3_t origin, vec3_t dir,
...
=================
*/
void CG_MissileHitEntity(
    weapon_t weaponNum, weaponMode_t weaponMode, vec3_t origin, vec3_t dir, int entityNum, int charge)
{
    vec3_t normal;
    weaponInfo_t *weapon = &cg_weapons[weaponNum];
