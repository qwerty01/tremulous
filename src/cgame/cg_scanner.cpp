...
void CG_UpdateEntityPositions(void)
{
    ...
    for (i = 0; i < cg.snap->numEntities; i++)
    {
        ...
        if (cent->currentState.eType == ET_BUILDABLE && !(cent->currentState.eFlags & EF_DEAD))
        {
...
=============
*/
static void CG_DrawBlips(Rectangle *rect, vec3_t origin, vec4_t colour)
{
    vec3_t drawOrigin;
    vec3_t up = {0, 0, 1};
    float alphaMod = 1.0f;
    float timeFractionSinceRefresh =
        1.0f - ((float)(cg.time - entityPositions.lastUpdateTime) / (float)HUMAN_SCANNER_UPDATE_PERIOD);
    vec4_t localColour;
    ...
    if (drawOrigin[2] > 0)
        CG_DrawPic(rect->x + (rect->w / 2) - (STALKWIDTH / 2) - drawOrigin[0],
            rect->y + (rect->h / 2) + drawOrigin[1] - drawOrigin[2], STALKWIDTH, drawOrigin[2],
            cgs.media.scannerLineShader);
    else
        CG_DrawPic(rect->x + (rect->w / 2) - (STALKWIDTH / 2) - drawOrigin[0], rect->y + (rect->h / 2) + drawOrigin[1],
            STALKWIDTH, -drawOrigin[2], cgs.media.scannerLineShader);

    CG_DrawPic(rect->x + (rect->w / 2) - (BLIPX / 2) - drawOrigin[0],
        rect->y + (rect->h / 2) - (BLIPY / 2) + drawOrigin[1] - drawOrigin[2], BLIPX, BLIPY,
...
=============
*/
static void CG_DrawDir(Rectangle *rect, vec3_t origin, vec4_t colour)
{
    vec3_t drawOrigin;
    vec3_t noZOrigin;
    ...
    CG_DrawPic(rect->x + (rect->w / 2) - (BLIPX2 / 2) - drawOrigin[0] * (rect->w / 2),
        rect->y + (rect->h / 2) - (BLIPY2 / 2) + drawOrigin[1] * (rect->h / 2), BLIPX2, BLIPY2,
        cgs.media.scannerBlipShader);
    trap_R_SetColor(NULL);
}
...
void THZ_DrawScanner(Rectangle *rect)
{
    ...
    // Draw cross
    CG_FillRect(rect->x + (rect->w / 2), rect->y, 1, rect->h, color);
    CG_FillRect(rect->x, rect->y + (rect->h / 2), rect->w, 1, color);
    ...
    for (i = 0; i < entityPositions.numHumanBuildables; i++)
    {
        ...
        if (VectorLength(relOrigin) < thz_radarrange.integer)
        {
            ...
            CG_FillRect(rect->x + (rect->w / 2) + -drawOrigin[0] - 5, rect->y + (rect->h / 2) + drawOrigin[1] - 5, 10,
                10, buildcolor);
        }
    }
    ...
    for (i = 0; i < entityPositions.numHumanClients; i++)
    {
        ...
        if (VectorLength(relOrigin) < thz_radarrange.integer)
        {
            ...
            CG_FillRect(rect->x + (rect->w / 2) + -drawOrigin[0] - 3, rect->y + (rect->h / 2) + drawOrigin[1] - 3, 6, 6,
                humancolor);
        }
    }
