...
extern "C" Q_EXPORT intptr_t vmMain(int command, int arg0, int arg1, int arg2)
{
    switch (command)
    {
        ...
        case CG_DRAW_ACTIVE_FRAME:
            CG_DrawActiveFrame(arg0, static_cast<stereoFrame_t>(arg1), static_cast<bool>(arg2));
            return 0;
        ...
        case CG_KEY_EVENT:
            CG_KeyEvent(arg0, static_cast<bool>(arg1));
            return 0;
...
void CG_RegisterCvars(void)
{
    ...
    cgs.localServer = static_cast<bool>(atoi(var));
}
...
static void CG_SetUIVars(void)
{
    ...
    *carriageCvar = 0;
    ...
    {
        if (BG_InventoryContainsUpgrade(i, cg.snap->ps.stats) && BG_Upgrade(i)->purchasable)
...
void CG_AddNotifyText(void)
{
    ...
    cg.consoleLines[cg.numConsoleLines].length = MIN(bufferLen, MAX_CONSOLE_TEXT - textLen - 1);
    cg.numConsoleLines++;
}
...
=================
*/
int CG_FileExists(const char *filename) { return trap_FS_FOpenFile(filename, NULL, FS_READ); }
...
static void CG_RegisterSounds(void)
{
    ...
    for (i = 0; i < 4; i++)
        cgs.media.humanBuildableDamage[i] = trap_S_RegisterSound(va("sound/buildables/human/damage%d.wav", i), false);
...
static void CG_RegisterGraphics(void)
{
    ...
    static const char *buildWeaponTimerPieShaders[8] = {
...
static void CG_RegisterClients(void)
{
    ...
    {
        CG_PrecacheClientInfo(i, BG_ClassConfig(i)->modelName, BG_ClassConfig(i)->skinName);
...
static bool CG_OwnerDrawHandleKey(int ownerDraw, int key) { return false; }
...
static const char *CG_FeederItemText(int feederID, int index, int column, qhandle_t *handle)
{
    ...
    *handle = -1;
    ...
    if (cg.intermissionStarted && CG_ClientIsReady(sp->client))
        showIcons = false;
    else if (cg.snap->ps.pm_type == PM_SPECTATOR || cg.snap->ps.pm_type == PM_NOCLIP ||
             cg.snap->ps.pm_flags & PMF_FOLLOW || team == cg.snap->ps.stats[STAT_TEAM] || cg.intermissionStarted)
    {
        showIcons = true;
    }
...
static qhandle_t CG_FeederItemImage(int feederID, int index) { return 0; }
...
static bool CG_OwnerDrawVisible(int parameter) { return false; }
...
void CG_AssetCache(void)
{
    ...
    {
        cgDC.Assets.emoticonCount = BG_LoadEmoticons(cgDC.Assets.emoticons, MAX_EMOTICONS);
    }
    else
    ...
    for (i = 0; i < cgDC.Assets.emoticonCount; i++)
    {
        cgDC.Assets.emoticons[i].shader = trap_R_RegisterShaderNoMip(
            va("emoticons/%s_%dx1.tga", cgDC.Assets.emoticons[i].name, cgDC.Assets.emoticons[i].width));
    }
}
...
void CG_Init(int serverMessageNum, int serverCommandSequence, int clientNum)
{
    ...
    Q_strncpyz(cgs.voteString[TEAM_ALIENS], CG_ConfigString(CS_VOTE_STRING + TEAM_ALIENS),
        sizeof(cgs.voteString[TEAM_ALIENS]));
    Q_strncpyz(cgs.voteString[TEAM_HUMANS], CG_ConfigString(CS_VOTE_STRING + TEAM_ALIENS),
...
=================
*/
void CG_Shutdown(void) { CG_UnregisterCommands(); }
