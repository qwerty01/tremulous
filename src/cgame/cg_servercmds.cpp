...
void CG_SetConfigValues(void)
{
    ...
    if (alienStages[0])
    {
        sscanf(alienStages, "%d %d %d", &cgs.alienStage, &cgs.alienCredits, &cgs.alienNextStageThreshold);
    }
    else
    ...
    if (humanStages[0])
    {
        sscanf(humanStages, "%d %d %d", &cgs.humanStage, &cgs.humanCredits, &cgs.humanNextStageThreshold);
    }
    else
...
static void CG_ConfigStringModified(void)
{
    ...
    else if (num == CS_ALIEN_STAGES)
    {
        ...
        if (str[0])
        {
            sscanf(str, "%d %d %d", &cgs.alienStage, &cgs.alienCredits, &cgs.alienNextStageThreshold);
    ...
    else if (num == CS_HUMAN_STAGES)
    {
        ...
        if (str[0])
        {
            sscanf(str, "%d %d %d", &cgs.humanStage, &cgs.humanCredits, &cgs.humanNextStageThreshold);
    ...
    else if (num >= CS_VOTE_TIME && num < CS_VOTE_TIME + NUM_TEAMS)
    {
        ...
        if (num - CS_VOTE_TIME == TEAM_NONE)
            ...
            trap_Cvar_Set("ui_alienTeamVoteActive", cgs.voteTime[TEAM_ALIENS] ? "1" : "0");
        else if (num - CS_VOTE_TIME == TEAM_HUMANS)
            trap_Cvar_Set("ui_humanTeamVoteActive", cgs.voteTime[TEAM_HUMANS] ? "1" : "0");
    }
...
void CG_Menu(int menu, int arg)
{
    ...
    const char *dialog;
    dialogType_t type = DT_INTERACTIVE;  // controls which cg_disable var will switch it off
    ...
    switch (menu)
    {
        ...
        case MN_A_TEAMLOCKED:
            longMsg =
                "The alien team is locked. You cannot join the aliens "
                "at this time.";
        ...
        case MN_H_TEAMLOCKED:
            longMsg =
                "The human team is locked. You cannot join the humans "
                "at this time.";
        ...
        case MN_PLAYERLIMIT:
            longMsg =
                "The maximum number of playing clients has been reached. "
        ...
        case MN_CMD_CHEAT_TEAM:
            shortMsg =
                "Cheats are not enabled on this server, so "
        ...
        case MN_B_LASTSPAWN:
            longMsg =
                "This action would remove your team's last spawn point, "
        ...
        case MN_B_SUDDENDEATH:
            longMsg =
                "Neither team has prevailed after a certain time and the "
        ...
        case MN_B_REVOKED:
            longMsg =
                "Your teammates have lost faith in your ability to build "
        ...
        case MN_B_SURRENDER:
            longMsg =
                "Your team has decided to admit defeat and concede the game:"
        ...
        case MN_H_NOBP:
            if (cgs.markDeconstruct)
                longMsg =
                    "There is no power remaining. Free up power by marking "
                    "existing buildable objects.";
            else
                longMsg =
                    "There is no power remaining. Free up power by deconstructing "
                    "existing buildable objects.";
        ...
        case MN_H_ONEREACTOR:
            longMsg =
                "There can only be one Reactor. Deconstruct the existing one if you "
                "wish to move it.";
        ...
        case MN_H_NOENERGYAMMOHERE:
            longMsg =
                "You must be near a Reactor or a powered Armoury or Repeater "
        ...
        case MN_H_ARMOURYBUILDTIMER:
            ...
            shortMsg =
                "You can not buy or sell weapons until your build timer "
                "expires";
            type = DT_ARMOURYEVOLVE;
            break;
        ...
        case MN_A_INFEST:
            trap_Cvar_Set(
                "ui_currentClass", va("%d %d", cg.snap->ps.stats[STAT_CLASS], cg.snap->ps.persistant[PERS_CREDIT]));
        ...
        case MN_A_CANTEVOLVE:
            shortMsg = va("You cannot evolve into a %s", BG_ClassConfig(arg)->humanName);
            type = DT_ARMOURYEVOLVE;
            break;
        ...
        case MN_A_CLASSNOTSPAWN:
            shortMsg = va("You cannot spawn as a %s", BG_ClassConfig(arg)->humanName);
            type = DT_ARMOURYEVOLVE;
            break;

        case MN_A_CLASSNOTALLOWED:
            shortMsg = va("The %s is not allowed", BG_ClassConfig(arg)->humanName);
            type = DT_ARMOURYEVOLVE;
            break;

        case MN_A_CLASSNOTATSTAGE:
            shortMsg = va("The %s is not allowed at Stage %d", BG_ClassConfig(arg)->humanName, cgs.alienStage + 1);
            type = DT_ARMOURYEVOLVE;
            break;
...
static void CG_Say(int clientNum, saymode_t mode, const char *text)
{
    ...
    if (clientNum >= 0 && clientNum < MAX_CLIENTS)
    {
        ...
        if ((mode == SAY_TEAM || mode == SAY_AREA) && cg.snap->ps.pm_type != PM_INTERMISSION)
        {
            int locationNum;
    ...
    switch (mode)
    {
...
#ifdef MODULE_INTERFACE_11
            CG_Printf(
                "%s%s(%s" S_COLOR_WHITE ")%s%s " S_COLOR_CYAN "%s\n", ignore, prefix, name, location, maybeColon, text);
#else
            CG_Printf("%s%s(%s" S_COLOR_WHITE ")%s%s %c" S_COLOR_CYAN "%s\n", ignore, prefix, name, location,
                maybeColon, INDENT_MARKER, text);
#endif
            break;
        case SAY_ADMINS:
        case SAY_ADMINS_PUBLIC:
            ...
            CG_Printf("%s%s%s%s" S_COLOR_WHITE "%s " S_COLOR_MAGENTA "%s\n", ignore, prefix,
                (mode == SAY_ADMINS) ? "[ADMIN]" : "[PLAYER]", name, maybeColon, text);
#else
            CG_Printf("%s%s%s%s" S_COLOR_WHITE "%s %c" S_COLOR_MAGENTA "%s\n", ignore, prefix,
                (mode == SAY_ADMINS) ? "[ADMIN]" : "[PLAYER]", name, maybeColon, INDENT_MARKER, text);
#endif
            break;
        case SAY_AREA:
#ifdef MODULE_INTERFACE_11
            CG_Printf(
                "%s%s<%s" S_COLOR_WHITE ">%s%s " S_COLOR_BLUE "%s\n", ignore, prefix, name, location, maybeColon, text);
#else
        ...
        case SAY_TPRIVMSG:
            ...
            CG_Printf("%s%s[%s" S_COLOR_WHITE " -> %s" S_COLOR_WHITE "]%s %s%s\n", ignore, prefix, name,
                cgs.clientinfo[cg.clientNum].name, maybeColon, color, text);
#else
            CG_Printf("%s%s[%s" S_COLOR_WHITE " -> %s" S_COLOR_WHITE "]%s %c%s%s\n", ignore, prefix, name,
                cgs.clientinfo[cg.clientNum].name, maybeColon, INDENT_MARKER, color, text);
#endif
            if (!ignore[0])
            {
                CG_CenterPrint(
                    va("%sPrivate message from: " S_COLOR_WHITE "%s", color, name), 200, GIANTCHAR_WIDTH * 4);
                if (clientNum < 0 || clientNum >= MAX_CLIENTS)
...
=================
*/
void CG_CenterPrint_f(void) { CG_CenterPrint(CG_Argv(1), SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH); }
...
static void CG_Chat_f(void)
{
    ...
    CG_Say(atoi(id), static_cast<saymode_t>(atoi(mode)), CG_Argv(3));
}
...
static consoleCommand_t svcommands[] = {{"chat", CG_Chat_f}, {"cmds", CG_GameCmds_f}, {"cp", CG_CenterPrint_f},
    {"cs", CG_ConfigStringModified}, {"map_restart", CG_MapRestart}, {"poisoncloud", CG_PoisonCloud_f},
    {"print", CG_Print_f}, {"scores", CG_ParseScores}, {"serverclosemenus", CG_ServerCloseMenus_f},
    {"servermenu", CG_ServerMenu_f}, {"tinfo", CG_ParseTeamInfo}, {"voice", CG_ParseVoice}};
...
static void CG_ServerCommand(void)
{
    ...
    cmd = CG_Argv(0);
    command =
        static_cast<consoleCommand_t *>(bsearch(cmd, svcommands, ARRAY_LEN(svcommands), sizeof(svcommands[0]), cmdcmp));
