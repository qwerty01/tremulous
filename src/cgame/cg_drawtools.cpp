...
=================
*/
void CG_ClearClipRegion(void) { trap_R_SetClipRegion(NULL); }
...
*/
void CG_DrawFadePic(
    float x, float y, float width, float height, vec4_t fcolor, vec4_t tcolor, float amount, qhandle_t hShader)
{
    vec4_t finalcolor;
    float inverse;
...
char CG_GetColorCharForHealth(int clientnum)
{
    ...
    healthPercent = (int)(100.0f * (float)cgs.clientinfo[clientnum].health / (float)maxHealth);
