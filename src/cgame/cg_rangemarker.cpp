...
================
*/
bool CG_GetRangeMarkerPreferences(bool *drawSurface, bool *drawIntersection, bool *drawFrontline, float *surfaceOpacity,
    float *lineOpacity, float *lineThickness)
{
    ...
    *lineThickness = cg_rangeMarkerLineThickness.value;
    ...
    if ((*drawSurface && *surfaceOpacity > 0.0f) ||
        ((*drawIntersection || *drawFrontline) && *lineOpacity > 0.0f && *lineThickness > 0.0f &&
            cg_binaryShaderScreenScale.value > 0.0f))
    {
...
void CG_UpdateBuildableRangeMarkerMask(void)
{
    ...
    if (cg_rangeMarkerBuildableTypes.modificationCount != mc)
    {
        ...
        for (;;)
        {
            ...
            {
                brmMask |= (1 << BA_A_OVERMIND) | (1 << BA_A_SPAWN) | (1 << BA_A_ACIDTUBE) | (1 << BA_A_TRAPPER) |
                           (1 << BA_A_HIVE) | (1 << BA_H_REACTOR) | (1 << BA_H_REPEATER) | (1 << BA_H_DCC) |
            ...
            else
            {
                ...
                {
                    pp = p + 5;
                    only = (1 << BA_A_OVERMIND) | (1 << BA_A_SPAWN) | (1 << BA_A_ACIDTUBE) | (1 << BA_A_TRAPPER) |
                           (1 << BA_A_HIVE);
                }
                else if (!Q_stricmpn(p, "human", 5))
                {
                    pp = p + 5;
                    only = (1 << BA_H_REACTOR) | (1 << BA_H_REPEATER) | (1 << BA_H_DCC) | (1 << BA_H_MGTURRET) |
                           (1 << BA_H_TESLAGEN);
                }
                else
                {
                ...
                {
                    brmMask |= only & ((1 << BA_A_OVERMIND) | (1 << BA_A_SPAWN) | (1 << BA_H_REACTOR) |
                                          (1 << BA_H_REPEATER) | (1 << BA_H_DCC));
                }
...
void CG_DrawSphericalCone(
    const vec3_t tip, const vec3_t rotation, float radius, bool a240, int customShader, const float *shaderRGBA)
{
    refEntity_t re;
    memset(&re, 0, sizeof(re));
...
void CG_DrawRangeMarker(rangeMarkerType_t rmType, const vec3_t origin, const float *angles, float range,
    bool drawSurface, bool drawIntersection, bool drawFrontline, const vec3_t rgb, float surfaceOpacity,
    float lineOpacity, float lineThickness)
{
    if (drawSurface)
    {
...
void CG_GhostBuildableRangeMarker(buildable_t buildable, const vec3_t origin, const vec3_t normal)
{
        ...
        CG_GetBuildableRangeMarkerProperties(buildable, &rmType, &range, rgb))
    {
        ...
        CG_DrawRangeMarker(
            rmType, localOrigin, (rmType != RMT_SPHERE ? angles : NULL), range, drawS, drawI, drawF, rgb, so, lo, th);
    }
}
...
void CG_RangeMarker(centity_t *cent)
{
    ...
    {
        CG_DrawRangeMarker(rmType, cent->lerpOrigin, (rmType > 0 ? cent->lerpAngles : NULL), range, drawS, drawI, drawF,
            rgb, so, lo, th);
    }
}
