...
static void CG_CalculateBeamNodeProperties( trailBeam_t *tb )
{
  ...
  TCRange = tb->klass->backTextureCoord -
    tb->klass->frontTextureCoord;
  widthRange = tb->klass->backWidth -
    tb->klass->frontWidth;
  alphaRange = tb->klass->backAlpha -
    tb->klass->frontAlpha;
  VectorSubtract( tb->klass->backColor,
      tb->klass->frontColor, colorRange );
  ...
  for( j = 0, i = tb->nodes; i; i = i->next, j++ )
  {
    ...
    {
      i->textureCoord = tb->klass->frontTextureCoord +
    ...
    else if( tb->klass->textureType == TBTT_REPEAT )
    {
      if( tb->klass->clampToBack )
        i->textureCoord = ( totalDistance - position ) /
          tb->klass->repeatLength;
      else
        i->textureCoord = position / tb->klass->repeatLength;
    }

    i->halfWidth = ( tb->klass->frontWidth +
      ( ( position / totalDistance ) * widthRange ) ) / 2.0f;
    i->alpha = (byte)( (float)0xFF * ( tb->klass->frontAlpha +
      ( ( position / totalDistance ) * alphaRange ) ) * fadeAlpha );
    VectorMA( tb->klass->frontColor, ( position / totalDistance ),
        colorRange, i->color );
    ...
    if( i && i->timeLeft >= 0 )
    {
      ...
      else if( i->timeLeft >= 0 && i->prev )
      {
        ...
        length = VectorNormalize( dir ) *
          ( (float)i->timeLeft / (float)tb->klass->segmentTime );
...
static void CG_GarbageCollectTrailSystems( void )
{
  ...
  for( i = 0; i < MAX_TRAIL_SYSTEMS; i++ )
  {
    ...
    // lifetime expired
    if( ts->destroyTime <= 0 && ts->klass->lifeTime &&
        ts->birthTime + ts->klass->lifeTime < cg.time )
    {
      trailSystem_t *tempTS = ts;
      ...
      if( cg_debugTrails.integer >= 1 )
        CG_Printf( "TS %s expired (born %d, lives %d, now %d)\n",
                   ts->klass->name, ts->birthTime, ts->klass->lifeTime,
                   cg.time );
    }
