...
static void CG_CalculateBeamNodeProperties(trailBeam_t *tb)
{
    ...
    TCRange = tb->klass->backTextureCoord - tb->klass->frontTextureCoord;
    widthRange = tb->klass->backWidth - tb->klass->frontWidth;
    alphaRange = tb->klass->backAlpha - tb->klass->frontAlpha;
    VectorSubtract(tb->klass->backColor, tb->klass->frontColor, colorRange);
    ...
    {
        if (tb->klass->textureType == TBTT_STRETCH)
        {
            i->textureCoord = tb->klass->frontTextureCoord + ((position / totalDistance) * TCRange);
        }
        else if (tb->klass->textureType == TBTT_REPEAT)
        {
            if (tb->klass->clampToBack)
                i->textureCoord = (totalDistance - position) / tb->klass->repeatLength;
            else
                i->textureCoord = position / tb->klass->repeatLength;
        }

        i->halfWidth = (tb->klass->frontWidth + ((position / totalDistance) * widthRange)) / 2.0f;
        i->alpha =
            (byte)((float)0xFF * (tb->klass->frontAlpha + ((position / totalDistance) * alphaRange)) * fadeAlpha);
        VectorMA(tb->klass->frontColor, (position / totalDistance), colorRange, i->color);
...
static void CG_RenderBeam(trailBeam_t *tb)
{
    ...
    trap_R_AddPolysToScene(tb->klass->shader, 4, &verts[0], numVerts / 4);
}
    ...
    else if (CG_Attached(&ts->frontAttachment))
    {
        ...
        if (i && i->timeLeft >= 0)
        {
            ...
            else if (i->timeLeft >= 0 && i->prev)
            {
                ...
                length = VectorNormalize(dir) * ((float)i->timeLeft / (float)tb->klass->segmentTime);
    ...
    while (1)
    {
        ...
        if (!Q_stricmp(token, "{"))
        {
            if (tsNameSet)
            {
                ...
                {
                    CG_Printf(
                        S_COLOR_RED "ERROR: maximum number of trail systems (%d) reached\n", MAX_BASETRAIL_SYSTEMS);
                    return false;
                }
                else
                    numBaseTrailSystems++;
...
static void CG_GarbageCollectTrailSystems(void)
{
    ...
    for (i = 0; i < MAX_TRAIL_SYSTEMS; i++)
    {
        ...
        if ((centNum = CG_AttachmentCentNum(&ts->frontAttachment)) >= 0 && centNum != cg.snap->ps.clientNum)
        {
        ...
        if ((centNum = CG_AttachmentCentNum(&ts->backAttachment)) >= 0 && centNum != cg.snap->ps.clientNum)
        {
            trailSystem_t *tempTS = ts;
        ...
        // lifetime expired
        if (ts->destroyTime <= 0 && ts->klass->lifeTime && ts->birthTime + ts->klass->lifeTime < cg.time)
        {
            trailSystem_t *tempTS = ts;

            CG_DestroyTrailSystem(&tempTS);
            if (cg_debugTrails.integer >= 1)
                CG_Printf("TS %s expired (born %d, lives %d, now %d)\n", ts->klass->name, ts->birthTime,
                    ts->klass->lifeTime, cg.time);
        }
