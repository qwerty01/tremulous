...
static void CG_DestroyParticle(particle_t *p, vec3_t impactNormal)
{
    ...
    if (p->klass->onDeathSystemName[0] != '\0')
    {
        particleSystem_t *ps;

        ps = CG_SpawnNewParticleSystem(p->klass->onDeathSystemHandle);
...
static particle_t *CG_SpawnNewParticle(baseParticle_t *bp, particleEjector_t *parent)
{
    ...
    for (i = 0; i < MAX_PARTICLES; i++)
    {
        ...
        if (!p->valid && cg.clientFrame > p->frameWhenInvalidated + 1)
        {
            ...
            p->dLightRadius.initial = CG_RandomiseValue(bp->dLightRadius.initial, bp->dLightRadius.initialRandFrac);
            p->dLightRadius.final = CG_RandomiseValue(bp->dLightRadius.final, bp->dLightRadius.finalRandFrac);
            ...
            p->bounceMarkCount = rint(CG_RandomiseValue((float)bp->bounceMarkCount, bp->bounceMarkCountRandFrac));
            p->bounceSoundCount = rint(CG_RandomiseValue((float)bp->bounceSoundCount, bp->bounceSoundCountRandFrac));
            ...
            VectorScale(
                p->velocity, CG_RandomiseValue(bp->velMoveValues.mag, bp->velMoveValues.magRandFrac), p->velocity);
        ...
        if (pe->valid)
        {
            ...
            if (!ps->lazyRemove)
            {
                while (pe->nextEjectionTime <= cg.time && (pe->count > 0 || pe->totalParticles == PARTICLES_INFINITE))
                {
                    ...
                    pe->nextEjectionTime = cg.time + (int)CG_RandomiseValue(CG_LerpValues(pe->ejectPeriod.initial,
                                                                                pe->ejectPeriod.final, lerpFrac),
...
===============
*/
static particleEjector_t *CG_SpawnNewParticleEjector(baseParticleEjector_t *bpe, particleSystem_t *parent)
{
    int i;
...
===============
*/
static bool CG_ParseColor(byte *c, char **text_p)
{
    char *token;
    int i;
...
static bool CG_ParseFinal(pLerpValues_t *plv, char **text_p)
{
    char *token = COM_Parse(text_p);
    if (!*token)
        return false;
    ...
    {
        CG_ParseValueAndVariance(token, &plv->final, &plv->finalRandFrac, false);
    }
    ...
    while (1)
    {
        ...
        else if (!Q_stricmp(token, "modelAnimation"))
        {
            ...
            else
            {
                ...
                bp->modelAnimation.frameLerp = bp->modelAnimation.initialLerp = 1000 / fps;
            }
        }
        ///
        ...
        else if (!Q_stricmp(token, "parentVelocityFraction"))
        {
            ...
            CG_ParseValueAndVariance(
                token, &bp->velMoveValues.parentVelFrac, &bp->velMoveValues.parentVelFracRandFrac, false);
        }
...
static bool CG_ParseParticleSystem(baseParticleSystem_t *bps, char **text_p, const char *name)
{
    ...
    while (1)
    {
        ...
        if (!Q_stricmp(token, "{"))
        {
            ...
            {
                CG_Printf(
                    S_COLOR_RED "ERROR: maximum number of particle ejectors (%d) reached\n", MAX_BASEPARTICLE_EJECTORS);
                return false;
            }
...
void CG_LoadParticleSystems(void)
{
    ...
    for (i = 0; i < numBaseParticles; i++)
    {
        ...
        {
            // particle klass has a child, resolve the name
...
static void CG_GarbageCollectParticleSystems(void)
{
    ...
    for (i = 0; i < MAX_PARTICLE_SYSTEMS; i++)
    {
        ...
        if ((centNum = CG_AttachmentCentNum(&ps->attachment)) >= 0 && centNum != cg.snap->ps.clientNum)
        {
...
#define MAX_ACC_RADIUS 1000.0f
    ...
    else if (bp->accMoveValues.dirType == PMD_LINEAR)
    {
        ...
        CG_SpreadVector(acceleration, bp->accMoveValues.dirRandAngle);
        VectorScale(
            acceleration, CG_RandomiseValue(bp->accMoveValues.mag, bp->accMoveValues.magRandFrac), acceleration);
    }
    ...
    CG_Trace(&trace, p->origin, mins, maxs, newOrigin, CG_AttachmentCentNum(&ps->attachment), CONTENTS_SOLID);
    ...
    {
        CG_ImpactMark(bp->bounceMark, trace.endpos, trace.plane.normal, random() * 360, 1, 1, 1, 1, true,
            bp->bounceMarkRadius, false);
        p->bounceMarkCount--;
    }
...
static void CG_RenderParticle(particle_t *p)
{
    ...
    baseParticle_t *bp = p->klass;
    particleSystem_t *ps = p->parent->parent;
    baseParticleSystem_t *bps = ps->klass;
    ...
    if (bp->numFrames)  // shader based
    {
        ...
        else
        {
            ...
            VectorMA(bp->initialColor, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->colorDelay), colorRange,
                re.shaderRGBA);
        }

        re.shaderRGBA[3] = (byte)((float)0xFF * CG_LerpValues(p->alpha.initial, p->alpha.final,
    ...
    if (bps->thirdPersonOnly && CG_AttachmentCentNum(&ps->attachment) == cg.snap->ps.clientNum &&
    ...
    if (bp->dynamicLight && !(re.renderfx & RF_THIRD_PERSON))
    {
        trap_R_AddLightToScene(p->origin,
            ...
            (float)bp->dLightColor[0] / (float)0xFF, (float)bp->dLightColor[1] / (float)0xFF,
