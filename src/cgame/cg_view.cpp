...
void CG_OffsetShoulderView( void )
{
    ...
    classConfig = BG_ClassConfig( ( class_t )cg.snap->ps.stats[ STAT_CLASS ] );
...
static void CG_StepOffset( void )
{
    ...
    steptime = BG_Class( ( class_t )ps->stats[ STAT_CLASS ] )->steptime;
...
void CG_OffsetFirstPersonView( void )
{
    ...
    if( cg.snap->ps.persistant[ PERS_SPECSTATE ] != SPECTATOR_NOT )
        ...
        bob2 = BG_Class( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ] )->bob;
...
static int CG_CalcFov( void )
{
    ...
    else
    {
        ...
        attribFov = BG_Class( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ] )->fov * 0.75f;
        fov_y = attribFov;
        ...
        if( cg.spawnTime > ( cg.time - FOVWARPTIME ) &&
                BG_ClassHasAbility( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ], SCA_FOVWARPS ) )
        {
        ...
        // account for zooms
        zoomFov = BG_Weapon( ( weapon_t )cg.predictedPlayerState.weapon )->zoomFov * 0.75f;
        if( zoomFov < 1.0f )
            zoomFov = 1.0f;
        ...
        if( BG_Weapon( ( weapon_t )cg.predictedPlayerState.weapon )->canZoom )
        {
            if( cg.zoomed )
            {
...
static int CG_CalcViewValues( void )
{
    ...
    if( BG_ClassHasAbility( ( class_t )ps->stats[ STAT_CLASS ], SCA_WALLCLIMBER ) )
        CG_smoothWWTransitions( ps, ps->viewangles, cg.refdefViewAngles );
    else if( BG_ClassHasAbility( ( class_t )ps->stats[ STAT_CLASS ], SCA_WALLJUMPER ) )
    ...
    if( !BG_ClassHasAbility( ( class_t )ps->stats[ STAT_CLASS ], SCA_WALLJUMPER ) )
    {
