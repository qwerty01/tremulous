...
void CG_OffsetThirdPersonView(void)
{
    ...
    if ((cg_thirdPersonShoulderViewMode.integer == 2) ||
        ((cg_thirdPersonShoulderViewMode.integer == 1) && (cg.snap->ps.stats[STAT_STATE] & SS_WALLCLIMBING) &&
...
#define LEVEL3_FEEDBACK 20.0f
    ...
    if ((cg.predictedPlayerState.weapon == WP_ALEVEL3 || cg.predictedPlayerState.weapon == WP_ALEVEL3_UPG) &&
        cg.predictedPlayerState.stats[STAT_MISC] > 0)
    {
        ...
        fraction1 = (float)cg.predictedPlayerState.stats[STAT_MISC] / LEVEL3_POUNCE_TIME_UPG;
        if (fraction1 > 1.0f)
            fraction1 = 1.0f;
...
#define STRUGGLE_TIME 250
    ...
    if ((cg.predictedPlayerEntity.currentState.eFlags & EF_POISONCLOUDED) &&
        (cg.time - cg.poisonedTime < PCLOUD_DISORIENT_DURATION) && !(cg.snap->ps.pm_flags & PMF_FOLLOW))
    {
        float scale, fraction, pitchFraction;

        scale = 1.0f - (float)(cg.time - cg.poisonedTime) / BG_PlayerPoisonCloudTime(&cg.predictedPlayerState);
        if (scale < 0.0f)
            scale = 0.0f;

        fraction = sin((cg.time - cg.poisonedTime) / 500.0f * M_PI * PCLOUD_ROLL_FREQUENCY) * scale;
        pitchFraction = sin((cg.time - cg.poisonedTime) / 200.0f * M_PI * PCLOUD_ROLL_FREQUENCY) * scale;
    ...
    if (cg.predictedPlayerState.stats[STAT_TEAM] == TEAM_HUMANS &&
        (cg.predictedPlayerState.pm_type == PM_NORMAL || cg.predictedPlayerState.pm_type == PM_JETPACK))
    {
...
static int CG_CalcFov(void)
{
    ...
    if (cg.predictedPlayerState.pm_type == PM_INTERMISSION ||
        (cg.snap->ps.persistant[PERS_SPECSTATE] != SPECTATOR_NOT) || (cg.renderingThirdPerson))
    {
    ...
    if ((cg.predictedPlayerEntity.currentState.eFlags & EF_POISONCLOUDED) &&
        (cg.time - cg.poisonedTime < PCLOUD_DISORIENT_DURATION) && cg.predictedPlayerState.stats[STAT_HEALTH] > 0 &&
        !(cg.snap->ps.pm_flags & PMF_FOLLOW))
    {
        float scale = 1.0f - (float)(cg.time - cg.poisonedTime) / BG_PlayerPoisonCloudTime(&cg.predictedPlayerState);
