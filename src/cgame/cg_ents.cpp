...
void CG_PositionRotatedEntityOnTag(
    refEntity_t *entity, const refEntity_t *parent, qhandle_t parentModel, const char *tagName)
{
    int i;
    orientation_t lerped;
    vec3_t tempAxis[3];
    ...
    if (cent->currentState.loopSound)
    {
        if (cent->currentState.eType != ET_SPEAKER)
        {
            trap_S_AddLoopingSound(
                cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.gameSounds[cent->currentState.loopSound]);
        }
        else
        {
...
static void CG_WeaponDrop(centity_t *cent)
{
    ...
    if (BG_Weapon(es->modelindex)->number == WP_NONE)
    {
...
static void CG_Missile(centity_t *cent)
{
    ...
    {
        trap_R_AddLightToScene(cent->lerpOrigin, wim->missileDlight, wim->missileDlightColor[0],
            wim->missileDlightColor[1], wim->missileDlightColor[2]);
    }
...
#define SETBOUNDS(v1, v2, r)                                                                           \
    ((v1)[0] = (-r / 2), (v1)[1] = (-r / 2), (v1)[2] = (-r / 2), (v2)[0] = (r / 2), (v2)[1] = (r / 2), \
        (v2)[2] = (r / 2))
...
static void CG_CalcEntityLerpPositions(centity_t *cent)
{
    ...
    if (cg_projectileNudge.integer && !cg.demoPlayback && cent->currentState.eType == ET_MISSILE &&
    ...
    {
        CG_AdjustPositionForMover(
            cent->lerpOrigin, cent->currentState.groundEntityNum, cg.snap->serverTime, cg.time, cent->lerpOrigin);
    }
}
