...
==============
*/
const classAttributes_t* BG_Class( class_t _class )
{
    return ( _class >= PCL_NONE && _class < PCL_NUM_CLASSES ) ?
           &bg_classList[ _class ] : &nullClass;
}
...
==============
*/
bool BG_ClassAllowedInStage( class_t _class, stage_t stage )
{
    int stages = BG_Class( _class )->stages;
...
==============
*/
classConfig_t* BG_ClassConfig( class_t _class )
{
...
==============
*/
void BG_ClassBoundingBox( class_t _class,
                          vec3_t mins, vec3_t maxs,
                          vec3_t cmaxs, vec3_t dmins, vec3_t dmaxs )
{
    classConfig_t* classConfig = BG_ClassConfig( _class );
...
==============
*/
bool BG_ClassHasAbility( class_t _class, int ability )
{
    int abilities = BG_Class( _class )->abilities;
    ...
    for( i = 0; i < bg_numClasses; i++ )
    {
        ...
        for( j = 0; j < 3; j++ )
        {
            ...
            if( thruClass == PCL_NONE || !BG_ClassAllowedInStage( ( class_t )thruClass, ( stage_t )stage ) ||
                    !BG_ClassIsAllowed( ( class_t )thruClass ) )
                continue;
                
            evolveCost = BG_Class( ( class_t )thruClass )->cost * ALIEN_CREDITS_PER_KILL;
            if( thruClass == tclass )
                value = cost + evolveCost;
            else
                value = BG_ClassCanEvolveFromTo( ( class_t )thruClass, tclass, credits, stage, cost + evolveCost );
...
==============
*/
bool BG_AlienCanEvolve( class_t _class, int credits, int stage )
{
    int i, j, tclass;
    
    for( i = 0; i < bg_numClasses; i++ )
    {
        if( bg_classList[ i ].number != _class )
            continue;
        ...
        for( j = 0; j < 3; j++ )
        {
            ...
            if( tclass != PCL_NONE && BG_ClassAllowedInStage( ( class_t )tclass, ( stage_t )stage ) &&
                    BG_ClassIsAllowed( ( class_t )tclass ) &&
                    credits >= BG_Class( ( class_t )tclass )->cost * ALIEN_CREDITS_PER_KILL )
                return true;
        }
...
void BG_InitClassConfigs( void )
{
    ...
    {
        cc = BG_ClassConfig( ( class_t )i );
        
        BG_ParseClassFile( va( "configs/classes/%s.cfg", BG_Class( ( class_t )i )->name ), cc );
    }
}
...
int BG_SlotsForInventory( int stats[ ] )
{
    ...
    slots = BG_Weapon( ( weapon_t )stats[ STAT_WEAPON ] )->slots;
    ...
    for( i = UP_NONE; i < UP_NUM_UPGRADES; i++ )
    {
        ...
        {
            slot = BG_Upgrade( ( upgrade_t )i )->slots;
        ...
        vec3_t outOrigin, vec3_t outAngles, trace_t* tr )
{
    ...
    buildDist = BG_Class( ( class_t )ps->stats[ STAT_CLASS ] )->buildDist;
...
int BG_GetValueOfPlayer( playerState_t* ps )
{
    ...
    worth = BG_Class( ( class_t )ps->stats[ STAT_CLASS ] )->value;
    ...
    if( ps->stats[ STAT_TEAM ] == TEAM_HUMANS )
    {
        ...
        if( ps->stats[ STAT_WEAPON ] != WP_NONE )
            worth += BG_Weapon( ( weapon_t )ps->stats[ STAT_WEAPON ] )->price;
    }
...
============
*/
bool BG_ClassIsAllowed( class_t _class )
{
    int i;
