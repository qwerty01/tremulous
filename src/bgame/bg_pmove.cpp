...
static void PM_Friction( void )
{
    ...
    if( pm->waterlevel <= 1 )
    {
        if( ( pml.walking || pml.ladder ) && !( pml.groundTrace.surfaceFlags & SURF_SLICK ) )
        {
            ...
            {
                float stopSpeed = BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->stopSpeed;
                float friction = BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->friction;
...
static bool PM_CheckWallJump( void )
{
    ...
    if( !( BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->abilities & SCA_WALLJUMPER ) )
        return false;
    ...
    VectorMA( pm->ps->velocity, BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->jumpMagnitude,
              dir, pm->ps->velocity );
...
static bool PM_CheckJump( void )
{
    ...
    if( BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->jumpMagnitude == 0.0f )
        return false;
    ...
    if( BG_ClassHasAbility( ( class_t )pm->ps->stats[ STAT_CLASS ], SCA_WALLJUMPER ) )
    {
        pm->ps->pm_flags |= PMF_TIME_WALLJUMP;
        pm->ps->pm_time = 200;
    }
...
static bool PM_CheckDodge( void )
{
    ...
    jump = BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->jumpMagnitude;
...
static void PM_JetPackMove( void )
{
    ...
    scale = PM_CmdScale( &pm->cmd, false );
...
static void PM_AirMove( void )
{
    ...
    PM_Accelerate( wishdir, wishspeed,
                   BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->airAcceleration );
...
//  wishvel[2] = 0;
    ...
    if( ( pml.groundTrace.surfaceFlags & SURF_SLICK ) || pm->ps->pm_flags & PMF_TIME_KNOCKBACK )
        accelerate = BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->airAcceleration;
    else
        accelerate = BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->acceleration;
...
static void PM_CheckLadder( void )
{
    ...
    if( !BG_ClassHasAbility( ( class_t )pm->ps->stats[ STAT_CLASS ], SCA_CANUSELADDERS ) )
    {
        pml.ladder = false;
        return;
    }
...
static void PM_GroundTraceMissed( void )
{
    ...
    if( BG_ClassHasAbility( ( class_t )pm->ps->stats[ STAT_CLASS ], SCA_TAKESFALLDAMAGE ) )
    {
...
static void PM_GroundTrace( void )
{
    ...
    if( BG_ClassHasAbility( ( class_t )pm->ps->stats[ STAT_CLASS ], SCA_WALLCLIMBER ) )
    {
...
static void PM_SetViewheight( void )
{
    pm->ps->viewheight = ( pm->ps->pm_flags & PMF_DUCKED )
                         ? BG_ClassConfig( ( class_t )pm->ps->stats[ STAT_CLASS ] )->crouchViewheight
                         : BG_ClassConfig( ( class_t )pm->ps->stats[ STAT_CLASS ] )->viewheight;
}
...
static void PM_CheckDuck( void )
{
    ...
    BG_ClassBoundingBox( ( class_t )pm->ps->stats[ STAT_CLASS ], PCmins, PCmaxs, PCcmaxs, NULL, NULL );
...
static void PM_Footsteps( void )
{
    ...
    //
    if( BG_ClassHasAbility( ( class_t )pm->ps->stats[ STAT_CLASS ], SCA_WALLCLIMBER ) && ( pml.groundPlane ) )
    {
    ...
    bobmove *= BG_Class( ( class_t )pm->ps->stats[ STAT_CLASS ] )->bobCycle;
...
static void PM_Weapon( void )
{
    ...
    if( !pm->ps->ammo && !pm->ps->clips && !BG_Weapon( ( weapon_t )pm->ps->weapon )->infiniteAmmo )
    {
        if( attack1 ||
                ( BG_Weapon( ( weapon_t )pm->ps->weapon )->hasAltMode && attack2 ) ||
                ( BG_Weapon( ( weapon_t )pm->ps->weapon )->hasThirdMode && attack3 ) )
        {
    ...
    {
        pm->ps->clips--;
        pm->ps->ammo = BG_Weapon( ( weapon_t )pm->ps->weapon )->maxAmmo;
        
        if( BG_Weapon( ( weapon_t )pm->ps->weapon )->usesEnergy &&
    ...
    if( !BG_Weapon( ( weapon_t )pm->ps->weapon )->infiniteAmmo &&
            ...
            pm->ps->clips > 0 )
    {
        ...
        pm->ps->weaponTime += BG_Weapon( ( weapon_t )pm->ps->weapon )->reloadTime;
        return;
    }
    ...
    if( attack3 )
    {
        if( BG_Weapon( ( weapon_t )pm->ps->weapon )->hasThirdMode )
        {
            ...
            addTime = BG_Weapon( ( weapon_t )pm->ps->weapon )->repeatRate3;
        }
        else
        {
            pm->ps->weaponTime = 0;
    ...
    {
        if( BG_Weapon( ( weapon_t )pm->ps->weapon )->hasAltMode )
        {
            pm->ps->generic1 = WPM_SECONDARY;
            PM_AddEvent( EV_FIRE_WEAPON2 );
            addTime = BG_Weapon( ( weapon_t )pm->ps->weapon )->repeatRate2;
        }
        else
        {
            pm->ps->weaponTime = 0;
    ...
    else if( attack1 )
    {
        ...
        addTime = BG_Weapon( ( weapon_t )pm->ps->weapon )->repeatRate1;
    }
...
void PmoveSingle( pmove_t* pmove )
{
    ...
    if( !( pm->ps->pm_flags & PMF_RESPAWNED ) && pm->ps->pm_type != PM_INTERMISSION &&
            ( pm->cmd.buttons & BUTTON_ATTACK ) &&
            ( ( pm->ps->ammo > 0 || pm->ps->clips > 0 ) || BG_Weapon( ( weapon_t )pm->ps->weapon )->infiniteAmmo ) )
