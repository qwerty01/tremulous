...
void RE_TakeVideoFrame( int width, int height, unsigned char *captureBuffer, unsigned char *encodeBuffer, bool motionJpeg )
{
	if( !tr.registered ) {
		return;
	}

	videoFrameCommand_t	* cmd = (videoFrameCommand_t *)R_GetCommandBuffer( sizeof( *cmd ) );
	if( !cmd ) {
		return;
	}
