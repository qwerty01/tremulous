...
void updateMVP(VkBool32 isPortal, VkBool32 is2D, const float mvMat4x4[16])
{
	if (isPortal)
    {
        ...
        // NOTE: backEnd.orientation.modelMatrix incorporates s_flipMatrix,
        // so it should be taken into account when computing clipping plane too.

		push_constants[16] = backEnd.orientation.modelMatrix[0];
		push_constants[17] = backEnd.orientation.modelMatrix[4];
		push_constants[18] = backEnd.orientation.modelMatrix[8];
		push_constants[19] = backEnd.orientation.modelMatrix[12];

		push_constants[20] = backEnd.orientation.modelMatrix[1];
		push_constants[21] = backEnd.orientation.modelMatrix[5];
		push_constants[22] = backEnd.orientation.modelMatrix[9];
		push_constants[23] = backEnd.orientation.modelMatrix[13];

		push_constants[24] = backEnd.orientation.modelMatrix[2];
		push_constants[25] = backEnd.orientation.modelMatrix[6];
		push_constants[26] = backEnd.orientation.modelMatrix[10];
		push_constants[27] = backEnd.orientation.modelMatrix[14];
	
        // Clipping plane in eye coordinates.
		struct rplane_s eye_plane;

        R_TransformPlane(backEnd.viewParms.orientation.axis, backEnd.viewParms.orientation.origin, &eye_plane);
...
//    updateMVP(backEnd.viewParms.isPortal, backEnd.projection2D, 
//            backEnd.orientation.modelMatrix);
