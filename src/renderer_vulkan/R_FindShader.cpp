...
void ScanAndLoadShaderFiles( void )
{
	...
	union {
		char *c[MAX_SHADER_FILES];
		void *v[MAX_SHADER_FILES];
	} buffers;
	...
	for ( i = 0; i < numShaderFiles; i++ )
	{
		...
		long summand = ri.FS_ReadFile( filename, &buffers.v[i] );
	...
	// build single large buffer
    BuildSingleLargeBuffer(buffers.c, numShaderFiles, sum);
...
void R_RemapShader(const char *shaderName, const char *newShaderName, const char *timeOffset)
{
    ...
    //R_FindShaderByName( newShaderName );
    {
        ...
        if (sh2 == tr.defaultShader)
        {
            ...
            pSh = R_FindShader( newShaderName, 0, true );
