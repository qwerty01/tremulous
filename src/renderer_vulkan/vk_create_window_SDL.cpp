...
static void VKimp_DetectAvailableModes(void)
{
	...
	int numModes = 0;
	SDL_Rect* modes = (SDL_Rect *)SDL_calloc(numSDLModes, sizeof( SDL_Rect ));
	if ( !modes )
	{
        ////////////////////////////////////
...
static int VKimp_SetMode(int mode, bool fullscreen)
{
	...
	{
        R_GetModeInfo( (unsigned int *)&glConfig.vidWidth, (unsigned int *)&glConfig.vidHeight, 
...
void vk_createWindow(void)
{
    ...
    glConfig.textureEnvAddAvailable = qfalse; // not used
