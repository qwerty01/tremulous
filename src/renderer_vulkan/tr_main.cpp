...
typedef struct {
	...
	float		viewOrigin[3];	// viewParms->orientation.origin in local coordinates
} orientationr_t;

=================
*/
void R_RotateForEntity(const trRefEntity_t* const ent, const viewParms_t* const viewParms, orientationr_t* const orientation)
{
    ...
    memcpy(orientation->origin, ent->e.origin, 12);
    memcpy(orientation->axis, ent->e.axis, 36);
	...
	MatrixMultiply4x4_SSE( glMatrix, viewParms->world.modelMatrix, orientation->modelMatrix );

	// calculate the viewer origin in the model's space
	// needed for fog, specular, and environment mapping
    
    R_WorldPointToLocal(viewParms->orientation.origin, orientation, orientation->viewOrigin);
    ...
    if ( ent->e.nonNormalizedAxes )
    {
         if ( ent->e.nonNormalizedAxes )
        {
            ...
            orientation->viewOrigin[0] *= axisLength;
            orientation->viewOrigin[1] *= axisLength;
            orientation->viewOrigin[2] *= axisLength;
        }
    }
/*  
    vec3_t delta;
    ...
    R_WorldVectorToLocal(delta, orientation->axis, orientation->viewOrigin);
...
*/
    // printMat1x3f("viewOrigin", orientation->viewOrigin);
    // printMat4x4f("modelMatrix", orientation->modelMatrix);
...
static void R_RotateForViewer ( viewParms_t * const pViewParams, orientationr_t * const pEntityPose) 
{
    ...
    // for current entity
    // orientationr_t * const pEntityPose = &tr.orientation;
    ...
    pEntityPose->viewOrigin[0] = o0 = pViewParams->orientation.origin[0];
    pEntityPose->viewOrigin[1] = o1 = pViewParams->orientation.origin[1];
    pEntityPose->viewOrigin[2] = o2 = pViewParams->orientation.origin[2];

    
    float viewerMatrix[16] QALIGN(16);
	viewerMatrix[0] = pViewParams->orientation.axis[0][0];
	viewerMatrix[1] = pViewParams->orientation.axis[1][0];
	viewerMatrix[2] = pViewParams->orientation.axis[2][0];
	viewerMatrix[3] = 0;

	viewerMatrix[4] = pViewParams->orientation.axis[0][1];
	viewerMatrix[5] = pViewParams->orientation.axis[1][1];
	viewerMatrix[6] = pViewParams->orientation.axis[2][1];
	viewerMatrix[7] = 0;

	viewerMatrix[8] = pViewParams->orientation.axis[0][2];
	viewerMatrix[9] = pViewParams->orientation.axis[1][2];
	viewerMatrix[10] = pViewParams->orientation.axis[2][2];
	viewerMatrix[11] = 0;
...
static void R_SetupFrustum (viewParms_t * const pViewParams)
{
	
    {
        ...
        VectorScale( pViewParams->orientation.axis[0], xs, temp1 );
        VectorScale( pViewParams->orientation.axis[1], xc, temp2);

        VectorAdd(temp1, temp2, pViewParams->frustum[0].normal);
		pViewParams->frustum[0].dist = DotProduct (pViewParams->orientation.origin, pViewParams->frustum[0].normal);
        pViewParams->frustum[0].type = PLANE_NON_AXIAL;

        VectorSubtract(temp1, temp2, pViewParams->frustum[1].normal);
		pViewParams->frustum[1].dist = DotProduct (pViewParams->orientation.origin, pViewParams->frustum[1].normal);
   ...
   
    {
        ...
        VectorScale( pViewParams->orientation.axis[0], xs, temp1);
        VectorScale( pViewParams->orientation.axis[2], xc, temp2);

        VectorAdd(temp1, temp2, pViewParams->frustum[2].normal);
		pViewParams->frustum[2].dist = DotProduct (pViewParams->orientation.origin, pViewParams->frustum[2].normal);
        pViewParams->frustum[2].type = PLANE_NON_AXIAL;

        VectorSubtract(temp1, temp2, pViewParams->frustum[3].normal);
		pViewParams->frustum[3].dist = DotProduct (pViewParams->orientation.origin, pViewParams->frustum[3].normal);
	...
	for (i=0; i < 4; i++)
    {
		...
		// pViewParams->frustum[i].dist = DotProduct (pViewParams->orientation.origin, pViewParams->frustum[i].normal);
							 ...
							 vec3_t pvsOrigin, bool *mirror )
{
	...
	if ( entityNum != REFENTITYNUM_WORLD ) {
		...
		R_LocalNormalToWorld( originalPlane.normal, &tr.orientation, plane.normal );
...
static bool SurfIsOffscreen( const drawSurf_t *drawSurf, vec4_t clipDest[128] )
{
	...
	R_RotateForViewer(&tr.viewParms, &tr.orientation);
	...
	for ( i = 0; i < tess.numVertexes; i++ )
	{
        ...
        TransformModelToClip_SSE(tess.xyz[i], tr.orientation.modelMatrix, tr.viewParms.projectionMatrix, clip);
		for ( j = 0; j < 3; j++ )
		{
			if ( clip[j] >= clip[3] )
			{
...
void R_AddEntitySurfaces (viewParms_t * const pViewParam)
{
		  ...
		  tr.currentEntityNum++ )
    {
		...
		case RT_MODEL:
			// we must set up parts of tr.orientationfor model culling
			R_RotateForEntity( ent, pViewParam, &tr.orientation);
...
static void R_SetupProjection( viewParms_t * const pViewParams)
{
    ...
    else
    {
        ...
        o[0] = pViewParams->orientation.origin[0];
        o[1] = pViewParams->orientation.origin[1];
        o[2] = pViewParams->orientation.origin[2];
