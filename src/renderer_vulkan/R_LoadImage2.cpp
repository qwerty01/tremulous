...
static void LoadJPG( const char* name, unsigned char** pic, uint32_t* width, uint32_t* height)
{
    ...
    int len = ri.FS_ReadFile(name, &fbuffer.v);
    if (!fbuffer.v) {
        return;
    }
  
    int components;
    *pic = stbi_load_from_memory((unsigned char*)fbuffer.c, len, (int*)width, (int*)height, &components, STBI_rgb_alpha);
    if (*pic == NULL) {
