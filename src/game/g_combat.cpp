...
float G_RewardAttackers( gentity_t* self )
{
    ...
    {
        value = BG_Buildable( ( buildable_t )self->s.modelindex )->value;
        
        // only give partial credits for a buildable not yet completed
        if( !self->spawned )
        {
            value *= ( float )( level.time - self->buildTime ) /
                     BG_Buildable( ( buildable_t )self->s.modelindex )->buildTime;
        }
        
        team = self->buildableTeam;
        maxHealth = BG_Buildable( ( buildable_t )self->s.modelindex )->health;
    }
    else
        return totalDamage;
...
============
*/
static float GetRegionDamageModifier( gentity_t* targ, int _class, int piece )
{
...
============
*/
static float GetNonLocDamageModifier( gentity_t* targ, int _class )
{
...
void G_InitDamageLocations( void )
{
    ...
    {
        modelName = BG_ClassConfig( ( class_t )i )->modelName;
    ...
    for( i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++ )
    {
        modelName = BG_Upgrade( ( upgrade_t )i )->name;
               ...
               vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
    ...
    if( inflictor->s.weapon != WP_NONE )
    {
        knockback = ( int )( ( float )knockback *
                             BG_Weapon( ( weapon_t )inflictor->s.weapon )->knockbackScale );
    }
    
    if( targ->client )
    {
        knockback = ( int )( ( float )knockback *
                             BG_Class( ( class_t )targ->client->ps.stats[ STAT_CLASS ] )->knockbackScale );
    }
...
void G_LogDestruction( gentity_t* self, gentity_t* actor, int mod )
{
        ...
        default:
            if( actor->client )
            {
                if( actor->client->pers.teamSelection ==
                        BG_Buildable( ( buildable_t )self->s.modelindex )->team )
                {
                    fate = BF_TEAMKILL;
                }
                else
    ...
    G_LogPrintf( S_COLOR_YELLOW "Deconstruct: %d %d %s %s: %s %s by %s\n",
                 ...
                 ( int )( self - g_entities ),
                 BG_Buildable( ( buildable_t )self->s.modelindex )->name,
                 modNames[ mod ],
                 BG_Buildable( ( buildable_t )self->s.modelindex )->humanName,
    ...
    if( actor->client && actor->client->pers.teamSelection ==
            BG_Buildable( ( buildable_t )self->s.modelindex )->team )
    {
        G_TeamCommand( ( team_t )actor->client->ps.stats[ STAT_TEAM ],
