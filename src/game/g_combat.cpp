...
const char *modNames[] = {"MOD_UNKNOWN", "MOD_SHOTGUN", "MOD_BLASTER", "MOD_PAINSAW", "MOD_MACHINEGUN", "MOD_CHAINGUN",
    "MOD_PRIFLE", "MOD_MDRIVER", "MOD_LASGUN", "MOD_LCANNON", "MOD_LCANNON_SPLASH", "MOD_FLAMER", "MOD_FLAMER_SPLASH",
    "MOD_GRENADE", "MOD_WATER", "MOD_SLIME", "MOD_LAVA", "MOD_CRUSH", "MOD_TELEFRAG", "MOD_FALLING", "MOD_SUICIDE",
    "MOD_TARGET_LASER", "MOD_TRIGGER_HURT",

    "MOD_ABUILDER_CLAW", "MOD_LEVEL0_BITE", "MOD_LEVEL1_CLAW", "MOD_LEVEL1_PCLOUD", "MOD_LEVEL3_CLAW",
    "MOD_LEVEL3_POUNCE", "MOD_LEVEL3_BOUNCEBALL", "MOD_LEVEL2_CLAW", "MOD_LEVEL2_ZAP", "MOD_LEVEL4_CLAW",
    "MOD_LEVEL4_TRAMPLE", "MOD_LEVEL4_CRUSH",

    "MOD_SLOWBLOB", "MOD_POISON", "MOD_SWARM",

    "MOD_HSPAWN", "MOD_TESLAGEN", "MOD_MGTURRET", "MOD_REACTOR",

    "MOD_ASPAWN", "MOD_ATUBE", "MOD_OVERMIND", "MOD_DECONSTRUCT", "MOD_REPLACE", "MOD_NOCREEP"};
...
float G_RewardAttackers(gentity_t *self)
{
    ...
    else if (self->s.eType == ET_BUILDABLE)
    {
        ...
        if (!self->spawned)
        {
            value *= (float)(level.time - self->buildTime) / BG_Buildable(self->s.modelindex)->buildTime;
        }
    ...
    for (i = 0; i < level.maxclients; i++)
    {
        ...
        if (player->client->pers.teamSelection != team)
        {
            ...
            {
                G_AddCreditToClient(player->client, stageValue, true);
    ...
    if (alienCredits)
    {
        trap_Cvar_Set("g_alienCredits", va("%d", g_alienCredits.integer + alienCredits));
        trap_Cvar_Update(&g_alienCredits);
    }
    if (humanCredits)
    {
        trap_Cvar_Set("g_humanCredits", va("%d", g_humanCredits.integer + humanCredits));
...
============
*/
static float GetRegionDamageModifier(gentity_t *targ, int klass, int piece)
{
    ...
    overlap = &g_damageRegions[klass][piece];
    ...
    if (g_debugDamage.integer > 2)
        G_Printf(
            ...
            ".   modifier = %f\n",
            overlap->minAngle, overlap->maxAngle, overlap->minHeight, overlap->maxHeight, overlap->name,
            overlap->modifier);
    ...
    {
        if (!BG_InventoryContainsUpgrade(j, targ->client->ps.stats) || !g_numArmourRegions[j])
            continue;
        ...
        for (i = 0; i < g_numArmourRegions[j]; i++)
        {
            ...
            heightSpan = MIN(overlap->maxHeight, regions[i].maxHeight) - MAX(overlap->minHeight, regions[i].minHeight);
            if (heightSpan < 0.0f)
                heightSpan = 0.0f;
            if (heightSpan > 1.0f)
                heightSpan = 1.0f;
            ...
            if (g_debugDamage.integer > 2)
                G_Printf(
                    ".   armourRegion = [%d %d %f %f] (%s)\n"
                    ".   .   modifier = %f\n"
                    ".   .   angleSpan = %f\n"
                    ".   .   heightSpan = %f\n",
                    regions[i].minAngle, regions[i].maxAngle, regions[i].minHeight, regions[i].maxHeight,
                    regions[i].name, regions[i].modifier, angleSpan, heightSpan);
...
============
*/
static float GetNonLocDamageModifier(gentity_t *targ, int klass)
{
    ...
    crouch = targ->client->ps.pm_flags & PMF_DUCKED;
    for (i = 0; i < g_numDamageRegions[klass]; i++)
    {
        damageRegion_t *region;

        region = &g_damageRegions[klass][i];

        if (region->crouch != crouch)
            continue;

        modifier += GetRegionDamageModifier(targ, klass, i);
...
============
*/
static float GetPointDamageModifier(gentity_t *targ, damageRegion_t *regions, int len, float angle, float height)
{
    float modifier = 1.0f;
    int i;
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if ((regions[i].minAngle <= regions[i].maxAngle &&
            ...
            (regions[i].minAngle > regions[i].maxAngle && angle > regions[i].maxAngle && angle < regions[i].minAngle))
            continue;
...
============
*/
static float G_CalcDamageModifier(vec3_t point, gentity_t *targ, gentity_t *attacker, int klass, int dflags)
{
    ...
    if (dflags & DAMAGE_NO_LOCDAMAGE)
        return GetNonLocDamageModifier(targ, klass);
    ...
    modifier = GetPointDamageModifier(targ, g_damageRegions[klass], g_numDamageRegions[klass], hitRotation, hitRatio);

    for (i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++)
    {
        if (BG_InventoryContainsUpgrade(i, targ->client->ps.stats))
        {
            modifier *= GetPointDamageModifier(targ, g_armourRegions[i], g_numArmourRegions[i], hitRotation, hitRatio);
        }
    }
...
void G_InitDamageLocations(void)
{
    ...
    for (i = PCL_NONE + 1; i < PCL_NUM_CLASSES; i++)
    {
        ...
        {
            G_Printf(S_COLOR_RED "file too large: %s is %i, max allowed is %i", filename, len, MAX_DAMAGE_REGION_TEXT);
    ...
    int dflags, int mod)
{
    ...
    if (targ->client)
    {
        knockback = (int)((float)knockback * BG_Class(targ->client->ps.stats[STAT_CLASS])->knockbackScale);
    }
    ...
    if ((mod == MOD_LEVEL4_TRAMPLE || mod == MOD_LEVEL3_POUNCE || mod == MOD_LEVEL4_CRUSH) &&
    ...
    if (!(dflags & DAMAGE_NO_PROTECTION))
    {
        ...
        if (targ != attacker && OnSameTeam(targ, attacker))
        {
            ...
            if (g_dretchPunt.integer && targ->client->ps.stats[STAT_CLASS] == PCL_ALIEN_LEVEL0)
            {
                vec3_t dir, push;
        ...
        if (targ->s.eType == ET_BUILDABLE && attacker->client && mod != MOD_DECONSTRUCT && mod != MOD_SUICIDE &&
            mod != MOD_REPLACE && mod != MOD_NOCREEP)
        {
            if (targ->buildableTeam == attacker->client->pers.teamSelection && !g_friendlyBuildableFire.integer)
            {
                return;
            }

            // base is under attack warning if DCC'd
            if (targ->buildableTeam == TEAM_HUMANS && G_FindDCC(targ) && level.time > level.humanBaseAttackTimer)
            {
    ...
    if (client)
    {
        ...
        take = (int)(take * G_CalcDamageModifier(point, targ, attacker, client->ps.stats[STAT_CLASS], dflags) + 0.5f);

        // if boosted poison every attack
        if (attacker->client && attacker->client->ps.stats[STAT_STATE] & SS_BOOSTED)
        {
            if (targ->client->ps.stats[STAT_TEAM] == TEAM_HUMANS && mod != MOD_LEVEL2_ZAP && mod != MOD_POISON &&
                mod != MOD_LEVEL1_PCLOUD && mod != MOD_HSPAWN && mod != MOD_ASPAWN &&
...
void G_LogDestruction(gentity_t *self, gentity_t *actor, int mod)
{
    ...
    switch (mod)
    {
        ...
        default:
            if (actor->client)
            {
                if (actor->client->pers.teamSelection == BG_Buildable(self->s.modelindex)->team)
                {
                    fate = BF_TEAMKILL;
                }
                else
    ...
    G_LogPrintf(S_COLOR_YELLOW "Deconstruct: %d %d %s %s: %s %s by %s\n", (int)(actor - g_entities),
        (int)(self - g_entities), BG_Buildable(self->s.modelindex)->name, modNames[mod],
        BG_Buildable(self->s.modelindex)->humanName, mod == MOD_DECONSTRUCT ? "deconstructed" : "destroyed",
    ...
    if (mod == MOD_NOCREEP && actor->client && actor->client->pers.teamSelection == TEAM_HUMANS)
        return;

    if (actor->client && actor->client->pers.teamSelection == BG_Buildable(self->s.modelindex)->team)
    {
        G_TeamCommand(actor->client->ps.stats[STAT_TEAM],
            va("print \"%s ^3%s^7 by %s\n\"", BG_Buildable(self->s.modelindex)->humanName,
                mod == MOD_DECONSTRUCT ? "DECONSTRUCTED" : "DESTROYED", actor->client->pers.netname));
    }
}
