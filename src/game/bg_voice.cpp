    ...
    for (i = 0; i < numFiles; i++, filePtr += fileLen + 1)
    {
        ...
        if (fileLen > MAX_VOICE_NAME_LEN + 8)
        {
            Com_Printf(S_COLOR_YELLOW
                "WARNING: MAX_VOICE_NAME_LEN is %d. "
...
static bool BG_VoiceParseTrack(int handle, voiceTrack_t *voiceTrack)
{
    ...
    while (foundToken)
    {
        ...
        else if (!Q_stricmp(token.string, "team"))
        {
            ...
            if (!found)
            {
                BG_VoiceParseError(handle, "BG_VoiceParseTrack(): missing \"team\" value");
            }
            continue;
        }
        else if (!Q_stricmp(token.string, "class"))
        {
            ...
            if (!found)
            {
                BG_VoiceParseError(handle, "BG_VoiceParseTrack(): missing \"class\" value");
            }
            continue;
        }
        else if (!Q_stricmp(token.string, "weapon"))
        {
            ...
            if (!found)
            {
                BG_VoiceParseError(handle, "BG_VoiceParseTrack(): missing \"weapon\" value");
            }
            continue;
        }
        else if (!Q_stricmp(token.string, "text"))
        {
            ...
            voiceTrack->text = static_cast<char *>(BG_Alloc(strlen(token.string) + 1));
...
static voiceTrack_t *BG_VoiceParseCommand(int handle)
{
    ...
    while (trap_Parse_ReadToken(handle, &token))
    {
        ...
        if (top == NULL)
        {
            voiceTracks = static_cast<voiceTrack_t *>(BG_Alloc(sizeof(voiceTrack_t)));
            top = voiceTracks;
        }
        else
        {
            voiceTracks->next = static_cast<voiceTrack_t *>(BG_Alloc(sizeof(voiceCmd_t)));
...
static voiceCmd_t *BG_VoiceParse(char *name)
{
    ...
    while (trap_Parse_ReadToken(handle, &token))
    {
        ...
        if (top == NULL)
        {
            voiceCmds = static_cast<voiceCmd_t *>(BG_Alloc(sizeof(voiceCmd_t)));
            top = voiceCmds;
        }
        else
        {
            voiceCmds->next = static_cast<voiceCmd_t *>(BG_Alloc(sizeof(voiceCmd_t)));
...
============
*/
voiceTrack_t *BG_VoiceTrackFind(
    voiceTrack_t *head, team_t team, class_t klass, weapon_t weapon, int enthusiasm, int *trackNum)
{
    voiceTrack_t *vt = head;
    int highestMatch = 0;
    int matchCount = 0;
    ...
    while (vt)
    {
        if ((vt->team >= 0 && !(vt->team & (1 << team))) || (vt->klass >= 0 && !(vt->klass & (1 << klass))) ||
            (vt->weapon >= 0 && !(vt->weapon & (1 << weapon))) || vt->enthusiasm > enthusiasm)
        {
            vt = vt->next;
            continue;
        }
    ...
    while (vt)
    {
        j++;
        if ((vt->team >= 0 && !(vt->team & (1 << team))) || (vt->klass >= 0 && !(vt->klass & (1 << klass))) ||
            (vt->weapon >= 0 && !(vt->weapon & (1 << weapon))) || vt->enthusiasm != highestMatch)
        {
            vt = vt->next;
            continue;
        }
