...
static void G_Bounce( gentity_t* ent, trace_t* trace )
{
    ...
    {
        minNormal = BG_Buildable( ( buildable_t )ent->s.modelindex )->minNormal;
        invert = BG_Buildable( ( buildable_t )ent->s.modelindex )->invertNormal;
    }
    else
        minNormal = 0.707f;
...
void G_Physics( gentity_t* ent, int msec )
{
    ...
    if( ent->s.groundEntityNum == ENTITYNUM_NONE )
    {
        if( ent->s.eType == ET_BUILDABLE )
        {
            if( ent->s.pos.trType != BG_Buildable( ( buildable_t )ent->s.modelindex )->traj )
            {
                ent->s.pos.trType = BG_Buildable( ( buildable_t )ent->s.modelindex )->traj;
                ent->s.pos.trTime = level.time;
            }
        }
