            ...
            if( ent->s.modelindex == BA_H_REACTOR && distance <= REACTOR_BASESIZE )
            {
                ...
                if( self->s.modelindex != BA_NONE )
                {
                    ...
                    for( j = MAX_CLIENTS, ent2 = g_entities + j; j < level.num_entities; j++, ent2++ )
                    {
                        ...
                        {
                            buildPoints -= BG_Buildable( ( buildable_t )ent2->s.modelindex )->buildPoints;
                        }
                    }
                    
                    buildPoints -= level.humanBuildPointQueue;
                    
                    buildPoints -= BG_Buildable( ( buildable_t )self->s.modelindex )->buildPoints;
...
int G_GetMarkedBuildPoints( const vec3_t pos, team_t team )
{
    ...
    for( i = MAX_CLIENTS, ent = g_entities + i; i < level.num_entities; i++, ent++ )
    {
        ...
        if( ent->deconstruct )
            sum += BG_Buildable( ( buildable_t )ent->s.modelindex )->buildPoints;
    }
            ...
            self->s.time = -( level.time -
                              ( int )( ( float )CREEP_SCALEDOWN_TIME *
                                       ( 1.0f - ( ( float )( level.time - self->buildTime ) /
                                                  ( float )BG_Buildable( ( buildable_t )self->s.modelindex )->buildTime ) ) ) );
    }
...
void AGeneric_Think( gentity_t* self )
{
    ...
    self->nextthink = level.time + BG_Buildable( ( buildable_t )self->s.modelindex )->nextthink;
...
void ATrapper_FireOnEnemy( gentity_t* self, int firespeed, float range )
{
    ...
    float     distanceToTarget = BG_Buildable( ( buildable_t )self->s.modelindex )->turretRange;
    int       lowMsec = 0;
    int       highMsec = ( int )( (
                                      ( ( distanceToTarget * LOCKBLOB_SPEED ) +
                                        ( distanceToTarget * BG_Class( ( class_t )enemy->client->ps.stats[ STAT_CLASS ] )->speed ) ) /
...
{
    int range =     BG_Buildable( ( buildable_t )self->s.modelindex )->turretRange;
    int firespeed = BG_Buildable( ( buildable_t )self->s.modelindex )->turretFireSpeed;
...
static void G_IdlePowerState( gentity_t* self )
{
    if( self->powered )
    {
        if( self->s.torsoAnim == BANIM_IDLE3 )
            G_SetIdleBuildableAnim( self, ( buildableAnimNumber_t )BG_Buildable( ( buildable_t )self->s.modelindex )->idleAnim );
    }
    else
    {
        if( self->s.torsoAnim != BANIM_IDLE3 )
...
================
*/
bool HMGTurret_CheckTarget( gentity_t* self, gentity_t* target,
                            bool los_check )
{
    trace_t   tr;
    vec3_t    dir, end;
...
void HMGTurret_Think( gentity_t* self )
{
    self->nextthink = level.time +
                      BG_Buildable( ( buildable_t )self->s.modelindex )->nextthink;
    ...
    FireWeapon( self );
    self->s.eFlags |= EF_FIRING;
    self->timestamp = level.time + BG_Buildable( ( buildable_t )self->s.modelindex )->turretFireSpeed;
...
static int G_QueueValue( gentity_t* self )
{
    ...
    queuePoints = ( int )( queueFraction * ( double ) BG_Buildable( ( buildable_t )self->s.modelindex )->buildPoints );
    return queuePoints;
}
...
void G_BuildableTouchTriggers( gentity_t* ent )
{
    ...
    BG_BuildableBoundingBox( ( buildable_t )ent->s.modelindex, bmins, bmaxs );
...
{
    int maxHealth = BG_Buildable( ( buildable_t )ent->s.modelindex )->health;
    int regenRate = BG_Buildable( ( buildable_t )ent->s.modelindex )->regenRate;
    int buildTime = BG_Buildable( ( buildable_t )ent->s.modelindex )->buildTime;
...
===============
*/
static bool G_BuildablesIntersect( buildable_t a, vec3_t originA,
...
static int G_CompareBuildablesForRemoval( const void* a, const void* b )
{
    ...
    aMatches = G_BuildablesIntersect( cmpBuildable, cmpOrigin,
                                      ( buildable_t )buildableA->s.modelindex, buildableA->r.currentOrigin );
    bMatches = G_BuildablesIntersect( cmpBuildable, cmpOrigin,
                                      ( buildable_t )buildableB->s.modelindex, buildableB->r.currentOrigin );
                             ...
                             char* nums, int nsize )
{
    ...
    for( i = 0; i < level.numBuildablesForRemoval; i++ )
    {
        ...
        bNum = BG_Buildable( ( buildable_t )ent->s.modelindex )->number;
    ...
    for( i = 0; i < BA_NUM_BUILDABLES; i++ )
    {
        if( removalCounts[ i ] )
        {
            ...
            Q_strcat( readable, rsize, va( "%s", BG_Buildable( ( buildable_t )i )->humanName ) );
        ...
        vec3_t          origin )
{
    ...
    if( !g_markDeconstruct.integer )
    {
        ...
        for( i = MAX_CLIENTS, ent = g_entities + i; i < level.num_entities; i++, ent++ )
        {
            ...
            if( G_BuildablesIntersect( buildable, origin, ( buildable_t )ent->s.modelindex, ent->r.currentOrigin ) )
                return IBE_NOROOM;
        }
    ...
    for( i = MAX_CLIENTS, ent = g_entities + i; i < level.num_entities; i++, ent++ )
    {
        ...
        collision = G_BuildablesIntersect( buildable, origin, ( buildable_t )ent->s.modelindex, ent->r.currentOrigin );
        ...
        if( ent->deconstruct )
        {
            ...
            if( collision || repeaterInRange )
            {
                ...
                if( ent->powered )
                    pointsYielded += BG_Buildable( ( buildable_t )ent->s.modelindex )->buildPoints;
                level.numBuildablesForRemoval++;
            }
            else if( BG_Buildable( ( buildable_t )ent->s.modelindex )->uniqueTest &&
            ...
            level.numBuildablesForRemoval > numRequired )
    {
        ...
        if( ent->powered )
            pointsUnYielded = BG_Buildable( ( buildable_t )ent->s.modelindex )->buildPoints;
                           ...
                           const vec3_t origin, const vec3_t normal, const vec3_t angles, int groundEntNum )
{
    ...
    built->buildableTeam = ( team_t )( built->s.modelindex2 = BG_Buildable( buildable )->team );
    ...
    {
        G_TeamCommand( ( team_t )builder->client->ps.stats[ STAT_TEAM ],
                       va( "print \"%s ^2built^7 by %s%s%s\n\"",
                           BG_Buildable( ( buildable_t )built->s.modelindex )->humanName,
        ...
        G_LogPrintf( "Construct: %d %d %s%s: %s" S_COLOR_WHITE " is building "
                     ...
                     BG_Buildable( ( buildable_t )built->s.modelindex )->name,
                     buildnums,
                     builder->client->pers.netname,
                     BG_Buildable( ( buildable_t )built->s.modelindex )->humanName,
...
bool G_BuildIfValid( gentity_t* ent, buildable_t buildable )
{
    ...
    dist = BG_Class( ( class_t )ent->client->ps.stats[ STAT_CLASS ] )->buildDist;
    ...
    for( i = MAX_CLIENTS; i < level.num_entities; i++ )
    {
        ...
        if( ent->s.eType == ET_BUILDABLE )
        {
            if( !bAllowed[ BA_NONE ] && !bAllowed[ ent->s.modelindex ] )
                continue;
            name = BG_Buildable( ( buildable_t )ent->s.modelindex )->name;
        }
        ...
        if( *layout == '\n' )
        {
            ...
            else
            {
                if( bAllowed[ BA_NONE ] || bAllowed[ buildable ] )
                    G_LayoutBuildItem( ( buildable_t )buildable, origin, angles, origin2, angles2 );
            }
        }
        layout++;
    }
...
void G_BuildLogSet( buildLog_t* log, gentity_t* ent )
{
    ...
    log->powerSource = ( buildable_t )( ent->parentNode ? ent->parentNode->s.modelindex : BA_NONE );
...
void G_BuildLogRevertThink( gentity_t* ent )
{
    ...
    G_LogPrintf( "revert: restore %d %s\n",
                 ( int )( built - g_entities ), BG_Buildable( ( buildable_t )built->s.modelindex )->name );
                    ...
                    if( VectorLengthSquared( dist ) <= 2.0f )
                    {
                        if( ent->s.eType == ET_BUILDABLE )
                            G_LogPrintf( "revert: remove %d %s\n",
                                         ( int )( ent - g_entities ), BG_Buildable( ( buildable_t )ent->s.modelindex )->name );
                        G_RemoveRangeMarkerFrom( ent );
                        G_FreeEntity( ent );
                        break;
                    }
...
void G_UpdateBuildableRangeMarkers( void )
{
    ...
    for( e = &g_entities[ MAX_CLIENTS ]; e < &g_entities[ level.num_entities ]; ++e )
    {
        ...
        bType = ( buildable_t )e->s.modelindex;
