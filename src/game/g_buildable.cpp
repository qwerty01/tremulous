...
================
*/
void G_SetIdleBuildableAnim(gentity_t *ent, buildableAnimNumber_t anim) { ent->s.torsoAnim = anim; }
...
int G_GetBuildPoints(const vec3_t pos, team_t team)
{
    ...
    else if (team == TEAM_HUMANS)
    {
        ...
        if (powerPoint && powerPoint->s.modelindex == BA_H_REPEATER && powerPoint->usesBuildPointZone &&
...
int G_GetMarkedBuildPoints(const vec3_t pos, team_t team)
{
    ...
    for (i = MAX_CLIENTS, ent = g_entities + i; i < level.num_entities; i++, ent++)
    {
        ...
        if (team == TEAM_HUMANS && ent->s.modelindex != BA_H_REACTOR && ent->s.modelindex != BA_H_REPEATER &&
...
bool G_FindCreep(gentity_t *self)
{
    ...
    if (self->client || self->parentNode == NULL || !self->parentNode->inuse || self->parentNode->health <= 0)
    {
        for (i = MAX_CLIENTS, ent = g_entities + i; i < level.num_entities; i++, ent++)
        {
            if (ent->s.eType != ET_BUILDABLE)
...
void AGeneric_CreepRecede(gentity_t *self)
{
    ...
    if (!(self->s.eFlags & EF_DEAD))
    {
        ...
        if (self->spawned)
            ...
            self->s.time = -(level.time - (int)((float)CREEP_SCALEDOWN_TIME *
...
void AGeneric_Blast(gentity_t *self)
{
    ...
    G_SelectiveRadiusDamage(self->s.pos.trBase, g_entities + self->killedBy, self->splashDamage, self->splashRadius,
        self, self->splashMethodOfDeath, TEAM_ALIENS);
...
{
    G_SetBuildableAnim(self, BANIM_DESTROY1, true);
...
void ASpawn_Think(gentity_t *self)
{
    ...
    if (self->spawned)
    {
        ...
        {
            if ((ent = G_CheckSpawnPoint(self->s.number, self->r.currentOrigin, self->s.origin2, BA_A_SPAWN, NULL)) !=
                NULL)
            {
...
void AOvermind_Think(gentity_t *self)
{
    ...
    {
        // do some damage
        if (G_SelectiveRadiusDamage(
                self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, MOD_OVERMIND, TEAM_ALIENS))
        {
            self->timestamp = level.time;
...
void ABarricade_Shrink(gentity_t *self, bool shrink)
{
    ...
    if (!shrink && (!self->shrunkTime || level.time < self->shrunkTime + BARRICADE_SHRINKTIMEOUT))
        return;
    ...
    else
    {
        ...
        trap_Trace(&tr, self->r.currentOrigin, self->r.mins, self->r.maxs, self->r.currentOrigin, self->s.number,
            MASK_PLAYERSOLID);
        ...
        if (self->spawned && self->health > 0 && anim != BANIM_CONSTRUCT1 && anim != BANIM_CONSTRUCT2)
        {
...
void ABarricade_Touch(gentity_t *self, gentity_t *other, trace_t *trace)
{
    ...
    min_z = self->r.currentOrigin[2] - 18 + (int)(self->r.maxs[2] * BARRICADE_SHRINKPROP);
    if (client_z < min_z)
        return;
    ABarricade_Shrink(self, true);
}
        ...
        for (i = 0; i < num; i++)
        {
            ...
            if (enemy->client && enemy->client->ps.stats[STAT_TEAM] == TEAM_HUMANS)
            {
                ...
                G_SelectiveRadiusDamage(
                    self->s.pos.trBase, self, ACIDTUBE_DAMAGE, ACIDTUBE_RANGE, self, MOD_ATUBE, TEAM_ALIENS);
...
static bool AHive_CheckTarget(gentity_t *self, gentity_t *enemy)
{
    ...
    VectorMA(self->s.pos.trBase, self->r.maxs[2], self->s.origin2, tip_origin);
    if (Distance(tip_origin, enemy->r.currentOrigin) > HIVE_SENSE_RANGE)
        return false;

    trap_Trace(&trace, tip_origin, NULL, NULL, enemy->s.pos.trBase, self->s.number, MASK_SHOT);
...
void AHive_Think(gentity_t *self)
{
    ...
    if (self->spawned && !self->active && self->powered)
    {
        ...
        vec3_t mins, maxs, range = {HIVE_SENSE_RANGE, HIVE_SENSE_RANGE, HIVE_SENSE_RANGE};
...
================
*/
static bool G_SuicideIfNoPower(gentity_t *self)
{
    if (self->buildableTeam != TEAM_HUMANS)
        return false;
    ...
    if (!self->powered)
    {
        ...
        {
            if (self->parentNode)
                G_Damage(self, NULL, g_entities + self->parentNode->killedBy, NULL, NULL, self->health, 0, MOD_NOCREEP);
            else
                G_Damage(self, NULL, NULL, NULL, NULL, self->health, 0, MOD_NOCREEP);
            return true;
        }
...
void HSpawn_Blast(gentity_t *self)
{
    ...
    G_RadiusDamage(self->s.pos.trBase, g_entities + self->killedBy, self->splashDamage, self->splashRadius, self,
...
void HSpawn_Think(gentity_t *self)
{
    ...
    // set parentNode
    self->powered = G_FindPower(self, false);
    ...
    if (self->spawned)
    {
        ...
        if (self->s.groundEntityNum != ENTITYNUM_NONE)
        {
            if ((ent = G_CheckSpawnPoint(self->s.number, self->r.currentOrigin, self->s.origin2, BA_H_SPAWN, NULL)) !=
                NULL)
            {
                // If the thing blocking the spawn is a buildable, kill it.
                // If it's part of the map, kill self.
...
void HRepeater_Use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
    ...
    if (other && other->client)
        G_GiveClientMaxAmmo(other, true);
}
...
void HReactor_Think(gentity_t *self)
{
    ...
    if (self->spawned && (self->health > 0))
    {
        ...
        if (fired)
        {
            self->timestamp = level.time;
            if (self->dcc)
                G_SelectiveRadiusDamage(self->s.pos.trBase, self, REACTOR_ATTACK_DCC_DAMAGE, REACTOR_ATTACK_DCC_RANGE,
                    self, MOD_REACTOR, TEAM_HUMANS);
            else
                G_SelectiveRadiusDamage(self->s.pos.trBase, self, REACTOR_ATTACK_DAMAGE, REACTOR_ATTACK_RANGE, self,
...
================
*/
void HMedistat_Die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod)
{
...
void HMedistat_Think(gentity_t *self)
{
    ...
    if (self->spawned)
    {
        ...
        for (i = 0; i < num; i++)
        {
            ...
            if (self->enemy == player && player->client && player->client->ps.stats[STAT_TEAM] == TEAM_HUMANS &&
                player->health < player->client->ps.stats[STAT_MAX_HEALTH] && PM_Alive(player->client->ps.pm_type))
            {
                occupied = true;
        ...
        {
            G_SetBuildableAnim(self, BANIM_CONSTRUCT2, true);
...
================
*/
bool HMGTurret_CheckTarget(gentity_t *self, gentity_t *target, bool los_check)
{
    trace_t tr;
    vec3_t dir, end;
...
bool HMGTurret_TrackEnemy(gentity_t *self)
{
    ...
    return (fabs(angularDiff[YAW]) - MGTURRET_ANGULARSPEED <= MGTURRET_ACCURACY_TO_FIRE) &&
           (fabs(angularDiff[PITCH]) - MGTURRET_ANGULARSPEED <= MGTURRET_ACCURACY_TO_FIRE);
}
...
static bool HMGTurret_State(gentity_t *self, int state)
{
    ...
    if (state == MGT_STATE_INACTIVE)
    {
        if (angle < MGTURRET_VERTICALCAP)
        {
            ...
            self->s.angles2[PITCH] = MIN(MGTURRET_VERTICALCAP, angle + self->speed);
            return true;
        }
        else
    ...
    else if (state == MGT_STATE_ACTIVE)
    {
        if (!self->enemy && angle > 0.0f)
        {
            ...
            self->s.angles2[PITCH] = MAX(0.0f, angle - MGTURRET_ANGULARSPEED * 0.5f);
        }
        else
...
void HMGTurret_Think(gentity_t *self)
{
    ...
    if (!self->powered)
    {
        ...
        if (self->spawned && HMGTurret_State(self, MGT_STATE_INACTIVE))
            return;
        ...
        case TEAM_HUMANS:
            ...
            if (powerEntity)
            {
                ...
                switch (powerEntity->s.modelindex)
                {
                    case BA_H_REACTOR:
                        nqt = G_NextQueueTime(
                            level.humanBuildPointQueue, g_humanBuildPoints.integer, g_humanBuildQueueTime.integer);
                        if (!level.humanBuildPointQueue || level.time + nqt < level.humanNextQueueTime)
                    ...
                    case BA_H_REPEATER:
                            ...
                            level.buildPointZones[powerEntity->buildPointZone].active)
                        {
                            ...
                            nqt = G_NextQueueTime(
                                zone->queuedBuildPoints, zone->totalBuildPoints, g_humanRepeaterBuildQueueTime.integer);

                            if (!zone->queuedBuildPoints || level.time + nqt < zone->nextQueueTime)
...
===============
*/
static bool G_BuildablesIntersect(buildable_t a, vec3_t originA, buildable_t b, vec3_t originB)
{
    vec3_t minsA, maxsA;
    vec3_t minsB, maxsB;
...
static int G_CompareBuildablesForRemoval(const void *a, const void *b)
{
    ...
    bMatches = G_BuildablesIntersect(cmpBuildable, cmpOrigin, buildableB->s.modelindex, buildableB->r.currentOrigin);
...
===============
*/
static itemBuildError_t G_SufficientBPAvailable(buildable_t buildable, vec3_t origin)
{
    int i;
    int numBuildables = 0;
    int numRequired = 0;
    ...
    for (i = MAX_CLIENTS, ent = g_entities + i; i < level.num_entities; i++, ent++)
    {
        ...
        if (buildable == BA_H_REPEATER && buildable == ent->s.modelindex &&
        ...
        if (team == TEAM_HUMANS && buildable != BA_H_REACTOR && buildable != BA_H_REPEATER &&
        ...
        if (team == TEAM_HUMANS && G_PowerEntityForPoint(origin) == ent && buildable != BA_H_REPEATER &&
            buildable != core)
            continue;
        ...
        if (ent->deconstruct)
        {
            ...
            else if (BG_Buildable(ent->s.modelindex)->uniqueTest && ent->s.modelindex == buildable)
            {
    ...
    while (changed && level.numBuildablesForRemoval > 1 && level.numBuildablesForRemoval > numRequired)
    {
        ...
        if (pointsYielded - pointsUnYielded >= buildPoints)
        {
            ...
            memmove(&level.markedBuildables[numRequired], &level.markedBuildables[numRequired + 1],
                (level.numBuildablesForRemoval - numRequired) * sizeof(gentity_t *));
            level.numBuildablesForRemoval--;
            changed = true;
        }
    }
...
================
*/
static void G_SetBuildableLinkState(bool link)
{
    int i;
    gentity_t *ent;
...
static void G_SetBuildableMarkedLinkState(bool link)
{
    int i;
    gentity_t *ent;
...
================
*/
static gentity_t *G_Build(gentity_t *builder, buildable_t buildable, const vec3_t origin, const vec3_t normal,
    const vec3_t angles, int groundEntNum)
{
    gentity_t *built;
    ...
    if (builder->client && g_cheats.integer)
    {
        ...
        built->buildTime = built->s.time = level.time - BG_Buildable(buildable)->buildTime;
    }
    ...
    else if ((built->powered = G_FindPower(built, false)))
        built->s.eFlags |= EF_B_POWERED;
    ...
    if (built->builtBy)
        G_SetBuildableAnim(built, BANIM_CONSTRUCT1, true);
    ...
    if (builder && builder->client)
    {
        G_TeamCommand(builder->client->ps.stats[STAT_TEAM],
            va("print \"%s ^2built^7 by %s%s%s\n\"", BG_Buildable(built->s.modelindex)->humanName,
                builder->client->pers.netname, (readable[0]) ? "^7, ^3replacing^7 " : "", readable));
        G_LogPrintf("Construct: %d %d %s%s: %s" S_COLOR_WHITE
                    " is building "
                    "%s%s%s\n",
            (int)(builder - g_entities), (int)(built - g_entities), BG_Buildable(built->s.modelindex)->name, buildnums,
            builder->client->pers.netname, BG_Buildable(built->s.modelindex)->humanName,
            readable[0] ? ", replacing " : "", readable);
    }
...
================
*/
static gentity_t *G_FinishSpawningBuildable(gentity_t *ent, bool force)
{
    trace_t tr;
    vec3_t normal, dest;
    gentity_t *built;
    ...
    built = G_Build(ent, buildable, ent->r.currentOrigin, normal, ent->r.currentAngles, ENTITYNUM_NONE);
...
{
    G_FinishSpawningBuildable(ent, false);
...
{
    char *lstrPipePtr;
    bool bAllowed[BA_NUM_BUILDABLES + NUM_TEAMS];
    char map[MAX_QPATH];
    ...
    if ((lstrPipePtr = strchr(lstr, '|')))
    {
        ...
        *lstrPipePtr = '\0';
        ...
        for (i = 0; bList[i] != BA_NONE; i++)
            bAllowed[bList[i]] = true;
        *lstrPipePtr = '|';
        lstr = lstrPipePtr + 1;
    }
    else
        bAllowed[BA_NONE] = true;  // allow all
    ...
    for (i = MAX_CLIENTS; i < level.num_entities; i++)
    {
        ...
        s = va("%s %f %f %f %f %f %f %f %f %f %f %f %f\n", name, ent->r.currentOrigin[0], ent->r.currentOrigin[1],
            ent->r.currentOrigin[2], ent->r.currentAngles[0], ent->r.currentAngles[1], ent->r.currentAngles[2],
            ent->s.origin2[0], ent->s.origin2[1], ent->s.origin2[2], ent->s.angles2[0], ent->s.angles2[1],
            ent->s.angles2[2]);
...
static void G_SpawnIntermissionViewOverride(const char *cn, vec3_t origin, vec3_t angles)
{
...
loadAnotherLayout:
    ...
    trap_Cvar_VariableStringBuffer("mapname", map, sizeof(map));
    len = trap_FS_FOpenFile(va("layouts/%s/%s.dat", map, lstr), &f, FS_READ);
    if (len < 0)
    {
        G_Printf("ERROR: layout %s could not be opened\n", lstr);
        return;
    }
    layoutHead = layout = static_cast<char *>(BG_Alloc(len + 1));
    ...
    while (*layout)
    {
        ...
        {
            i = 0;
            sscanf(line, "%s %f %f %f %f %f %f %f %f %f %f %f %f\n", buildName, &origin[0], &origin[1], &origin[2],
                &angles[0], &angles[1], &angles[2], &origin2[0], &origin2[1], &origin2[2], &angles2[0], &angles2[1],
                &angles2[2]);
            ...
            else if (!Q_stricmp(buildName, "ivo_human"))
            {
                ...
                G_Printf(S_COLOR_YELLOW
                    "WARNING: bad buildable name (%s) in layout."
                    " skipping\n",
                    buildName);
            else
            {
...
void G_BuildLogRevertThink(gentity_t *ent)
{
    ...
    built = G_FinishSpawningBuildable(ent, true);
        ...
        else
        {
            ...
            if (log->fate == BF_DESTROY || log->fate == BF_TEAMKILL)
            {
                ...
                else
                {
                    ...
                    {
                        level.humanBuildPointQueue = MAX(0, level.humanBuildPointQueue - value);
                    }
...
void G_UpdateBuildableRangeMarkers(void)
{
    ...
    bool maskingExtension = (trap_Cvar_VariableIntegerValue("sv_gppExtension") >= 1);
