...
static float PM_CmdScale(usercmd_t *cmd, bool zFlight)
{
    ...
    if (pm->ps->stats[STAT_TEAM] == TEAM_HUMANS && pm->ps->pm_type == PM_NORMAL)
    {
        ...
        {
            if (cmd->buttons & BUTTON_SPRINT && !(pm->ps->pm_flags & PMF_SPRINTHELD))
            {
                sprint = !sprint;
...
static bool PM_CheckPounce(void)
{
    ...
    if ((pm->ps->pm_flags & PMF_CHARGE) || pm->ps->stats[STAT_MISC] < LEVEL3_POUNCE_TIME_MIN ||
...
static bool PM_CheckJump(void)
{
    ...
    VectorMA(pm->ps->velocity, BG_Class(pm->ps->stats[STAT_CLASS])->jumpMagnitude, normal, pm->ps->velocity);
...
static bool PM_CheckDodge(void)
{
    ...
    if (pm->ps->pm_flags & (PMF_TIME_LAND | PMF_CHARGE) || pm->ps->groundEntityNum == ENTITYNUM_NONE ||
    ...
    if ((pm->ps->stats[STAT_STATE] & SS_SLOWLOCKED) || (pm->ps->stats[STAT_STATE] & SS_CREEPSLOWED) ||
    ...
    for (i = 0; i < 3; i++)
    {
        if ((velocity[i] < 0.0f && pm->ps->velocity[i] > velocity[i]) ||
            (velocity[i] > 0.0f && pm->ps->velocity[i] < velocity[i]))
...
{
    pm->ps->viewheight = (pm->ps->pm_flags & PMF_DUCKED) ? BG_ClassConfig(pm->ps->stats[STAT_CLASS])->crouchViewheight
...
static void PM_Weapon(void)
{
    ...
    int attack1 = pm->cmd.buttons & BUTTON_ATTACK;
    int attack2 = pm->cmd.buttons & BUTTON_ATTACK2;
    int attack3 = pm->cmd.buttons & BUTTON_USE_HOLDABLE;
            ...
            else if (!(pm->ps->stats[STAT_STATE] & SS_CHARGING))
            {
                if (pm->ps->stats[STAT_MISC] > LEVEL4_TRAMPLE_CHARGE_MIN)
                {
                    ...
                    pm->ps->stats[STAT_MISC] =
                        pm->ps->stats[STAT_MISC] * LEVEL4_TRAMPLE_DURATION / LEVEL4_TRAMPLE_CHARGE_MAX;
                    pm->ps->stats[STAT_STATE] |= SS_CHARGING;
    ...
    if ((pm->ps->weapon == WP_ALEVEL3 || pm->ps->weapon == WP_ALEVEL3_UPG) && (pm->cmd.buttons & BUTTON_ATTACK) &&
        (pm->ps->pm_flags & PMF_CHARGE))
        return;
    ...
    if (pm->ps->weaponstate == WEAPON_RELOADING)
    {
        ...
        if (BG_Weapon(pm->ps->weapon)->usesEnergy && BG_InventoryContainsUpgrade(UP_BATTPACK, pm->ps->stats))
    ...
    if (!BG_Weapon(pm->ps->weapon)->infiniteAmmo && (pm->ps->ammo <= 0 || (pm->ps->pm_flags & PMF_WEAPON_RELOAD)) &&
        pm->ps->clips > 0)
    {
        pm->ps->pm_flags &= ~PMF_WEAPON_RELOAD;
    ...
    if (!BG_Weapon(pm->ps->weapon)->infiniteAmmo || (pm->ps->weapon == WP_ALEVEL3_UPG && attack3))
    {
        // Special case for lcannon
        if (pm->ps->weapon == WP_LUCIFER_CANNON && attack1 && !attack2)
            pm->ps->ammo -= (pm->ps->stats[STAT_MISC] * LCANNON_CHARGE_AMMO + LCANNON_CHARGE_TIME_MAX - 1) /
