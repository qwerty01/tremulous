...
static void Svcmd_LayoutSave_f(void)
{
    ...
    {
        if (isalnum(*s) || *s == '-' || *s == '_' || ((*s == '|' || *s == ',') && !pipeEncountered))
        {
            str2[i++] = *s;
            str2[i] = '\0';
            if (*s == '|')
...
static void Svcmd_ListMapsWrapper(void) { Cmd_ListMaps_f(NULL); }
...
static void Svcmd_G_AdvanceMapRotation_f(void) { G_AdvanceMapRotation(0); }

struct svcmd {
    const char *cmd;
    bool dedicated;
    void (*function)(void);
} svcmds[] = {{"a", true, Svcmd_MessageWrapper}, {"admitDefeat", false, Svcmd_AdmitDefeat_f},
    {"advanceMapRotation", false, Svcmd_G_AdvanceMapRotation_f}, {"alienWin", false, Svcmd_TeamWin_f},
    {"chat", true, Svcmd_MessageWrapper}, {"cp", true, Svcmd_CenterPrint_f}, {"dumpuser", false, Svcmd_DumpUser_f},
    {"eject", false, Svcmd_EjectClient_f}, {"entityList", false, Svcmd_EntityList_f},
    {"evacuation", false, Svcmd_Evacuation_f}, {"forceTeam", false, Svcmd_ForceTeam_f},
    {"game_memory", false, BG_MemoryInfo}, {"humanWin", false, Svcmd_TeamWin_f},
    {"layoutLoad", false, Svcmd_LayoutLoad_f}, {"layoutSave", false, Svcmd_LayoutSave_f},
    {"listmaps", true, Svcmd_ListMapsWrapper}, {"loadcensors", false, G_LoadCensors}, {"m", true, Svcmd_MessageWrapper},
    {"mapRotation", false, Svcmd_MapRotation_f}, {"pr", false, Svcmd_Pr_f}, {"printqueue", false, Svcmd_PrintQueue_f},
    {"say", true, Svcmd_MessageWrapper}, {"say_team", true, Svcmd_TeamMessage_f}, {"status", false, Svcmd_Status_f},
    {"stopMapRotation", false, G_StopMapRotation}, {"suddendeath", false, Svcmd_SuddenDeath_f}};
...
bool ConsoleCommand(void)
{
    ...
    command = static_cast<svcmd *>(bsearch(cmd, svcmds, ARRAY_LEN(svcmds), sizeof(struct svcmd), cmdcmp));
