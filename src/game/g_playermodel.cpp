...
//-------------------------------------------------------------------------
//
static bool _is_playermodel_uniq(const char *model)
{
    int i;
...
void G_InitPlayerModel(void)
{
    ...
    numFiles =
        trap_FS_GetFilteredFiles("/models/players", "", "models*players*head_*.skin", fileList, sizeof(fileList));
    filePtr = fileList;
...
bool G_IsValidPlayerModel(const char *model) { return !_is_playermodel_uniq(model); }
...
void G_GetPlayerModelSkins(const char *modelname, char skins[][64], int maxskins, int *numskins)
{
    ...
    *numskins = 0;
    nFiles = trap_FS_GetFilteredFiles(
        "models/players", ".skin", va("models*players*%s*skin", modelname), fileList, sizeof(fileList));
    filePtr = fileList;
    for (i = 0; i < nFiles && i < maxskins; i++)
    {
        ...
        *end = '\0';
        ...
        else if (start[0] == 'n' && start[1] == 'o' && start[2] == 'n' && start[3] == 's' && start[4] == 'e' &&
                 start[5] == 'g' && start[6] == '_')
            start += 7;
