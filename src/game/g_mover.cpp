...
bool G_TryPushingEntity(gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove)
{
    ...
    if (check->s.eType == ET_BUILDABLE && check->s.groundEntityNum != pusher->s.number)
        return false;
...
bool G_MoverPush(gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle)
{
    ...
    *obstacle = NULL;
    ...
    if (pusher->r.currentAngles[0] || pusher->r.currentAngles[1] || pusher->r.currentAngles[2] || amove[0] ||
        amove[1] || amove[2])
    {
        float radius;
    ...
    for (e = 0; e < listedEntities; e++)
    {
        ...
        if (check->s.eType != ET_ITEM && check->s.eType != ET_BUILDABLE && check->s.eType != ET_CORPSE &&
...
void Use_BinaryMover(gentity_t *ent, gentity_t *other, gentity_t *activator)
{
    ...
    for (ent = master; ent; ent = ent->teamchain)
    {
        ...
        else if (ent->moverState == MOVER_POS2 && !(teamState == MOVER_1TO2 || other == master))
        {
        ...
        else if (ent->moverState == ROTATOR_POS2 && !(teamState == MOVER_1TO2 || other == master))
        {
            // if all the way up, just delay before coming down
            master->think = ReturnToPos1orApos1;
            master->nextthink = MAX(master->nextthink, level.time + ent->wait);
...
START_OPEN  the door to moves to its destination when spawned, and operate in reverse.  It is used to temporarily or
permanently close off an area when triggered (not useful for touch or takedamage doors). NOMONSTER monsters will not
trigger this door
...
When a button is touched, it moves some distance in the direction of its angle, triggers all of its targets, waits some
time, then returns to its original position where it can be triggered again.
...
Pendulums always swing north / south on unrotated models.  Add an angles field to the model to allow rotation in other
directions. Pendulum frequency is a physical constant based on the length of the beam and gravity. "model2"  .md3 model
to also draw "speed"   the number of degrees each way the pendulum swings, (30 default) "phase"   the 0.0 to 1.0 offset
in the cycle to start at "dmg"   damage to inflict when blocked (2 default) "color"   constantLight color "light"
