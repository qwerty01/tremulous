...
*/
void SP_target_position(gentity_t *self) { G_SetOrigin(self, self->r.currentOrigin); }
...
void SP_target_location(gentity_t *self)
{
    ...
    if (self->count)
    {
        ...
        message = va("%c%c%s" S_COLOR_WHITE, Q_COLOR_ESCAPE, self->count + '0', (const char *)self->message);
    }
    else
...
===============
*/
void SP_target_human_win(gentity_t *self) { self->use = target_human_win_use; }
