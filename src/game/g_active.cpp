...
static void ClientShove(gentity_t *ent, gentity_t *victim)
{
    ...
    if (!OnSameTeam(ent, victim) && victim->client->ps.groundEntityNum != ent - g_entities)
        return;
...
void G_TouchTriggers(gentity_t *ent)
{
    ...
    BG_ClassBoundingBox(ent->client->ps.stats[STAT_CLASS], pmins, pmaxs, NULL, NULL, NULL);
...
=================
*/
bool ClientInactivityTimer(gentity_t *ent)
{
    ...
    {
        if (level.time > client->inactivityTime && !G_admin_permission(ent, ADMF_ACTIVITY))
        {
...
void ClientTimerActions(gentity_t *ent, int msec)
{
    ...
    usercmd_t *ucmd;
    int aForward, aRight;
    bool walking = false, stopped = false, crouched = false;
    int i;
    ...
    while (client->time100 >= 100)
    {
        ...
        if (weapon == WP_ABUILD || weapon == WP_ABUILD2 || client->ps.stats[STAT_WEAPON] == WP_HBUILD)
        {
            // Update build timer
        ...
        switch (weapon)
        {
            ...
            case WP_HBUILD:
                ...
                if ((client->ps.stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT) > BA_NONE)
                {
                    ...
                    if (G_CanBuild(ent, client->ps.stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT, dist, dummy, dummy2,
                            &dummy3) == IBE_NONE)
        ...
        if (ent->client->pers.teamSelection == TEAM_HUMANS && (client->ps.stats[STAT_STATE] & SS_HEALING_2X))
        {
            ...
            {
                if (ent->health < ent->client->ps.stats[STAT_MAX_HEALTH] && ent->client->medKitHealthToRestore &&
    ...
    while (client->time1000 >= 1000)
    {
        ...
        {
            G_Damage(ent, NULL, NULL, NULL, NULL, BG_Class(client->ps.stats[STAT_CLASS])->regenRate, DAMAGE_NO_ARMOR,
                MOD_SUICIDE);
        }
        ...
        if (g_freeFundPeriod.integer > 0 && client->pers.secondsAlive % g_freeFundPeriod.integer == 0)
        {
            // Give clients some credit periodically
            if (G_TimeTilSuddenDeath() > 0)
            {
                if (client->ps.stats[STAT_TEAM] == TEAM_ALIENS)
                    G_AddCreditToClient(client, FREEKILL_ALIEN, true);
                else if (client->ps.stats[STAT_TEAM] == TEAM_HUMANS)
                    G_AddCreditToClient(client, FREEKILL_HUMAN, true);
            }
        }
    }
    ...
    while (client->time10000 >= 10000)
    {
        ...
        if (ent->client->pers.hasHealed)
        {
            ...
            ent->client->pers.hasHealed = false;
        }
    }
...
void ClientEvents(gentity_t *ent, int oldEventSequence)
{
    ...
    client = ent->client;
    klass = client->ps.stats[STAT_CLASS];
    ...
    for (i = oldEventSequence; i < client->ps.eventSequence; i++)
    {
        ...
        switch (event)
        {
            ...
            case EV_FALL_FAR:
                ...
                damage = (int)((float)BG_Class(klass)->health * BG_Class(klass)->fallDamage * fallDistance);

                VectorSet(dir, 0, 0, 1);
                BG_ClassBoundingBox(klass, mins, NULL, NULL, NULL, NULL);
                mins[0] = mins[1] = 0.0f;
...
void G_UnlaggedCalc(int time, gentity_t *rewindEnt)
{
    ...
    for (i = 0; i < level.maxclients; i++)
    {
        ...
        VectorLerp2(lerp, ent->client->unlaggedHist[startIndex].mins, ent->client->unlaggedHist[stopIndex].mins,
            ent->client->unlaggedCalc.mins);
        VectorLerp2(lerp, ent->client->unlaggedHist[startIndex].maxs, ent->client->unlaggedHist[stopIndex].maxs,
            ent->client->unlaggedCalc.maxs);
        VectorLerp2(lerp, ent->client->unlaggedHist[startIndex].origin, ent->client->unlaggedHist[stopIndex].origin,
...
static void G_UnlaggedDetectCollisions(gentity_t *ent)
{
    ...
    trap_Trace(
        &tr, ent->client->oldOrigin, ent->r.mins, ent->r.maxs, ent->client->ps.origin, ent->s.number, MASK_PLAYERSOLID);
...
void ClientThink_real(gentity_t *ent)
{
    ...
    if ((client->ps.eFlags & EF_POISONCLOUDED) &&
        BG_PlayerPoisonCloudTime(&client->ps) - level.time + client->lastPoisonCloudedTime <= 0)
        ...
        else
        {
            ...
            for (i = 0; i < num; i++)
            {
                ...
                if (modifier < BOOSTER_REGEN_MOD && boost->s.eType == ET_BUILDABLE &&
                    boost->s.modelindex == BA_A_BOOSTER && boost->spawned && boost->health > 0 && boost->powered)
                {
                ...
                if (boost->s.eType == ET_PLAYER && boost->client &&
                    boost->client->pers.teamSelection == ent->client->pers.teamSelection && boost->health > 0)
                {
                    class_t klass = boost->client->ps.stats[STAT_CLASS];
                    bool didBoost = false;

                    if (klass == PCL_ALIEN_LEVEL1 && modifier < LEVEL1_REGEN_MOD)
                    {
                        modifier = LEVEL1_REGEN_MOD;
                        didBoost = true;
                    }
                    else if (klass == PCL_ALIEN_LEVEL1_UPG && modifier < LEVEL1_UPG_REGEN_MOD)
                    {
                    ...
                    if (didBoost && ent->health < client->ps.stats[STAT_MAX_HEALTH])
                        boost->client->pers.hasHealed = true;
                }
            }
...
#define USE_OBJECT_RANGE 64
        ...
        else
        {
            ...
            {
                if (BG_AlienCanEvolve(client->ps.stats[STAT_CLASS], client->pers.credit, g_alienStage.integer))
                {
                    // no nearby objects and alien - show klass menu
    ...
    client->ps.persistant[PERS_MARKEDBP] = G_GetMarkedBuildPoints(client->ps.origin, client->ps.stats[STAT_TEAM]);
