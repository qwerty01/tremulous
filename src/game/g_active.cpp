...
void  G_TouchTriggers( gentity_t* ent )
{
    ...
    BG_ClassBoundingBox( ( class_t )ent->client->ps.stats[ STAT_CLASS ],
...
void ClientTimerActions( gentity_t* ent, int msec )
{
    ...
    usercmd_t* ucmd;
    int       aForward, aRight;
    bool  walking = false, stopped = false,
          crouched = false;
    int       i;
    ...
    while( client->time100 >= 100 )
    {
        ...
        switch( weapon )
        {
            ...
            case WP_HBUILD:
                ...
                {
                    int     dist = BG_Class( ( class_t )ent->client->ps.stats[ STAT_CLASS ] )->buildDist;
                    vec3_t  dummy, dummy2;
                    int     dummy3;
                    
                    if( G_CanBuild( ent, ( buildable_t )( client->ps.stats[ STAT_BUILDABLE ] & ~SB_VALID_TOGGLEBIT ),
    ...
    while( client->time1000 >= 1000 )
    {
        ...
        {
            G_Damage( ent, NULL, NULL, NULL, NULL,
                      BG_Class( ( class_t )client->ps.stats[ STAT_CLASS ] )->regenRate,
    ...
    for( i = oldEventSequence; i < client->ps.eventSequence; i++ )
    {
        ...
        switch( event )
        {
            ...
            case EV_FALL_FAR:
                ...
                damage = ( int )( ( float )BG_Class( _class )->health *
                                  BG_Class( _class )->fallDamage * fallDistance );
                                  
                VectorSet( dir, 0, 0, 1 );
                BG_ClassBoundingBox( _class, mins, NULL, NULL, NULL, NULL );
                mins[ 0 ] = mins[ 1 ] = 0.0f;
...
void ClientThink_real( gentity_t* ent )
{
    ...
    {
        float regenRate =
            BG_Class( ( class_t )ent->client->ps.stats[ STAT_CLASS ] )->regenRate;
    ...
    if( client->ps.pm_type == PM_NOCLIP )
        ...
        client->ps.speed = g_speed.value *
                           BG_Class( ( class_t )client->ps.stats[ STAT_CLASS ] )->speed;
...
#define USE_OBJECT_RANGE 64
        ...
        else
        {
            ...
            {
                if( BG_AlienCanEvolve( ( class_t )client->ps.stats[ STAT_CLASS ],
                                       client->pers.credit,
    ...
    client->ps.persistant[ PERS_BP ] = G_GetBuildPoints( client->ps.origin,
                                       ( team_t )client->ps.stats[ STAT_TEAM ] );
