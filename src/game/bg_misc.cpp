...
==============
*/
bool BG_BuildableAllowedInStage(buildable_t buildable, stage_t stage)
{
...
==============
*/
buildableConfig_t *BG_BuildableConfig(buildable_t buildable) { return &bg_buildableConfigList[buildable]; }
...
======================
*/
static bool BG_ParseBuildableFile(const char *filename, buildableConfig_t *bc)
{
    char *text_p;
    int i;
    int len;
    ...
    float scale;
    int defined = 0;
    enum { MODEL = 1 << 0, MODELSCALE = 1 << 1, MINS = 1 << 2, MAXS = 1 << 3, ZOFFSET = 1 << 4 };
...
void BG_InitBuildableConfigs(void)
{
    ...
    for (i = BA_NONE + 1; i < BA_NUM_BUILDABLES; i++)
    {
        ...
        BG_ParseBuildableFile(va("configs/buildables/%s.cfg", BG_Buildable(i)->name), bc);
    }
}
...
==============
*/
bool BG_ClassAllowedInStage(class_t klass, stage_t stage)
{
    int stages = BG_Class(klass)->stages;

    return (bool)(stages & (1 << stage));
}
...
==============
*/
classConfig_t *BG_ClassConfig(class_t klass) { return &bg_classConfigList[klass]; }
...
==============
*/
void BG_ClassBoundingBox(class_t klass, vec3_t mins, vec3_t maxs, vec3_t cmaxs, vec3_t dmins, vec3_t dmaxs)
{
...
==============
*/
int BG_ClassCanEvolveFromTo(class_t fclass, class_t tclass, int credits, int stage, int cost)
{
    int i, j, best, value;
    ...
    for (i = 0; i < bg_numClasses; i++)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            ...
            if (thruClass == tclass)
                ...
                value = BG_ClassCanEvolveFromTo((class_t)thruClass, tclass, credits, (stage_t)stage, cost + evolveCost);
...
==============
*/
bool BG_AlienCanEvolve(class_t klass, int credits, int stage)
{
    int i, j, tclass;
    ...
    for (i = 0; i < bg_numClasses; i++)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            ...
            if (tclass != PCL_NONE && BG_ClassAllowedInStage((class_t)tclass, (stage_t)stage) &&
                BG_ClassIsAllowed(tclass) && credits >= BG_Class(tclass)->cost * ALIEN_CREDITS_PER_KILL)
                return true;
        }
...
======================
*/
static bool BG_ParseClassFile(const char *filename, classConfig_t *cc)
{
    char *text_p;
    int i;
    int len;
...
void BG_InitClassConfigs(void)
{
    ...
    for (i = PCL_NONE; i < PCL_NUM_CLASSES; i++)
    {
        ...
        BG_ParseClassFile(va("configs/classes/%s.cfg", BG_Class(i)->name), cc);
    }
}
...
==============
*/
bool BG_WeaponAllowedInStage(weapon_t weapon, stage_t stage)
{
...
static const upgradeAttributes_t bg_upgrades[] = {
    ...
    },
    {
        ...
        "Ammunition for the currently held weapon.", 0,
        true,  // bool  purchasable;
        false,  // bool  usable;
        TEAM_HUMANS  // team_t    team;
    }};
...
==============
*/
bool BG_UpgradeAllowedInStage(upgrade_t upgrade, stage_t stage)
{
...
const char *eventnames[] = {"EV_NONE",
    ...
    "EV_SHOTGUN", "EV_MASS_DRIVER",

    "EV_MISSILE_HIT", "EV_MISSILE_MISS", "EV_MISSILE_MISS_METAL", "EV_TESLATRAIL",
    "EV_BULLET",  // otherEntity is the shooter

    "EV_LEV1_GRAB", "EV_LEV4_TRAMPLE_PREPARE", "EV_LEV4_TRAMPLE_START",

    "EV_PAIN", "EV_DEATH1", "EV_DEATH2", "EV_DEATH3", "EV_OBITUARY",

    "EV_GIB_PLAYER",

    "EV_BUILD_CONSTRUCT", "EV_BUILD_DESTROY",
...
#ifdef DEBUG_EVENTS
    {
        ...
        if (atof(buf) != 0)
        {
            ...
            Com_Printf(" game event svt %5d -> %5d: num = %20s parm %d\n", ps->pmove_framecount /*ps->commandTime*/,
                ps->eventSequence, BG_EventName(newEvent), eventParm);
#else
        ...
        if (BG_InventoryContainsUpgrade(i, stats))
        {
            ...
            if (slots & slot)
            {
                Com_Printf(S_COLOR_YELLOW
                    "WARNING: held item %d conflicts with "
                    "inventory slot %d\n",
                    i, slot);
            }
...
========================
*/
void BG_AddUpgradeToInventory(int item, int stats[]) { stats[STAT_ITEMS] |= (1 << item); }
...
========================
*/
void BG_RemoveUpgradeFromInventory(int item, int stats[]) { stats[STAT_ITEMS] &= ~(1 << item); }
...
========================
*/
bool BG_InventoryContainsUpgrade(int item, int stats[]) { return (stats[STAT_ITEMS] & (1 << item)); }
...
========================
*/
void BG_ActivateUpgrade(int item, int stats[]) { stats[STAT_ACTIVEITEMS] |= (1 << item); }
...
========================
*/
void BG_DeactivateUpgrade(int item, int stats[]) { stats[STAT_ACTIVEITEMS] &= ~(1 << item); }
...
========================
*/
bool BG_UpgradeIsActive(int item, int stats[]) { return (stats[STAT_ACTIVEITEMS] & (1 << item)); }
...
===============
*/
void BG_PositionBuildableRelativeToPlayer(const playerState_t *ps, const vec3_t mins, const vec3_t maxs,
    void (*trace)(trace_t *, const vec3_t, const vec3_t, const vec3_t, const vec3_t, int, int), vec3_t outOrigin,
...
bool BG_PlayerCanChangeWeapon(playerState_t *ps)
{
    ...
    if (ps->weapon == WP_LUCIFER_CANNON && ps->stats[STAT_MISC] > LCANNON_CHARGE_TIME_MIN)
        return false;
...
void BG_PackEntityNumbers(entityState_t *es, const int *entityNums, int count)
{
    ...
    if (count > MAX_NUM_PACKED_ENTITY_NUMS)
    {
        ...
        Com_Printf(S_COLOR_YELLOW
            "WARNING: A maximum of %d entity numbers can be "
...
===============
*/
void BG_ParseCSVEquipmentList(
    const char *string, weapon_t *weapons, int weaponsSize, upgrade_t *upgrades, int upgradesSize)
{
...
void BG_InitAllowedGameElements(void)
{
    ...
    BG_ParseCSVEquipmentList(
        cvar, bg_disabledGameElements.weapons, WP_NUM_WEAPONS, bg_disabledGameElements.upgrades, UP_NUM_UPGRADES);
...
int BG_LoadEmoticons(emoticon_t *emoticons, int num)
{
    ...
    for (i = 0; i < numFiles && count < num; i++, filePtr += fileLen + 1)
    {
        ...
        if (fileLen < 9 || filePtr[fileLen - 8] != '_' || filePtr[fileLen - 7] < '1' || filePtr[fileLen - 7] > '9')
        {
...
char *G_CopyString(const char *str)
{
    ...
    char *cp = static_cast<char *>(BG_Alloc(size));
    memcpy(cp, str, size);
    return cp;
}
