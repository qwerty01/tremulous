...
//
typedef enum { F_INT, F_FLOAT, F_STRING, F_VECTOR, F_VECTOR4, F_ANGLEHACK } fieldtype_t;
...
bool G_CallSpawn(gentity_t *ent)
{
    ...
    s = static_cast<spawn_t *>(bsearch(ent->classname, spawns, ARRAY_LEN(spawns), sizeof(spawn_t), cmdcmp));
    if (s)
    {
        // found it
        s->spawn(ent);
...
char *G_NewString(const char *string)
{
    ...
    newb = static_cast<char *>(BG_Alloc(l));
...
void G_ParseField(const char *key, const char *value, gentity_t *ent)
{
    ...
    f = static_cast<field_t *>(bsearch(key, fields, ARRAY_LEN(fields), sizeof(field_t), cmdcmp));
    if (!f)
        return;
    b = (byte *)ent;
