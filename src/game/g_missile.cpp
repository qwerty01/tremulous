...
void G_ExplodeMissile(gentity_t *ent)
{
    ...
    if (ent->splashDamage)
        G_RadiusDamage(
            ent->r.currentOrigin, ent->parent, ent->splashDamage, ent->splashRadius, ent, ent->splashMethodOfDeath);
...
void G_MissileImpact(gentity_t *ent, trace_t *trace)
{
    ...
    if (other->takedamage)
    {
        ...
        if (ent->damage)
        {
            ...
            G_Damage(other, ent, attacker, velocity, ent->r.currentOrigin, ent->damage, DAMAGE_NO_LOCDAMAGE,
                ent->methodOfDeath);
        }
    }
    ...
    if (ent->splashDamage)
        G_RadiusDamage(
            trace->endpos, ent->parent, ent->splashDamage, ent->splashRadius, other, ent->splashMethodOfDeath);
    ...
    if (tr.fraction < 1.0f)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, CONTENTS_BODY);
...
void AHive_SearchAndDestroy(gentity_t *self)
{
    ...
    for (i = 0; i < MAX_CLIENTS; i++)
    {
        ...
        if (ent->client && ent->health > 0 && ent->client->ps.stats[STAT_TEAM] == TEAM_HUMANS &&
            (d = DistanceSquared(ent->r.currentOrigin, self->r.currentOrigin),
                (self->target_ent == NULL || d < nearest)))
        {
            trap_Trace(&tr, self->r.currentOrigin, self->r.mins, self->r.maxs, ent->r.currentOrigin, self->r.ownerNum,
                self->clipmask);
