...
typedef enum { CV_ERR, CV_RANDOM, CV_NUMCLIENTS, CV_LASTWIN } conditionVariable_t;

typedef enum { CO_LT, CO_EQ, CO_GT } conditionOp_t;
...
typedef enum { NT_MAP, NT_CONDITION, NT_GOTO, NT_RESUME, NT_LABEL, NT_RETURN } nodeType_t;
...
===============
*/
bool G_MapExists(const char *name) { return trap_FS_FOpenFile(va("maps/%s.bsp", name), NULL, FS_READ) > 0; }
...
{
    return !Q_stricmp(layout, "*BUILTIN*") ||
           trap_FS_FOpenFile(va("layouts/%s/%s.dat", map, layout), NULL, FS_READ) > 0;
}
...
===============
*/
static bool G_RotationExists(char *name)
{
    int i;
...
===============
*/
static bool G_LabelExists(int rotation, char *name)
{
...
===============
*/
static node_t *G_AllocateNode(void) { return static_cast<node_t *>(BG_Alloc(sizeof(node_t))); }
...
===============
*/
static bool G_ParseMapCommandSection(node_t *node, char **text_p)
{
    char *token;
...
===============
*/
static bool G_ParseNode(node_t **node, char *token, char **text_p, bool conditional)
{
...
static bool G_ParseMapRotationFile(const char *fileName)
{
    ...
    for (i = 0; i < mapRotations.numRotations; i++)
    {
        ...
        for (j = 0; j < mr->numNodes; j++)
        {
            ...
            if ((node->type == NT_GOTO || node->type == NT_RESUME) && !G_LabelExists(i, node->u.label.name) &&
...
===============
*/
static bool G_EvaluateMapCondition(condition_t **condition)
{
    bool result = false;
...
bool G_StepMapRotation(int rotation, int nodeIndex, int depth)
{
    ...
    if (depth > 32 && node->type != NT_MAP)
    {
        ...
        G_Printf(
            S_COLOR_YELLOW "WARNING: possible infinite loop in map rotation %s\n", G_RotationNameByIndex(rotation));
        return true;
    }
    ...
    while (step)
    {
        ...
        switch (node->type)
        {
            ...
            case NT_RETURN:
                ...
                if (returnRotation >= 0)
                {
                    G_SetCurrentNodeByIndex(G_NodeIndexAfter(nodeIndex, rotation), rotation);
...
void G_AdvanceMapRotation(int depth)
{
    ...
    {
        G_Printf(
            S_COLOR_YELLOW "WARNING: index incorrect for map rotation %s, trying 0\n", G_RotationNameByIndex(rotation));
        nodeIndex = 0;
        node = G_NodeByIndex(nodeIndex, rotation);
    }
...
===============
*/
bool G_MapRotationActive(void) { return (g_currentMapRotation.integer != NOT_ROTATING); }
