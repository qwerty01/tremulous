...
void ScoreboardMessage(gentity_t *ent)
{
    ...
    trap_SendServerCommand(ent - g_entities, va("scores %i %i%s", level.alienKills, level.humanKills, string));
}
...
void Cmd_VSay_f(gentity_t *ent)
{
    ...
    if (!voice)
    {
        trap_SendServerCommand(ent - g_entities, va("print \"%s: voice '%s' not found\n\"", vsay, voiceName));
        return;
    }
...
void Cmd_CallVote_f(gentity_t *ent)
{
        ...
        !Q_stricmp(vote, "denybuild") || !Q_stricmp(vote, "allowbuild"))
    {
        ...
        if (!arg[0])
        {
            trap_SendServerCommand(ent - g_entities, va("print \"%s: no target\n\"", cmd));
            return;
        }
        ...
        if (!Q_stricmp(vote, "kick") || !Q_stricmp(vote, "mute") || !Q_stricmp(vote, "denybuild"))
        {
            ...
            {
                trap_SendServerCommand(ent - g_entities, va("print \"%s: admin is immune\n\"", cmd));
            ...
            {
                trap_SendServerCommand(ent - g_entities, va("print \"%s: player is not on your team\n\"", cmd));
                return;
            }

            if (!reason[0] && !G_admin_permission(ent, ADMF_UNACCOUNTABLE))
            {
                trap_SendServerCommand(ent - g_entities, va("print \"%s: You must provide a reason\n\"", cmd));
                return;
            }
        }
    }
    ...
    else if (team == TEAM_NONE)
    {
        ...
        else if (!Q_stricmp(vote, "sudden_death"))
        {
            ...
            {
                trap_SendServerCommand(ent - g_entities, va("print \"callvote: Sudden Death is imminent\n\""));
                return;
            }
    ...
    else if (!Q_stricmp(vote, "denybuild"))
    {
        ...
        {
            trap_SendServerCommand(ent - g_entities, va("print \"%s: player already lost building rights\n\"", cmd));
            return;
        }
    ...
    else if (!Q_stricmp(vote, "allowbuild"))
    {
        if (!level.clients[clientNum].pers.namelog->denyBuild)
        {
            trap_SendServerCommand(ent - g_entities, va("print \"%s: player already has building rights\n\"", cmd));
            return;
        }
    ...
    trap_SetConfigstring(CS_VOTE_TIME + team, va("%d", level.voteTime[team]));
    trap_SetConfigstring(CS_VOTE_STRING + team, level.voteDisplayString[team]);
...
void Cmd_Vote_f(gentity_t *ent)
{
    ...
    trap_SendServerCommand(ent - g_entities, va("print \"%s: vote cast\n\"", cmd));
...
static bool G_RoomForClassChange(gentity_t *ent, int newClass, vec3_t newOrigin)
{
    ...
    // make REALLY sure
    trap_Trace(&tr, newOrigin, toMins, toMaxs, newOrigin, ent->s.number, MASK_PLAYERSOLID);
...
void Cmd_Follow_f(gentity_t *ent)
{
    ...
    else
    {
        ...
        if (i == -1)
        {
            trap_SendServerCommand(ent - g_entities, va("print \"follow: %s\"", err));
            return;
        }
...
commands_t cmds[] = {{"a", CMD_MESSAGE | CMD_INTERMISSION, Cmd_AdminMessage_f},
    {"build", CMD_TEAM | CMD_ALIVE, Cmd_Build_f}, {"buy", CMD_HUMAN | CMD_ALIVE, Cmd_Buy_f},
    {"callteamvote", CMD_MESSAGE | CMD_TEAM, Cmd_CallVote_f}, {"callvote", CMD_MESSAGE, Cmd_CallVote_f},
    {"class", CMD_TEAM, Cmd_Class_f}, {"damage", CMD_CHEAT | CMD_ALIVE, Cmd_Damage_f},
    {"deconstruct", CMD_TEAM | CMD_ALIVE, Cmd_Destroy_f}, {"destroy", CMD_CHEAT | CMD_TEAM | CMD_ALIVE, Cmd_Destroy_f},
    {"drop", CMD_HUMAN | CMD_CHEAT, Cmd_Drop_f}, {"follow", CMD_SPEC, Cmd_Follow_f},
    {"follownext", CMD_SPEC, Cmd_FollowCycle_f}, {"followprev", CMD_SPEC, Cmd_FollowCycle_f},
    {"give", CMD_CHEAT | CMD_TEAM, Cmd_Give_f}, {"god", CMD_CHEAT, Cmd_God_f}, {"ignore", 0, Cmd_Ignore_f},
    {"itemact", CMD_HUMAN | CMD_ALIVE, Cmd_ActivateItem_f}, {"itemdeact", CMD_HUMAN | CMD_ALIVE, Cmd_DeActivateItem_f},
    {"itemtoggle", CMD_HUMAN | CMD_ALIVE, Cmd_ToggleItem_f}, {"kill", CMD_TEAM | CMD_ALIVE, Cmd_Kill_f},
    ...
    {"lua", CMD_MESSAGE | CMD_INTERMISSION, Cmd_LuaLoad_f}, {"m", CMD_MESSAGE | CMD_INTERMISSION, Cmd_PrivateMessage_f},
    {"mt", CMD_MESSAGE | CMD_INTERMISSION, Cmd_PrivateMessage_f}, {"noclip", CMD_CHEAT_TEAM, Cmd_Noclip_f},
    {"notarget", CMD_CHEAT | CMD_TEAM | CMD_ALIVE, Cmd_Notarget_f}, {"reload", CMD_HUMAN | CMD_ALIVE, Cmd_Reload_f},
    {"say", CMD_MESSAGE | CMD_INTERMISSION, Cmd_Say_f}, {"say_area", CMD_MESSAGE | CMD_TEAM | CMD_ALIVE, Cmd_SayArea_f},
    {"say_team", CMD_MESSAGE | CMD_INTERMISSION, Cmd_Say_f}, {"score", CMD_INTERMISSION, ScoreboardMessage},
    {"sell", CMD_HUMAN | CMD_ALIVE, Cmd_Sell_f}, {"setviewpos", CMD_CHEAT_TEAM, Cmd_SetViewpos_f},
    {"team", 0, Cmd_Team_f}, {"teamvote", CMD_TEAM, Cmd_Vote_f}, {"test", CMD_CHEAT, Cmd_Test_f},
    {"unignore", 0, Cmd_Ignore_f}, {"vote", 0, Cmd_Vote_f}, {"vsay", CMD_MESSAGE | CMD_INTERMISSION, Cmd_VSay_f},
    {"vsay_local", CMD_MESSAGE | CMD_INTERMISSION, Cmd_VSay_f},
    {"vsay_team", CMD_MESSAGE | CMD_INTERMISSION, Cmd_VSay_f}, {"where", 0, Cmd_Where_f}};
