...
static bool G_RoomForClassChange( gentity_t*, class_t, vec3_t );
...
static void Give_Class( gentity_t* ent, char* s )
{
    ...
    if( !G_RoomForClassChange( ent, ( class_t )newClass, infestOrigin ) )
    {
    ...
    ent->client->pers.classSelection = ( class_t ) newClass;
...
static void Give_Gun( gentity_t* ent, char* s )
{
    ...
    ent->client->ps.ammo = BG_Weapon( ( weapon_t )w )->maxAmmo;
    ent->client->ps.clips = BG_Weapon( ( weapon_t )w )->maxClips;
    G_ForceWeaponChange( ent, ( weapon_t )w );
}
...
void Cmd_Give_f( gentity_t* ent )
{
    ...
    if( give_all || Q_stricmp( name, "ammo" ) == 0 )
    {
        ...
        if( client->ps.weapon != WP_ALEVEL3_UPG &&
                BG_Weapon( ( weapon_t )client->ps.weapon )->infiniteAmmo )
            return;
            
        client->ps.ammo = BG_Weapon( ( weapon_t )client->ps.weapon )->maxAmmo;
        client->ps.clips = BG_Weapon( ( weapon_t )client->ps.weapon )->maxClips;
        
        if( BG_Weapon( ( weapon_t )client->ps.weapon )->usesEnergy &&
...
void Cmd_Drop_f( gentity_t* ent )
{
    ...
    switch( ( i = BG_WeaponByName( t )->number ) )
    {
        ...
        default:
            G_DropWeapon( ent, ( weapon_t )i, ang );
            break;
    };
...
static bool G_RoomForClassChange( gentity_t* ent, class_t _class,
                                  vec3_t newOrigin )
{
    ...
    BG_ClassBoundingBox( _class, toMins, toMaxs, NULL, NULL, NULL );
...
void Cmd_Class_f( gentity_t* ent )
{
    ...
    if( ent->client->sess.spectatorState != SPECTATOR_NOT )
    {
        ...
        if( ent->client->pers.teamSelection == TEAM_ALIENS )
        {
            ...
            if( !BG_ClassAllowedInStage( newClass, ( stage_t )g_alienStage.integer ) )
            {
        ...
        if( traceEnt->health > 0 )
        {
            if( !deconstruct )
            {
                G_Damage( traceEnt, ent, ent, forward, tr.endpos,
                     ...
                     ( ent->client->pers.teamSelection != TEAM_HUMANS ||
                       G_FindPower( traceEnt , true ) || lastSpawn ) )
            {
            ...
            else
            {
                if( !g_cheats.integer ) // add a bit to the build timer
                {
                    ent->client->ps.stats[ STAT_MISC ] +=
                        BG_Buildable( ( buildable_t )traceEnt->s.modelindex )->buildTime / 4;
                }
                G_Damage( traceEnt, ent, ent, forward, tr.endpos,
...
void Cmd_Buy_f( gentity_t* ent )
{
    ...
    if( G_BuildableRange( ent->client->ps.origin, 100, BA_H_ARMOURY ) )
        energyOnly = false;
             ...
             BG_Weapon( ( weapon_t )ent->client->ps.stats[ STAT_WEAPON ] )->usesEnergy &&
    ...
    {
        if( upgrade == UP_AMMO &&
                BG_Weapon( ( weapon_t )ent->client->ps.weapon )->usesEnergy )
    ...
    else if( !Q_stricmp( s, "upgrades" ) )
    {
        for( i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++ )
        {
                    ...
                    BG_Upgrade( ( upgrade_t )i )->purchasable )
            {
                ...
                //add to funds
                G_AddCreditToClient( ent->client, ( short )BG_Upgrade( ( upgrade_t )i )->price, false );
            }
        }
    }
    else
...
void Cmd_Build_f( gentity_t* ent )
{
    ...
    if( buildable == BA_NONE || !BG_BuildableIsAllowed( buildable ) ||
            ...
            ( team == TEAM_ALIENS && !BG_BuildableAllowedInStage( buildable, ( stage_t )g_alienStage.integer ) ) ||
            ( team == TEAM_HUMANS && !BG_BuildableAllowedInStage( buildable, ( stage_t )g_humanStage.integer ) ) )
    {
...
void Cmd_Reload_f( gentity_t* ent )
{
    ...
    if( BG_Weapon( ( weapon_t )ps->weapon )->infiniteAmmo )
        return;
        
    if( ps->clips <= 0 )
        return;
        
    if( BG_Weapon( ( weapon_t )ps->weapon )->usesEnergy &&
            BG_InventoryContainsUpgrade( UP_BATTPACK, ps->stats ) )
        ammo = BG_Weapon( ( weapon_t )ps->weapon )->maxAmmo * BATTPACK_MODIFIER;
    else
        ammo = BG_Weapon( ( weapon_t )ps->weapon )->maxAmmo;
