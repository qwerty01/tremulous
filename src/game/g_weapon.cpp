...
{
    int maxAmmo;
    weapon_t weapon = ( weapon_t )ent->client->ps.stats[ STAT_WEAPON ];
...
void CheckCkitRepair( gentity_t* ent )
{
    ...
    {
        bHealth = BG_Buildable( ( buildable_t )traceEnt->s.modelindex )->health;
        if( traceEnt->health < bHealth )
        {
            ...
            ent->client->ps.weaponTime += BG_Weapon( ( weapon_t )ent->client->ps.weapon )->repeatRate1;
        }
    }
}
...
{
    buildable_t buildable = ( buildable_t )( ent->client->ps.stats[ STAT_BUILDABLE ]
                            & ~SB_VALID_TOGGLEBIT );
    ...
    for( i = 0; i < num; i++ )
    {
        ...
        if( ( ( enemy->client &&
                ...
                ( enemy->s.eType == ET_BUILDABLE &&
                  BG_Buildable( ( buildable_t )enemy->s.modelindex )->team == TEAM_HUMANS ) ) &&
...
void areaZapFire( gentity_t* ent )
{
    ...
    if( ( traceEnt->client && traceEnt->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS ) ||
            ( traceEnt->s.eType == ET_BUILDABLE &&
              BG_Buildable( ( buildable_t )traceEnt->s.modelindex )->team == TEAM_HUMANS ) )
    {
...
void G_CrushAttack( gentity_t* ent, gentity_t* victim )
{
    ...
    jump = BG_Class( ( class_t )ent->client->ps.stats[ STAT_CLASS ] )->jumpMagnitude;
