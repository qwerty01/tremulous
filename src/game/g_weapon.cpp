...
void G_ForceWeaponChange(gentity_t *ent, weapon_t weapon)
{
    ...
    ps->persistant[PERS_NEWWEAPON] = (weapon == WP_BLASTER) ? WP_BLASTER : ps->stats[STAT_WEAPON];
...
void G_GiveClientMaxAmmo(gentity_t *ent, bool buyingEnergyAmmo)
{
    ...
    if (BG_WeaponIsFull(weapon, ent->client->ps.stats, ent->client->ps.ammo, ent->client->ps.clips))
        return;
...
================
*/
static void G_WideTrace(trace_t *tr, gentity_t *ent, float range, float width, float height, gentity_t **target)
{
    vec3_t mins, maxs;
    vec3_t end;
...
static void WideBloodSpurt(gentity_t *attacker, gentity_t *victim, trace_t *tr)
{
    ...
    if (tr)
        ...
        VectorSubtract(attacker->client->ps.origin, victim->r.currentOrigin, normal);
...
===============
*/
void meleeAttack(gentity_t *ent, float range, float width, float height, int damage, meansOfDeath_t mod)
{
    trace_t tr;
    gentity_t *traceEnt;
...
void bulletFire(gentity_t *ent, float spread, int damage, int mod)
{
    ...
    // send bullet impact
    if (traceEnt->takedamage && (traceEnt->s.eType == ET_PLAYER || traceEnt->s.eType == ET_BUILDABLE))
    {
...
void lockBlobLauncherFire(gentity_t *ent) { fire_lockblob(ent, muzzle, forward); }
...
void painSawFire(gentity_t *ent)
{
    ...
    G_WideTrace(&tr, ent, PAINSAW_RANGE, PAINSAW_WIDTH, PAINSAW_HEIGHT, &traceEnt);
...
===============
*/
void LCChargeFire(gentity_t *ent, bool secondary)
{
    if (secondary && ent->client->ps.stats[STAT_MISC] <= 0)
        fire_luciferCannon(
            ent, muzzle, forward, LCANNON_SECONDARY_DAMAGE, LCANNON_SECONDARY_RADIUS, LCANNON_SECONDARY_SPEED);
    else
        fire_luciferCannon(ent, muzzle, forward,
            ent->client->ps.stats[STAT_MISC] * LCANNON_DAMAGE / LCANNON_CHARGE_TIME_MAX, LCANNON_RADIUS, LCANNON_SPEED);
...
void teslaFire(gentity_t *self)
{
    ...
    if (self->enemy->takedamage)
    {
        ...
        G_Damage(self->enemy, self, self, dir, tr.endpos, TESLAGEN_DMG, 0, MOD_TESLAGEN);
    }
...
void CheckCkitRepair(gentity_t *ent)
{
    ...
    if (tr.fraction < 1.0f && traceEnt->spawned && traceEnt->health > 0 && traceEnt->s.eType == ET_BUILDABLE &&
...
void cancelBuildFire(gentity_t *ent)
{
    ...
    if (ent->client->ps.weapon == WP_ABUILD || ent->client->ps.weapon == WP_ABUILD2)
        meleeAttack(
            ent, ABUILDER_CLAW_RANGE, ABUILDER_CLAW_WIDTH, ABUILDER_CLAW_WIDTH, ABUILDER_CLAW_DMG, MOD_ABUILDER_CLAW);
}
...
void buildFire(gentity_t *ent, dynMenu_t menu)
{
    ...
    if (buildable > BA_NONE)
    {
        ...
        if (G_BuildIfValid(ent, buildable))
        {
            if (!g_cheats.integer)
            {
                ent->client->ps.stats[STAT_MISC] += BG_Buildable(buildable)->buildTime;
            }
...
bool CheckVenomAttack(gentity_t *ent)
{
    ...
    G_WideTrace(&tr, ent, LEVEL0_BITE_RANGE, LEVEL0_BITE_WIDTH, LEVEL0_BITE_WIDTH, &traceEnt);
...
void poisonCloud(gentity_t *ent)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        {
            trap_Trace(&tr, muzzle, NULL, NULL, humanPlayer->r.currentOrigin, humanPlayer->s.number, CONTENTS_SOLID);
...
static void G_FindZapChainTargets(zap_t *zap)
{
    ...
    int entityList[MAX_GENTITIES];
    vec3_t range = {LEVEL2_AREAZAP_CHAIN_RANGE, LEVEL2_AREAZAP_CHAIN_RANGE, LEVEL2_AREAZAP_CHAIN_RANGE};
    vec3_t mins, maxs;
    int i, num;
    gentity_t *enemy;
    trace_t tr;
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (((enemy->client && enemy->client->ps.stats[STAT_TEAM] == TEAM_HUMANS) ||
                (enemy->s.eType == ET_BUILDABLE && BG_Buildable(enemy->s.modelindex)->team == TEAM_HUMANS)) &&
            ...
            distance <= LEVEL2_AREAZAP_CHAIN_RANGE)
        {
            ...
            trap_Trace(&tr, ent->r.currentOrigin, NULL, NULL, enemy->r.currentOrigin, ent->s.number, CONTENTS_SOLID);
...
static void G_CreateNewZap(gentity_t *creator, gentity_t *target)
{
    ...
    for (i = 0; i < MAX_ZAPS; i++)
    {
        ...
        if (target->health > 0)
        {
            G_Damage(target, creator, creator, forward, target->r.currentOrigin, LEVEL2_AREAZAP_DMG,
            ...
            for (i = 1; i < zap->numTargets; i++)
            {
                G_Damage(zap->targets[i], target, zap->creator, forward, target->r.currentOrigin,
                    LEVEL2_AREAZAP_DMG *
                            (1 - pow((zap->distances[i] / LEVEL2_AREAZAP_CHAIN_RANGE), LEVEL2_AREAZAP_CHAIN_FALLOFF)) +
                        1,
                    DAMAGE_NO_KNOCKBACK | DAMAGE_NO_LOCDAMAGE, MOD_LEVEL2_ZAP);
            }
        }
...
void areaZapFire(gentity_t *ent)
{
    ...
    if ((traceEnt->client && traceEnt->client->ps.stats[STAT_TEAM] == TEAM_HUMANS) ||
        (traceEnt->s.eType == ET_BUILDABLE && BG_Buildable(traceEnt->s.modelindex)->team == TEAM_HUMANS))
    {
...
bool CheckPounceAttack(gentity_t *ent)
{
    ...
    G_WideTrace(&tr, ent, pounceRange, LEVEL3_POUNCE_WIDTH, LEVEL3_POUNCE_WIDTH, &traceEnt);
    if (traceEnt == NULL)
        return false;
    ...
    ent->client->pmext.pouncePayload = 0;
    G_Damage(traceEnt, ent, ent, forward, tr.endpos, damage, DAMAGE_NO_LOCDAMAGE, MOD_LEVEL3_POUNCE);

    return true;
}

void bounceBallFire(gentity_t *ent) { fire_bounceBall(ent, muzzle, forward); }
...
void G_ChargeAttack(gentity_t *ent, gentity_t *victim)
{
    ...
    if (ent->client->ps.stats[STAT_MISC] <= 0 || !(ent->client->ps.stats[STAT_STATE] & SS_CHARGING) ||
    ...
    if (!victim->client)
    {
        ...
        ent->client->trampleBuildablesHit[ent->client->trampleBuildablesHitPos++ % MAX_TRAMPLE_BUILDABLES_TRACKED] =
            victim - g_entities;
    }
    ...
    G_Damage(victim, ent, ent, forward, victim->r.currentOrigin, damage, DAMAGE_NO_LOCDAMAGE, MOD_LEVEL4_TRAMPLE);
...
void G_CrushAttack(gentity_t *ent, gentity_t *victim)
{
    ...
    if (!victim->takedamage ||
        ent->client->ps.origin[2] + ent->r.mins[2] < victim->r.currentOrigin[2] + victim->r.maxs[2] ||
    ...
    G_Damage(victim, ent, ent, dir, victim->r.currentOrigin, damage, DAMAGE_NO_LOCDAMAGE, MOD_LEVEL4_CRUSH);
}
...
void FireWeapon(gentity_t *ent)
{
    ...
    switch (ent->s.weapon)
    {
        ...
        case WP_ALEVEL1_UPG:
            meleeAttack(
                ent, LEVEL1_CLAW_U_RANGE, LEVEL1_CLAW_WIDTH, LEVEL1_CLAW_WIDTH, LEVEL1_CLAW_DMG, MOD_LEVEL1_CLAW);
            break;
        case WP_ALEVEL3:
        ...
        case WP_ALEVEL3_UPG:
            meleeAttack(
                ent, LEVEL3_CLAW_UPG_RANGE, LEVEL3_CLAW_WIDTH, LEVEL3_CLAW_WIDTH, LEVEL3_CLAW_DMG, MOD_LEVEL3_CLAW);
            break;
        case WP_ALEVEL2:
        ...
        case WP_ALEVEL2_UPG:
            meleeAttack(
                ent, LEVEL2_CLAW_U_RANGE, LEVEL2_CLAW_WIDTH, LEVEL2_CLAW_WIDTH, LEVEL2_CLAW_DMG, MOD_LEVEL2_CLAW);
            break;
        case WP_ALEVEL4:
            meleeAttack(
                ent, LEVEL4_CLAW_RANGE, LEVEL4_CLAW_WIDTH, LEVEL4_CLAW_HEIGHT, LEVEL4_CLAW_DMG, MOD_LEVEL4_CLAW);
            break;
