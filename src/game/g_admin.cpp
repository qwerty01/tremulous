...
g_admin_cmd_t g_admin_cmds[ ] =
{
    ...
    {
        "setdevmode", G_admin_setdevmode, false, "setdevmode",
        "switch developer mode on or off",
        "[^3on|off^7]"
    },
    
    {
        "setivo", G_admin_setivo, false, "setivo",
    ...
    {
        "setnextmap", G_admin_setnextmap, false, "setnextmap",
    ...
    {
        "time", G_admin_time, true, "time",
        "show the current local server time",
        ""
    },
    {
        "transform", G_admin_transform, false, "magic",
...
{
    return ( g_admin_cmd_t* )bsearch( cmd, g_admin_cmds, adminNumCmds, sizeof( g_admin_cmd_t ), cmdcmp );
}
...
bool G_admin_builder( gentity_t* ent )
{
    ...
    if( tr.fraction < 1.0f && ( traceEnt->s.eType == ET_BUILDABLE ) )
    {
        ...
        ADMP( va( "^3builder: ^7%s%s%s^7%s\n",
                  BG_Buildable( ( buildable_t )traceEnt->s.modelindex )->humanName,
