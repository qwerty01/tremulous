...
static cvarTable_t gameCvarTable[] =
{
    ...
    {&g_lockTeamsAtStart, "g_lockTeamsAtStart", "0", CVAR_ROM, 0, false},
    ...
    {&g_adminMaxBan, "g_adminMaxBan", "2w", CVAR_ARCHIVE, 0, false},
    ...
    {&g_allowTeamOverlay, "g_allowTeamOverlay", "1", CVAR_ARCHIVE, 0, true},
...
extern "C" Q_EXPORT intptr_t vmMain(int command, int arg0, int arg1, int arg2)
{
    switch (command)
    {
        ...
        case GAME_CLIENT_CONNECT:
            return (intptr_t)ClientConnect(arg0, (bool)arg1);
        ...
        case GAME_CLIENT_USERINFO_CHANGED:
            ClientUserinfoChanged(arg0, false);
            return 0;
...
void G_InitGame(int levelTime, int randomSeed, int restart)
{
    ...
    trap_LocateGameData(
        level.gentities, level.num_entities, sizeof(gentity_t), &level.clients[0].ps, sizeof(level.clients[0]));
...
============
*/
int G_PeekSpawnQueue(spawnQueue_t *sq) { return sq->clients[sq->front]; }
...
void G_SpawnClients(team_t team)
{
    ...
    if (G_GetSpawnQueueLength(sq) > 0 && numSpawns > 0)
    {
        ...
        if ((spawn =
                    G_SelectTremulousSpawnPoint(team, ent->client->pers.lastDeathLocation, spawn_origin, spawn_angles)))
        {
...
void G_CalculateBuildPoints(void)
{
    ...
    // BP queue updates
    while (level.alienBuildPointQueue > 0 && level.alienNextQueueTime < level.time)
    {
        level.alienBuildPointQueue--;
        level.alienNextQueueTime +=
            G_NextQueueTime(level.alienBuildPointQueue, g_alienBuildPoints.integer, g_alienBuildQueueTime.integer);
    }

    while (level.humanBuildPointQueue > 0 && level.humanNextQueueTime < level.time)
    {
        level.humanBuildPointQueue--;
        level.humanNextQueueTime +=
    ...
    else if (G_TimeTilSuddenDeath() <= SUDDENDEATHWARNING && level.suddenDeathWarning < TW_IMMINENT)
    {
        trap_SendServerCommand(-1, va("cp \"Sudden Death in %d seconds!\"", (int)(G_TimeTilSuddenDeath() / 1000)));
        trap_SendServerCommand(
            -1, va("print \"Sudden Death will begin in %d seconds.\n\"", (int)(G_TimeTilSuddenDeath() / 1000)));
    ...
    for (i = MAX_CLIENTS; i < level.num_entities; i++)
    {
        ...
        if (ent->s.eType != ET_BUILDABLE || ent->s.eFlags & EF_DEAD || ent->buildableTeam != TEAM_HUMANS)
            continue;
        ...
        if (ent->usesBuildPointZone && level.buildPointZones[ent->buildPointZone].active)
        {
            ...
            {
                // BP queue updates
                while (zone->queuedBuildPoints > 0 && zone->nextQueueTime < level.time)
                {
                    zone->queuedBuildPoints--;
                    zone->nextQueueTime += G_NextQueueTime(
...
void G_CalculateStages(void)
{
    ...
    if (g_alienCredits.integer >= (int)(ceil((float)g_alienStage2Threshold.integer * alienPlayerCountMod)) &&
    ...
    if (g_alienCredits.integer >= (int)(ceil((float)g_alienStage3Threshold.integer * alienPlayerCountMod)) &&
    ...
    if (g_humanCredits.integer >= (int)(ceil((float)g_humanStage2Threshold.integer * humanPlayerCountMod)) &&
    ...
    if (g_humanCredits.integer >= (int)(ceil((float)g_humanStage3Threshold.integer * humanPlayerCountMod)) &&
    ...
    trap_SetConfigstring(
        CS_ALIEN_STAGES, va("%d %d %d", g_alienStage.integer, g_alienCredits.integer, alienNextStageThreshold));

    trap_SetConfigstring(
        CS_HUMAN_STAGES, va("%d %d %d", g_humanStage.integer, g_humanCredits.integer, humanNextStageThreshold));
}
...
void G_CalculateAvgPlayers(void)
{
    ...
    level.averageNumAlienClients = ((level.averageNumAlienClients * level.numAlienSamples) + level.numAlienClients) /
                                   (float)(level.numAlienSamples + 1);
    level.numAlienSamples++;

    level.averageNumHumanClients = ((level.averageNumHumanClients * level.numHumanSamples) + level.numHumanClients) /
...
void CalculateRanks(void)
{
    ...
    level.numNonSpectatorClients = level.numAlienClientsAlive + level.numHumanClientsAlive;
...
void G_AdminMessage(gentity_t *ent, const char *msg)
{
    ...
    Com_sprintf(string, sizeof(string), "chat %d %d \"%s\"", (int)(ent ? ent - g_entities : -1),
        G_admin_permission(ent, ADMF_ADMINCHAT) ? SAY_ADMINS : SAY_ADMINS_PUBLIC, msg);
    ...
    G_LogPrintf("%s: %d \"%s" S_COLOR_WHITE "\": " S_COLOR_MAGENTA "%s\n",
        G_admin_permission(ent, ADMF_ADMINCHAT) ? "AdminMsg" : "AdminMsgPublic", (int)(ent ? ent - g_entities : -1),
...
void CheckExitRules(void)
{
    ...
    if (level.uncondHumanWin || (!level.uncondAlienWin && (level.time > level.startTime + 1000) &&
                                    (level.numAlienSpawns == 0) && (level.numAlienClientsAlive == 0)))
    {
        // humans win
    ...
    else if (level.uncondAlienWin || ((level.time > level.startTime + 1000) && (level.numHumanSpawns == 0) &&
                                         (level.numHumanClientsAlive == 0)))
    {
        // aliens win
...
void G_Vote(gentity_t *ent, team_t team, bool voting)
{
    ...
    if (ent->client->pers.vote & (1 << team))
    {
        ...
        trap_SetConfigstring(CS_VOTE_YES + team, va("%d", level.voteYes[team]));
    }
    else
    {
        if (voting)
        ...
        trap_SetConfigstring(CS_VOTE_NO + team, va("%d", level.voteNo[team]));
    }
}
...
void G_CheckVote(team_t team)
{
    ...
    if (level.voteExecuteTime[team] && level.voteExecuteTime[team] < level.time)
    {
        G_ExecuteVote(team);
    }
        ...
        (level.voteYes[team] + level.voteNo[team] == level.numVotingClients[team]))
    {
        pass = (level.voteYes[team] &&
                (float)level.voteYes[team] / ((float)level.voteYes[team] + (float)level.voteNo[team]) >
                    votePassThreshold);
    }
    else
    {
        if ((float)level.voteYes[team] > (float)level.numVotingClients[team] * votePassThreshold)
        {
            pass = true;
        }
        else if ((float)level.voteNo[team] <= (float)level.numVotingClients[team] * (1.0f - votePassThreshold))
        {
            return;
        }
    }
    ...
    G_LogPrintf("EndVote: %s %s %d %d %d\n", team == TEAM_NONE ? "global" : BG_TeamName(team), pass ? "pass" : "fail",
        level.voteYes[team], level.voteNo[team], level.numVotingClients[team]);
