...
e_status trap_CIN_StopCinematic(int handle) { return static_cast<e_status>(syscall(UI_CIN_STOPCINEMATIC, handle)); }

// will run a frame of the cinematic but will not draw it.  Will return FMV_EOF if the end of the cinematic has been
// reached.
e_status trap_CIN_RunCinematic(int handle) { return static_cast<e_status>(syscall(UI_CIN_RUNCINEMATIC, handle)); }
