                             ...
                             int textvalign, vec4_t color, int textStyle )
{
    ...
    switch( item->type )
    {
        ...
        case INFOTYPE_CLASS:
            value = ( BG_ClassCanEvolveFromTo( ( class_t )_class, item->v.pclass, credits, UI_GetCurrentAlienStage(), 0 ) +
...
{
    uiInfo.alienClassList[uiInfo.alienClassCount].text = BG_ClassConfig( _class )->humanName;
    uiInfo.alienClassList[uiInfo.alienClassCount].cmd = String_Alloc( va( "cmd class %s\n", BG_Class( _class )->name ) );
...
static void UI_LoadHumanArmouryBuys( void )
{
    ...
    for( i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++ )
    {
        if( uiInfo.weapons & ( 1 << i ) )
            slots |= BG_Weapon( ( weapon_t )i )->slots;
    }
    
    for( i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++ )
    {
        if( uiInfo.upgrades & ( 1 << i ) )
            slots |= BG_Upgrade( ( upgrade_t )i )->slots;
    }
    ...
    for( i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++ )
    {
        if( BG_Weapon( ( weapon_t )i )->team == TEAM_HUMANS && BG_Weapon( ( weapon_t )i )->purchasable && BG_WeaponAllowedInStage( ( weapon_t )i, stage ) &&
                BG_WeaponIsAllowed( ( weapon_t )i ) && !( BG_Weapon( ( weapon_t )i )->slots & slots ) && !( uiInfo.weapons & ( 1 << i ) ) )
        {
            uiInfo.humanArmouryBuyList[j].text = BG_Weapon( ( weapon_t )i )->humanName;
            uiInfo.humanArmouryBuyList[j].cmd = String_Alloc( va( "cmd buy %s\n", BG_Weapon( ( weapon_t )i )->name ) );
    ...
    for( i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++ )
    {
        if( BG_Upgrade( ( upgrade_t )i )->team == TEAM_HUMANS && BG_Upgrade( ( upgrade_t )i )->purchasable && BG_UpgradeAllowedInStage( ( upgrade_t )i, stage ) &&
                BG_UpgradeIsAllowed( ( upgrade_t )i ) && !( BG_Upgrade( ( upgrade_t )i )->slots & slots ) && !( uiInfo.upgrades & ( 1 << i ) ) )
        {
            uiInfo.humanArmouryBuyList[j].text = BG_Upgrade( ( upgrade_t )i )->humanName;
            uiInfo.humanArmouryBuyList[j].cmd = String_Alloc( va( "cmd buy %s\n", BG_Upgrade( ( upgrade_t )i )->name ) );
...
static void UI_LoadHumanArmourySells( void )
{
    ...
    for( i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++ )
    {
        if( uiInfo.weapons & ( 1 << i ) )
        {
            uiInfo.humanArmourySellList[j].text = BG_Weapon( ( weapon_t )i )->humanName;
    ...
    for( i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++ )
    {
        if( uiInfo.upgrades & ( 1 << i ) )
        {
            uiInfo.humanArmourySellList[j].text = BG_Upgrade( ( upgrade_t )i )->humanName;
            uiInfo.humanArmourySellList[j].cmd = String_Alloc( va( "cmd sell %s\n", BG_Upgrade( ( upgrade_t )i )->name ) );
...
static void UI_LoadAlienUpgrades( void )
{
    ...
    {
        if( BG_ClassCanEvolveFromTo( ( class_t )_class, ( class_t )i, credits, stage, 0 ) >= 0 )
        {
            uiInfo.alienUpgradeList[j].text = BG_ClassConfig( ( class_t )i )->humanName;
...
static void UI_LoadAlienBuilds( void )
{
    ...
    {
        if( BG_Buildable( ( buildable_t )i )->team == TEAM_ALIENS && BG_Buildable( ( buildable_t )i )->buildWeapon & uiInfo.weapons &&
                BG_BuildableAllowedInStage( ( buildable_t )i, stage ) && BG_BuildableIsAllowed( ( buildable_t )i ) )
        {
            uiInfo.alienBuildList[j].text = BG_Buildable( ( buildable_t )i )->humanName;
            uiInfo.alienBuildList[j].cmd = String_Alloc( va( "cmd build %s\n", BG_Buildable( ( buildable_t )i )->name ) );
...
static void UI_LoadHumanBuilds( void )
{
    ...
    for( i = BA_NONE + 1; i < BA_NUM_BUILDABLES; i++ )
    {
        if( BG_Buildable( ( buildable_t )i )->team == TEAM_HUMANS && BG_Buildable( ( buildable_t )i )->buildWeapon & uiInfo.weapons &&
                BG_BuildableAllowedInStage( ( buildable_t )i, stage ) && BG_BuildableIsAllowed( ( buildable_t )i ) )
        {
            uiInfo.humanBuildList[j].text = BG_Buildable( ( buildable_t )i )->humanName;
...
int UI_MousePosition( void )
{
    return ( int )rint( uiInfo.uiDC.cursorx ) | ( int )rint( uiInfo.uiDC.cursory ) << 16;
}
