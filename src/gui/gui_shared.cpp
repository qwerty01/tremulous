...
bool voidFunction2( itemDef_t* var1, int var2 )
{
    return false;
}
...
void Item_RunScript( itemDef_t* item, const char* s )
{
    ...
    if( item && s && s[0] )
    {
        ...
        while( 1 )
        {
            ...
            cmd = ( commandDef_t* )bsearch( command, commandList, scriptCommandCount, sizeof( commandDef_t ), commandComp );
            if( cmd )
                cmd->handler( item, &p );
            else
...
void Rect_ToWindowCoords( rectDef_t* rect, Window* window )
{
...
// FIXME:
bool Display_MouseMove( void* p, float x, float y )
{
    int i;
